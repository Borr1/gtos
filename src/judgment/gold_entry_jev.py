...
def _score_question(qid: str, instructions: str) -> dict[str, Any]:
    ...
    if isinstance(criteria, list):
        kept = [str(item) for item in criteria if not _bad_text(str(item))]
        if kept:
            body["criteria"] = kept
        else:
...
def _read(family: str, answers: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if in_tree:
        for chunk in chunks_for(fam):
            depths[chunk] = _score_value(answers.get(f"include_{chunk}"))
        if quality_id:
            quality = _score_value(answers.get(quality_id))
        if fam == "news_fade":
