"""One Challenge fluid-gate cycle: ALIVE_MENU → CONF_GATE → DONE_OUTSIDE (+ S14/S15/everywhere).
...
S14 ``REGIME_GATE_SHADOW``, S15 ``COST_OF_ERROR``, Jev-everywhere, and
P0 ``warroom_shadow`` stubs compose on this same admit sidecar — not a
parallel place path. Outcomes are labels only. P0 ``apply`` is always
false.
"""
...
from .od13_ensemble import compose_od13_ensemble
from .place_choice import PlaceChoiceStamp, jev_repeatability_probe, stamp_place_choice
from .place_consume_flags import already_live_consume
from .everywhere import EVERYWHERE_QUESTION_IDS, EverywhereCompose, compose_everywhere
from .p0_hist_prove import hist_prove_allows_label_apply
from .p0_shadow_hooks import WarroomShadow, compose_warroom_shadow
from .research_armed import attach_research_overlay
...
EVERYWHERE_FANOUT_QUESTION_IDS = S14_FANOUT_QUESTION_IDS + EVERYWHERE_QUESTION_IDS
...
class CycleResult:
    ...
    everywhere: EverywhereCompose | None = None
    warroom_shadow: WarroomShadow | None = None
    place_choice: PlaceChoiceStamp | None = None
    conf_order: ConfOrderBlock | None = None
    od13_ensemble: dict[str, object] | None = None
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "everywhere": None if self.everywhere is None else self.everywhere.as_dict(),
            "warroom_shadow": None
            if self.warroom_shadow is None
            else self.warroom_shadow.as_dict(),
            "place_choice": None if self.place_choice is None else self.place_choice.as_dict(),
            "conf_order_block": None if self.conf_order is None else self.conf_order.as_dict(),
            "od13_ensemble": self.od13_ensemble,
        }
...
def _extract_noul(answers: Mapping[str, object] | None) -> float | None:
    if not answers:
        return None
    for payload in answers.values():
        if not isinstance(payload, Mapping) or "noul" not in payload:
            continue
        try:
...
) -> tuple[PlaceChoiceStamp, ConfOrderBlock, dict[str, object], dict[str, object]]:
    """Dig F consume: place_choice hashes + conf order-block + OD-13 ensemble."""

    place_stamp = stamp_place_choice(
        menu=menu,
        option_order=menu.option_ids,
        state=gold_state,
        refuse_incomplete=True,
        invented_files=invented_files,
    )
    order_block = conf_gate_order_block(
        conf_value,
        vendor_deterministic=False,
        stake=stake,
        choice_confidence=conf_value,
        noul=_extract_noul(answers),
    )
    od13 = compose_od13_ensemble(
        place_choice=place_stamp,
        conf_order=order_block,
        invented_files=invented_files,
    )
    repeat = jev_repeatability_probe(
        menu=menu,
        option_order=menu.option_ids,
        state=gold_state,
    )
    return place_stamp, order_block, od13, repeat
...
def run_fluid_gate_cycle(
    *,
    ...
    everywhere_answers: Mapping[str, Any] | None = None,
) -> CycleResult:
    ...
    assert_answers_have_no_place_path(dict(everywhere_answers) if everywhere_answers else None)
    ...
    # Overlay may synthesize identity when gold_state is omitted. S14 still
    # requires a caller-provided state — do not let the overlay invent a band.
    provided_gold = gold_state
    gold_state = attach_research_overlay(gold_state, tags=live.research_armed_tags)
    ...
    if everywhere_answers:
        merged_answers.update(dict(everywhere_answers))
        assert_answers_have_no_place_path(merged_answers)
    ...
    place_stamp, order_block, od13, repeat_probe = _stamp_place_conf(
        menu=menu,
        gold_state=gold_state,
        conf_value=conf_value,
        stake=stake,
        answers=merged_answers or answers,
        invented_files=invented_files,
    )
    ...
    # Optional LABEL APPLY only after FIRE 1201 hist-prove keeps KEEP wins.
    # Default env is still apply=false. Hist-prove itself refuses APPLY env.
    if decision.apply:
        prove_gate = hist_prove_allows_label_apply()
        if not prove_gate.get("allowed"):
            decision = ApplyDecision(
                False,
                str(prove_gate.get("reason") or "hist_prove_keep_wins_not_preserved"),
                "shadow",
            )

    everywhere = compose_everywhere(
        inventory=live,
        answers=merged_answers or answers,
        gold_state=gold_state,
        ...
        regime=regime,
        stake=stake,
        jev_done_choice=jev_done_choice,
        invented_files=invented_files,
    )
    warroom = compose_warroom_shadow(
        gold_state=gold_state,
        regime=regime,
        cost=cost,
        stake=stake,
    ...
    notes = [
        ...
        "chair_enforce_jev_everywhere",
        "chair_cf_sleeve_family_first",
        "chair_cf_d_primary_soft_policy",
        "chair_p0_warroom_shadow_stubs",
        "chair_p0_apply_always_false",
        "chair_research_armed_overlay_module_atr",
        "chair_apply_optional_after_hist_prove_keep_wins",
        ...
        "dig_f_place_choice_hash_stamps",
        "dig_f_conf_order_consume",
        "dig_f_od13_already_live_under_place_apply",
        "dig_f_no_standalone_order_ensemble",
        "dig_f_pack1b_beaten_false",
        *notes_s14,
        *everywhere.notes,
    ]
    ...
    payload = {
        ...
        "inventory": {
            ...
            "research_armed_tags": list(live.research_armed_tags),
        ...
        "fanout_book": {
            "label": "FANOUT_BOOK",
            "questions": list(EVERYWHERE_FANOUT_QUESTION_IDS),
            "note": "independent questions over one state; code consumes branches; "
            "toxic remint is close-only (remint_toxic). S14 regime + Jev-everywhere "
            "site questions share this sidecar call. Never a parallel place path.",
        },
        ...
        "steal_s15": "S15",
        "steal_everywhere": "JEV_EVERYWHERE",
        "regime": None if regime is None else regime.as_dict(),
        "s14_jev_state": s14_jev_state,
        "everywhere": everywhere.as_dict(),
        "warroom_shadow": warroom.as_dict(),
        "place_choice": place_stamp.as_dict(),
        "conf_order_block": order_block.as_dict(),
        "od13_ensemble": od13,
        "jev_repeatability_probe": repeat_probe,
        "already_live_consume": already_live_consume(),
        "steal_p0": "P0_UNWIRED_SHADOW_HOOK_STUBS",
        "close_state": None if close_state is None else dict(close_state),
    }
