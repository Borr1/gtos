...
def _read(answers: Mapping[str, Any], error: str | None) -> dict[str, Any]:
    ...
    admit_parameter = _score_of(answers.get(_Q_ADMIT_PARAMETER), _Q_ADMIT_PARAMETER)
    g4_parameter = _score_of(answers.get(_Q_G4_PARAMETER), _Q_G4_PARAMETER)
    g6_parameter = _score_of(answers.get(_Q_G6_PARAMETER), _Q_G6_PARAMETER)
    g8_parameter = _score_of(answers.get(_Q_G8_PARAMETER), _Q_G8_PARAMETER)
    missing = any(
        item is None
        for item in (
            residual,
