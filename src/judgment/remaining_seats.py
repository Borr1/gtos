...
def _pack_questions(questions: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, block in dict(questions).items():
        ...
        if qtype == "score" and "_anchor_values" not in row and isinstance(row.get("criteria"), list):
            built = _ordinal_block(str(key), str(row.get("instructions") or ""), tuple(row.get("criteria") or ()))
            inner = built.get(str(key)) if isinstance(built, dict) else None
...
def depths_from_answers(
    ...
    for name in chunks:
        ...
        if ordinal_index is not None:
            try:
                snapped = ordinal_index(answers.get(f"include_{name}"), len(_INCLUDE_CRITERIA))
            except Exception:
                snapped = None
        out[name] = snapped
    return out
...
def project_state(state: Mapping[str, Any], depths: Mapping[str, Any]) -> dict[str, Any]:
    """Query-shaped context. An include-depth ordinal snaps to hide, short, long, or full.

    A missing score stays unset. The snapped index is the ordinal, not an amount.
    """
            ...
            if "_anchor_values" in spec:
                ...
                if n_levels >= 2:
                    try:
                        ...
                        value = ordinal_index(block, n_levels)
                    except Exception:
                        value = None
        elif qtype == "noul":
