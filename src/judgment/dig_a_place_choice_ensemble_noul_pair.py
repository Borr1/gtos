...
def _score_question(qid: str, instructions: str, criteria: Sequence[str]) -> dict[str, Any]:
    text = _scrub_text(instructions)
    scale = [_scrub_text(str(item)) for item in criteria if not _banned_text(str(item))]
    block: dict[str, Any] = {}
    try:
    ...
    block["type"] = "score"
    block["instructions"] = text
    block["criteria"] = scale or list(_BETWEEN)
    return {qid: block}
...
def ensemble_place_choice(
    ...
    *,
    ...
    try:
        ...
        questions = pair_questions(menu, _levels(posted))
