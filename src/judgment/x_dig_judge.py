...
def _score_q(qid: str, instructions: str) -> dict[str, Any]:
    ...
    body: dict[str, Any] = {"type": "score", "instructions": text}
    try:
    ...
    if isinstance(criteria, list):
        kept = [_scrub_text(str(item)) for item in criteria if not _limit_key(str(item))]
        ...
        kept_map = {
            str(key): _scrub_text(str(text_item))
            for key, text_item in criteria.items()
            if not _limit_key(str(key))
        }
        kept_map = {key: text_item for key, text_item in kept_map.items() if text_item}
        if kept_map:
            body["criteria"] = kept_map
        else:
            body.pop("criteria", None)
    return {qid: body}
