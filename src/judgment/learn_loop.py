...
def _score_question(qid: str, instructions: str, facts: Mapping[str, Any]) -> dict[str, Any]:
    ...
    block = {"type": "score", "instructions": instructions, "criteria": criteria}
    try:
...
def _score_ask(facts: Mapping[str, Any], qid: str, instructions: str) -> float | None:
    ...
    value = _score_value(answers.get(qid)) if error is None else None
...
def _promotion_fields(
    ...
    *,
    ...
    bound = _score_value(answers.get("learn_loop_promotion_min_n")) if error is None else None
    _remember(
        facts,
        "learn_loop_component_exists",
        exists,
