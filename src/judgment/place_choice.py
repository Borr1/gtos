...
def place_questions(card: Any | None = None) -> dict[str, Any]:
    """Place card. Loop, splice, and width are counts on this card.

    Fewer than two count anchors does not post those Scores. The place
    choice still posts. An empty score stays unset.
    """
...
def build_place_state(intent: Any, *, extra: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        pass
    try:
        _attach_priors(state, place_questions(state))
    except Exception:
...
def _ask_place(state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        questions = place_questions(state)
