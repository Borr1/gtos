...
def _read(answers: Mapping[str, Any], error: str | None) -> dict[str, Any]:
    ...
    row["g4_size_ceiling"] = _score(answers.get(_G4_CEILING_ID), _G4_CEILING_ID)
    row["g6_size_ceiling"] = _score(answers.get(_G6_CEILING_ID), _G6_CEILING_ID)
    row["g7_size_ceiling"] = _score(answers.get(_G7_CEILING_ID), _G7_CEILING_ID)
    row["size_ceiling"] = _score(answers.get(_SIZE_CEILING_ID), _SIZE_CEILING_ID)
    row["threshold"] = _score(answers.get(_THRESHOLD_ID), _THRESHOLD_ID)
    row["loop_bound"] = _score(answers.get(_LOOP_ID), _LOOP_ID)
    row["parameter"] = _score(answers.get(_PARAMETER_ID), _PARAMETER_ID)
    return row
