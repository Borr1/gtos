...
def _read(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    row["threshold"] = _score_value(answers.get("fluid_threshold"))
    row["loop_bound"] = _score_value(answers.get("fluid_loop"))
    row["parameter"] = _score_value(answers.get("fluid_parameter"))
    for name in _NOULS:
        row[name] = _noul(answers.get(name))
    row["component_exists"] = row.get("fluid_component_exists")
    parameters = row["parameters"]
    parameters["fluid_status"] = _score_value(answers.get("fluid_status_parameter"))
    parameters["fluid_next"] = _score_value(answers.get("fluid_next_parameter"))
    parameters["fluid_threshold"] = row["threshold"]
    parameters["fluid_loop"] = row["loop_bound"]
    parameters["fluid_parameter"] = row["parameter"]
    for name in _NOULS:
        parameters[name] = _score_value(answers.get(f"{name}_parameter"))
    return row
