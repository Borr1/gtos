...
def _score_pack(qid: str, levels: list[float]) -> dict[str, Any]:
    instructions = _SCORE_INSTRUCTIONS[qid]
    body: dict[str, Any] = {"type": "score", "instructions": instructions}
    if levels and qid in ("manage_sl_price", "manage_tp_price"):
        body["criteria"] = [str(level) for level in levels]
    try:
        from .jev_questions import parameter_question

        built = parameter_question(qid, instructions)
        if isinstance(built, dict) and qid in built:
            block = built.get(qid)
            if isinstance(block, dict) and levels and qid in ("manage_sl_price", "manage_tp_price") and not block.get("criteria"):
                block["criteria"] = [str(level) for level in levels]
            return built
    except Exception:
        pass
    return {qid: body}
...
def _questions(act: str, levels: list[float]) -> dict[str, Any]:
    """One card for the ticket. Cash and persistence are not asked again."""

    del act
    pack = _choice_pack(LIFE_ID)
    pack.update(_score_pack("manage_sl_price", levels))
    pack.update(_score_pack("manage_tp_price", levels))
    pack.update(_score_pack("manage_scale", levels))
    return pack
...
def decide(
    ...
    *,
    ...
    try:
        ...
        pass
    try:
        questions = _questions(act, levels)
