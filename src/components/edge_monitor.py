...
class EdgeMonitor:
    ...
    def save_state(self, filepath: str) -> None:
        """Save current state to JSON for persistence across restarts.

        Serialises the full internal state of all three detectors so that
        ``load_state`` can reconstruct an identical monitor later.
        """
        state = {
            # Monitor-level
            "p0": self.p0,
            ...
            # SR
            "sr_R": self._sr.R,
            "sr_n_obs": self._sr.n_obs,
            "sr_n_alarms": self._sr.n_alarms,
            "sr_threshold": self._sr.threshold,
            # CUSUM
            "cusum_S": self._cusum.S,
            "cusum_n_obs": self._cusum.n_obs,
            "cusum_n_alarms": self._cusum.n_alarms,
            "cusum_threshold": self._cusum.threshold,
            # BOCPD
            "bocpd_alpha0": self._bocpd.alpha0,
            "bocpd_beta0": self._bocpd.beta0,
            "bocpd_hazard": self._bocpd.hazard,
            "bocpd_n_obs": self._bocpd.n_obs,
            "bocpd_log_joint": self._bocpd._log_joint.tolist(),
            "bocpd_run_wins": self._bocpd._run_wins.tolist(),
            "bocpd_run_n": self._bocpd._run_n.tolist(),
        }
        p = Path(filepath)
        p.parent.mkdir(parents=True, exist_ok=True)
        with open(p, "w") as f:
            json.dump(state, f, indent=2)
        logger.info("EdgeMonitor state saved: n=%d trades -> %s", self.n_trades, filepath)

    @classmethod
    def load_state(cls, filepath: str) -> "EdgeMonitor":
        """Load monitor state from JSON file.

        Reconstructs the full SR + CUSUM + BOCPD state so monitoring
        continues seamlessly after a process restart.
        """
        ...
        monitor = cls(
            p0=state["p0"],
            p1=state["p1"],
            sr_threshold=state["sr_threshold"],
            cusum_threshold=state["cusum_threshold"],
            bocpd_alpha0=state["bocpd_alpha0"],
            bocpd_beta0=state["bocpd_beta0"],
            bocpd_hazard=state["bocpd_hazard"],
        )

        # Restore monitor-level state
        monitor.n_trades = state["n_trades"]
        monitor._wins = state["wins"]
        monitor._trade_history = state["trade_history"]

        # Restore SR state
        monitor._sr.R = state["sr_R"]
        monitor._sr.n_obs = state["sr_n_obs"]
        monitor._sr.n_alarms = state["sr_n_alarms"]

        # Restore CUSUM state
        monitor._cusum.S = state["cusum_S"]
        monitor._cusum.n_obs = state["cusum_n_obs"]
        monitor._cusum.n_alarms = state["cusum_n_alarms"]

        # Restore BOCPD state. Older/corrupted state files can contain vectors
        # with different lengths; rebuild BOCPD from trade_history rather than
        # letting the next live exit crash the non-blocking monitor update.
        monitor._bocpd.n_obs = state["bocpd_n_obs"]
        monitor._bocpd._log_joint = np.array(state["bocpd_log_joint"], dtype=float)
        monitor._bocpd._run_wins = np.array(state["bocpd_run_wins"], dtype=float)
        monitor._bocpd._run_n = np.array(state["bocpd_run_n"], dtype=float)
        if not monitor._bocpd_state_is_consistent():
            monitor._rebuild_bocpd_from_trade_history()

        logger.info("EdgeMonitor state loaded: n=%d trades from %s", monitor.n_trades, filepath)
        return monitor

    def _bocpd_state_is_consistent(self) -> bool:
        """Return True when persisted BOCPD vectors can be updated safely."""
        n = len(self._bocpd._log_joint)
        if n == 0:
            return False
        if len(self._bocpd._run_wins) != n or len(self._bocpd._run_n) != n:
            return False
        if self._bocpd.n_obs < 0:
            return False
        # For untruncated live state, BOCPD keeps r=0..n_obs. For truncated
        # future state it may be shorter, but never longer than n_obs + 1.
        if n > self._bocpd.n_obs + 1:
            return False
        return bool(np.all(np.isfinite(self._bocpd._log_joint)))

    def _rebuild_bocpd_from_trade_history(self) -> None:
        """Repair only BOCPD from trade_history after a corrupt state load."""
        history = [bool(x) for x in (self._trade_history or [])]
        logger.warning(
            "EdgeMonitor BOCPD state inconsistent; rebuilding BOCPD from %d saved trades",
            len(history),
        )
        self._bocpd = BOCPDDetector(
            alpha0=self._bocpd.alpha0,
            beta0=self._bocpd.beta0,
            hazard=self._bocpd.hazard,
            max_run_length=self._bocpd.max_run_length,
        )
        for outcome in history:
            self._bocpd.update(outcome)
