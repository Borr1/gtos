"""dsp_two_open_bars_down_then_cascade -- unused-window lead 5, 2026-08-21.

The new cash session's first printed bars already leave price 2 ATR below the
close three bars ago near a 20-low. Session mask is the first two cash-session
bars (16:30+16:45 US; 15:30+15:45 where that is the first print after a hole).
Formula verbatim verified_w3.json.
V2 said SHORT (38% up) — WRONG. Side is best_both_halves LONG.
...
MEASURED (never assumed):
  ...
  best_both_halves LONG 0.75/6.0  hold R +0.226  (n_hold 14,662)
  ship 1.0/6.0 same side  hold R +0.186  (n_hold 14,662)  clears +0.12
  day-blocked hold SE 0.043 / MDE80 0.091 / blocks 931
...
from ._unused_a_session_mask import mask_first_two_cash

TAG = "dsp_two_open_bars_down_then_cascade"
...
def _signal(o, h, l, c, v, atr):
    """The discovered condition. Verbatim from verified_w3.json."""
    sig = (c <= np.roll(c,3) - 2.0*atr) & (((o-c)>=1.0*atr) | ((np.roll(o,1)-np.roll(c,1))>=1.0*np.roll(atr,1))) & (l <= np.array([np.min(l[max(0,i-19):i+1]) for i in range(len(l))]) + 0.30*atr)
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    session_mask = False
    try:
        session_mask = bool(mask_first_two_cash(symbol=symbol, bar_time=bar_time, bar_times=bar_times))
    except Exception:
        session_mask = False
    facts = {
        "sleeve": TAG,
    ...
    facts["session_mask_name"] = "mask_first_two_cash"
    pattern_side = "two_open_bars_down_then_cascade"
    spots = [
        {
            "id": "named_surface",
