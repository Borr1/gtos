"""dsp_first_crack_failed_reclaim -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

A long coil or grind at the highs finally prints a first down bar of about 1 ATR. The next
two or three bars cannot reclaim that bar's open. The first crack was real; the failed
reclaim is the last bid, then the air pocket.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.111 (n=179,178)  R_hold +0.103 (n=227,002)
...
TAG = "dsp_first_crack_failed_reclaim"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(16, n):
        coil_rng = np.max(h[t-15:t-3]) - np.min(l[t-15:t-3])
        if coil_rng > 3.5 * atr_s[t]:
            continue
        coil_hi = np.max(h[t-15:t-3])
        cracks = []
        for k in range(t-3, t+1):
            if (c[k] < o[k]) and (body[k] >= 1.0 * atr_s[k]) and (o[k] >= coil_hi - 0.8 * atr_s[t]):
                cracks.append(k)
        if len(cracks) == 0:
            continue
        first = cracks[0]
        failed = all(h[k] <= o[first] + 0.2 * atr_s[t] for k in range(first+1, t+1))
        sig[t] = failed and (c[t] < c[first])
    sig
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "first_crack_failed_reclaim"
    spots = [
        {
            "id": "named_surface",
