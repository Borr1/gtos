"""dsp_spring_first_print_of_range_low -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

The first print of a new 20-bar low after a long box, close back off the low, then the next
displacement is UP through the box. SOL (30) Saturday: one 1.89-ATR down bar made 79.53,
then 4.50 ATR back through 80.33 and kept going. This is the opposite of
kiss_level_not_through (which predicts the take). Both are on this shard; the split is
whether the low is the first print after a flat (spring) or a kiss inside an already-broken
tape (take).

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.066 (n=159,373)  R_hold +0.085 (n=187,864)
...
TAG = "dsp_spring_first_print_of_range_low"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(20,n):
        prior_lo=np.min(l[t-19:t]); first=(l[t]<prior_lo-1e-12)
        box=(np.max(h[t-16:t])-np.min(l[t-16:t]))<=3.5*atr[t]
        out[t]=first and (c[t]<o[t]) and ((c[t]-l[t])>=0.25*atr[t]) and box
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "spring_first_print_of_range_low"
