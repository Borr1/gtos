"""dsp_expanding_up_staircase -- discovered 2026-08-21 by the displacement precondition sweep.

Two or three consecutive up bars, each making a new high, bodies getting larger
(already 1.2-3.7 ATR, not yet 4). Algorithms that are already in keep adding. The
4-ATR bar is the next stair, not a surprise.

MEASURED (never assumed):
  displacement within 3 bars, base rate 1.252%
  discovery 2014-2021 : lift 3.35x   n_fire 35,195   P(disp) 5.27%
  holdout   2022-2026 : lift 2.92x   n_fire 46,064   P(disp) 3.65%
  direction: 63% of displacements resolve UP  ->  V2 up-rate labelled LONG (WRONG)
  geometry.py best_both_halves: SHORT 0.75/6.0  hold R +0.146  (n_hold 47,675)
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Proposed by swarm shard 00. Supporting events [17, 21, 24, 25, 32]; contradicting [6].
What would kill it: After two expanding up bars into a new high, the next 1-3
bars print a >=4 ATR DOWN at similar lift to UP. Event 6 is the in-shard threat;
if that shape is common, the staircase is just 'trend'.
...
TAG = "dsp_expanding_up_staircase"
...
def _signal(o, h, l, c, v, atr):
    ...
    atr_s = np.maximum(atr, 1e-12); body = np.abs(c - o); rng = np.maximum(h - l, 1e-12); n = len(c); sig = np.zeros(n, dtype=bool)
    sig[2:] = (h[2:] > h[1:-1]) & (h[1:-1] > h[:-2]) & (c[2:] > o[2:]) & (c[1:-1] > o[1:-1]) & (body[2:] >= 1.2*atr_s[2:]) & (body[1:-1] >= 1.0*atr_s[1:-1]) & ((c[2:]-l[2:])/rng[2:] >= 0.70)
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "expanding_up_staircase"
    spots = [
        {
            "id": "named_surface",
