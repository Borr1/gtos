"""dsp_bleed_accept_fresh_20low_second_push -- DISPREAD occupancy+geo, 2026-08-21.

The standing bar makes the 20-bar low and closes in the lower half, with ret16
<= -2 ATR or a 2 ATR down body already on this bar. Occupancy called this DOWN
(hold lift 1.41, same-way hold 0.601). Same-way is not a side. Geometry first-
touch 1.0/6.0 is LONG. TTE 12.2 is already 4.99 ATR onto 74.66 then another 5.14
with USOIL -7. The ship side is the snap the other way after the fresh 20-low is
accepted, not the occupancy continuation story. USDCNH 12.6 is the same local
tag then 4.69 ATR UP; LMT 13.30 tags 439.78 and snaps UP. Formula verbatim
DISPREAD-12-23.md §3 standing_20low_is_the_tag_already_falling.

MEASURED (never assumed):
  displacement within 3 bars, base after excludes 0.636% disc / 0.500% hold
  occupancy: lift 1.48x disc / 1.41x hold  n_fire 536,560 / 728,662
  occupancy claimed DOWN (same-way hold 0.601) — WRONG as a side
  geometry.py first-touch 1.0/6.0 LONG  hold R +0.135  (n_hold 700,653)
  (0.75/6.0 is best_both_halves +0.166; ship 1.0/6.0 to stay out of the FX 5-8 pip bleed)
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Proposed by DISPREAD 12-23 §3. Supporting events 12.2 TTE, 12.10 ETH, 12.29 TTE;
contradicting 12.6 USDCNH, 13.30 LMT, 22.2 XAUEUR.
What would kill it: a standing-bar 20-low tag that closes in the lower half,
with ret16<=-2 or a 2-ATR down body, is followed by a >=4 ATR UP (the ship
side) within 3 no more often than any other bar that merely touches the 20-low,
in both halves of a date split.
...
TAG = "dsp_bleed_accept_fresh_20low_second_push"
...
def _signal(o, h, l, c, v, atr):
    """The discovered condition. Verbatim DISPREAD-12-23.md §3 standing_20low_is_the_tag_already_falling."""
    atr_s=np.maximum(atr,1e-12); rng=np.maximum(h-l,1e-12)
    ll20=np.array([np.min(l[max(0,i-19):i+1]) for i in range(len(c))])
    ret16=np.zeros(len(c)); ret16[16:]=(c[16:]-c[:-16])/atr_s[16:]
    signed=(c-o)/atr_s
    sig=(l<=ll20+0.10*atr_s)&((c-l)/rng<=0.50)&((ret16<=-2.0)|(signed<=-2.0))
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "bleed_accept_fresh_20low_second_push"
    spots = [
        {
            "id": "named_surface",
    ...
    try:
        ...
        return None
    try:
        _ex = float(np.min(l[max(0, i-19):i+1]))
