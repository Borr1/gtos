"""dsp_isolated_20h_spike_then_fade -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

One lone 1.5+ ATR up-bar prints a new 20-bar high after a quiet tape or a session hole.
There is no staircase of prior impulse closes on highs. The next 1-3 bars fade it. XOM 11
(15:45 +2.44 ATR to 56.43 after overnight ticks, 16:00 down 4.16) and PLTR 38 (15:30 tags
56.46 and already closes on 55.64, 15:45 down 4.15) are the cash-open costume. RACE 10 is
the late-session costume: the last up impulses were real, but 22:45 still faded and the next
session undid it. UNI 16, XAUEUR 2, and GBPUSD 23 are the same location after a held
staircase and they went through. Isolation versus staircase is what close_pos cannot see.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves SHORT 0.75/6.0
  R_disc +0.106 (n=90,658)  R_hold +0.099 (n=106,930)
...
TAG = "dsp_isolated_20h_spike_then_fade"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(28, n):
        cands = []
        if h[t] >= np.max(h[t-19:t+1]) - 1e-12:
            cands.append(t)
        if h[t-1] >= np.max(h[t-20:t]) - 1e-12:
            cands.append(t-1)
        ok = False
        for i in cands:
            if (c[i] - o[i]) < 1.5 * atr_s[i]:
                continue
            prior = [(c[j] - o[j]) >= 1.2 * atr_s[j] for j in range(i-8, i)]
            if any(prior):
                continue
            if (h[i] - c[t]) < 0.15 * atr_s[t]:
                continue
            ok = True
            break
        sig[t] = ok
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "isolated_20h_spike_then_fade"
    spots = [
        {
            "id": "named_surface",
