"""dsp_already_wide_down_bar_second_wave -- discovered 2026-08-21 by the displacement precondition sweep.

The bar you are standing on is already a 2+ ATR down close, and it is not the
first: an earlier 1.5+ ATR down bar sits in the last 12 and was never repaired.
The 30 features see 'last bar was huge' and stop. A person sees this is wave two
of a book that is already breaking, so wave three is the displacement. ICP 27 is
the extreme (k=-1 is 9.57 ATR after k=-2 was 3.03). ICP 32 k=-1 is 2.06 ATR after
k=-22 was 3.31. XRP 28 k=-1 is 1.42 ATR after the rollover from 2.446. ETH 1 k=-2
is 1.41 ATR after the failed 4016 bounce. This will over-fire if last-bar-huge is
already the whole signal — test it as an AND with the unrepaired earlier impulse,
not as a restate.

MEASURED (never assumed):
  displacement within 3 bars, base rate 1.252%
  discovery 2014-2021 : lift 2.42x   n_fire 30,983   P(disp) 3.80%
  holdout   2022-2026 : lift 2.15x   n_fire 37,377   P(disp) 2.69%
  direction: 35% of displacements resolve UP  ->  V2 up-rate labelled SHORT (WRONG)
  geometry.py first-touch 1.0/6.0 LONG  hold R +0.140  (n_hold 38,712)
  (0.75/6.0 is best_both_halves +0.179; ship 1.0/6.0 to stay out of the FX 5-8 pip bleed)
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Proposed by swarm shard 25. Supporting events [1, 27, 28, 32]; contradicting [16, 25].
What would kill it: Adding 'there was an unrepaired 1.5 ATR down impulse in the
prior 12 bars' does not raise lift over the already-known last-bar-huge rule, or
those second-wave tapes reverse UP inside 3 bars as often as they continue
(16/25-style springs).
...
TAG = "dsp_already_wide_down_bar_second_wave"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(14,n):
        if (o[t]-c[t])<1.2*atr[t]: continue
        if (c[t]-l[t])>0.40*np.maximum(h[t]-l[t], 1e-12): continue
        i=None
        for j in range(t-2, t-13, -1):
            if (o[j]-c[j])>=1.5*atr[j]:
                i=j; break
        if i is None: continue
        if np.max(h[i:t+1]) > o[i]+0.10*atr[t]: continue
        out[t]=c[t]<c[i]
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "already_wide_down_bar_second_wave"
    spots = [
        {
            "id": "named_surface",
