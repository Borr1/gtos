"""dsp_reclaim_then_giveback -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

A 20-bar low was taken and then actually repaired — a later bar closed at least 0.5 ATR
above that low. The bar you are standing on gives the repair back: down, body 0.5+ ATR,
close back near the sweep. The people who bought the reclaim are the fuel. BNB 19 tagged
373.36 at k=-5, climbed to 384.15, then k=-1 dumped to 376.86 (1.51 ATR) and the next bar is
6.17 ATR. Heat 23 dumped to 4.028, ripped to 4.088, then k=-1 closed 4.06 and the next bar
is 4.28 ATR. This is not 'never reclaimed'. Features cannot see a completed repair that then
died.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.154 (n=394,022)  R_hold +0.153 (n=510,938)
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(22,n):
        sl=None; sj=None
        for j in range(t-1, t-9, -1):
            prior=np.min(l[max(j-19,0):j]) if j>0 else l[j]
            if l[j]<=prior+0.05*atr[t]:
                sl=l[j]; sj=j; break
        if sl is None: continue
        if np.max(c[sj:t]) < sl + 0.5*atr[t]: continue
        if (o[t]-c[t]) < 0.50*atr[t]: continue
        if c[t] > sl + 1.0*atr[t]: continue
        if c[t] >= np.max(c[sj:t]): continue
        out[t]=True
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "reclaim_then_giveback"
    spots = [
        {
            "id": "named_surface",
