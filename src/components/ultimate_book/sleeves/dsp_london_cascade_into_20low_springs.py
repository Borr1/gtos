"""dsp_london_cascade_into_20low_springs -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

London's first hour the other way. Three consecutive down bars of 1.0+ ATR each walk into a
fresh 20-low. That is a stop-run of the Asia low in one burst, not a multi-hour bleed; the
next expansion is the buyback. GBPCHF (34) printed 1.52 / 1.52 / 1.16 ATR into 1.14007 at
09:00-09:30, then 09:45 went UP 4.18 ATR. MANUSD (7) and ALGUSD (10) also closed on a new
20-low and continued DOWN — those were hours of bleed with a single modest tag, not a three-
bar London cascade. Event 16 is a one-bar climax at 21:00, a cousin in size, different
clock.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.128 (n=6,378)  R_hold +0.101 (n=8,652)
...
TAG = "dsp_london_cascade_into_20low_springs"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(24, n):
        ...
        cascade = ((o[t] - c[t]) >= 1.00 * atr_s[t]) and ((o[t-1] - c[t-1]) >= 1.00 * atr_s[t-1]) and ((o[t-2] - c[t-2]) >= 1.00 * atr_s[t-2])
        prior = np.max(np.abs(c[t-10:t-2] - o[t-10:t-2]))
        sig[t] = cascade and (l[t] <= ll20 + 1e-12) and (prior < 2.0 * atr_s[t]) and ((c[t] - l[t]) / rng <= 0.34)
    sig
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "london_cascade_into_20low_springs"
    spots = [
        {
            "id": "named_surface",
