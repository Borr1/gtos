"""dsp_shakeout_holds_run_lows -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

An up-run of at least 2 ATR over ten bars is already in place. The standing bar is a 1.0+
ATR down bar whose low does not break the run's rising lows. That is a shakeout of the late
longs, not a change of mind. SOL 1 dumps from 129.29 to 128.01 on 5.16× and holds above
127.29, then 6.93 through 132.60. BAR 32 dumps from 0.05363 to 0.05304 and holds above
0.05273, then 7.87. NEO 13 is not this shape (it is an up-tag on a soft tape) — there is no
clean same-shard case of this exact print then a down displacement.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.146 (n=69,686)  R_hold +0.108 (n=94,474)
...
TAG = "dsp_shakeout_holds_run_lows"
...
def _signal(o, h, l, c, v, atr):
    ...
    atr_s=np.maximum(atr,1e-12); n=len(c); out=np.zeros(n,dtype=bool)
    for t in range(10,n):
        run=(c[t-2]-c[t-10])/atr_s[t]
        out[t]=(run>=2.0)&((o[t]-c[t])/atr_s[t]>=1.0)&(l[t]>np.min(l[t-10:t-1]))&(c[t]>c[t-10])
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "shakeout_holds_run_lows"
