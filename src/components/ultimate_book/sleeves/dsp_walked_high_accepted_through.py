"""dsp_walked_high_accepted_through -- DISPREAD occupancy+geo, 2026-08-21.

The standing bar makes the 20-bar high and closes in the top quarter. Occupancy
called this UP (hold lift 1.22, same-way hold 0.607). Same-way is not a side.
Geometry first-touch 1.0/6.0 is SHORT. SNOW 12.7 closes on 308.12 then the next
session continues; UK100 19.32 / 19.33 close on the number and dump; USDJPY 12.40
is the 6.37 ATR spike against ret16 -7.8 then 10.46 DOWN. The ship side is the
fade of an accepted tag, not the occupancy through-UP story. Formula verbatim
DISPREAD-12-23.md §5 pressed_close_on_fresh_20high_then_through.

MEASURED (never assumed):
  displacement within 3 bars, base after excludes 0.636% disc / 0.500% hold
  occupancy: lift 1.27x disc / 1.22x hold  n_fire 478,833 / 677,826
  occupancy claimed UP (same-way hold 0.607) — WRONG as a side
  geometry.py first-touch 1.0/6.0 SHORT  hold R +0.138  (n_hold 655,906)
  (0.75/6.0 is best_both_halves +0.167; ship 1.0/6.0 to stay out of the FX 5-8 pip bleed)
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Proposed by DISPREAD 12-23 §5. Supporting events 12.7 SNOW, 12.13 NZDJPY, 13.12 XAU;
contradicting 12.40 USDJPY, 19.32 / 19.33 UK100, 21.13 META.
What would kill it: pressed close on a fresh 20-high is followed by DOWN
displacement (the ship side) within 3 at the base rate once you exclude session
holes and bars with ret16<=-2.
...
TAG = "dsp_walked_high_accepted_through"
...
def _signal(o, h, l, c, v, atr):
    """The discovered condition. Verbatim DISPREAD-12-23.md §5 pressed_close_on_fresh_20high_then_through."""
    ...
    sig=(h>=hh20-0.08*atr_s)&((c-l)/rng>=0.75)&~(((c-o)/atr_s>=2.5)&(ret16<=-2.0))
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "walked_high_accepted_through"
