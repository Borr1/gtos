"""dsp_climax_onto_20high_then_fade_cashhole -- UNUSED-GEO-6-10 lead 8 cash_hole. NOT ARMED.

Same isolated-thrust formula as the London sleeve, masked to the first print
after a >=60m hole. Separate fire set. Do not pool with London. Ship 1.0/6.0
LONG (0.75/6.0 won; 1.0/6.0 hold R +0.133 clears the ship gate).

MEASURED (never assumed):
  mask first: first print after >=60m hole
  n after mask: disc=410 / hold=442 (128 symbols fired)
  best_both_halves LONG 0.75/6.0 hold R +0.143  (n_hold 312)
  ship 1.0/6.0 same side hold R +0.133  (n_hold 312) both_positive
...
from .dsp_unused_b_clock import mask_cash_hole

TAG = "dsp_climax_onto_20high_then_fade_cashhole"
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    session_mask = False
    try:
        session_mask = bool(mask_cash_hole(symbol, bar_time, bar_times, n))
    except Exception:
        session_mask = False
    facts = {
        "sleeve": TAG,
    ...
    facts["session_mask_name"] = "mask_cash_hole"
    pattern_side = "climax_onto_20high_then_fade_cashhole"
    spots = [
        {
            "id": "named_surface",
