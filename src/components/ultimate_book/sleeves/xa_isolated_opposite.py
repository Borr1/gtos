"""xa_isolated_opposite -- timing setup 2026-08-21; geometry.py both-direction first-touch. Not armed.

The standing bar is already 3+ ATR and isolated — no other 2 ATR body in the prior eight
bars. The labeled displacement is the fade of that spike, not a continuation. IMX 37 is the
clean print: k=-1 is 10.05 ATR 0.5407 to 0.5557 on 4.40x, closed on the 20-bar high, then
k=0 is -4.72 and it stays down. The known 'last bar was huge' table scores this as an UP
precursor; the movie is DOWN. JPM 18's last bar is 3.75 ATR and the next bar continues UP —
that is a first-cash-bar reclaim, not an isolated spike in a live book. GER40 9's 3.08 ATR
bar is four bars back, not the standing bar. XTZ 36's last bar is only 1.27 ATR.

MEASURED (never assumed):
  displacement within 3 bars, base rate 1.250%
  discovery 2014-2021 : lift 5.47x   n_fire 0   P(disp) 0.00%
  holdout   2022-2026 : lift 5.89x   n_fire 46,723   P(disp) 7.38%
  direction: library LONG agrees with geometry
  geometry.py best_both_halves: LONG 1.0/6.0  hold R +0.163  (n_hold 48,902)
  (0.75/6.0 was +0.226; ship 1.0/6.0 to stay out of the FX 5-8 pip bleed)
  scored over 163/166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Proposed by swarm shard 31. Supporting events None; contradicting None.
What would kill it: An isolated 3 ATR standing bar is followed by a 4 ATR bar the other way no more often than by a same-direction 4 ATR bar, or no better than the 1.6% base.
...
TAG = "xa_isolated_opposite"
SETUP_NAME = "isolated_huge_bar_then_opposite"
GENERATE_KIND = "live"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(16,n):
        at=max(atr[t],1e-12)
        huge=abs(c[t]-o[t])>=3.0*at
        isolated=sum(1 for i in range(t-8,t) if abs(c[i]-o[i])>=2.0*max(atr[i],1e-12))==0
        out[t]=huge and isolated
    out
    sig = out
    return sig
