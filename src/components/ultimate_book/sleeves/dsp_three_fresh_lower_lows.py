"""dsp_three_fresh_lower_lows -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

Three consecutive bars each take a new low by more than 0.15 ATR, with volume not dying.
This is inventory being forced out in sequence, not a held level. The next bar continues
down. Distinct from two taps of the same price.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.081 (n=904,690)  R_hold +0.097 (n=1,287,057)
...
TAG = "dsp_three_fresh_lower_lows"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(20, n):
        fresh = (l[t] < l[t-1] - 0.15*atr_s[t]) and (l[t-1] < l[t-2] - 0.15*atr_s[t])
        downs = int(c[t] < o[t]) + int(c[t-1] < o[t-1]) + int(c[t-2] < o[t-2])
        vol_ok = True
        w = v[t-20:t+1]
        if np.nanmedian(w) > 0:
            vol_ok = v[t] >= 0.8 * np.nanmedian(w)
        sig[t] = fresh and (downs >= 2) and vol_ok
    sig
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "three_fresh_lower_lows"
