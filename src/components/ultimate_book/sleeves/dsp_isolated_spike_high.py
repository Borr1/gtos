"""dsp_isolated_spike_high -- leftover-3 ALL36, 2026-08-21.

The 20-bar high is made by a single 1-ATR-plus up bar that is not sitting on a
prior thrust. Last-hour AMD (24) spiked to 77.19 and died. This is the trap
twin of small_bar_on_thrust_high. Both must run; the shard's value is that 22
and 24 exist as a pair at the same clock.

MEASURED (never assumed):
  displacement within 3 bars, base rate 1.252%
  V2 split: lift 2.18x disc / 2.18x hold  n_fire 254,890  n_disp 6,964
  V2 up-rate 58.4% labelled LONG (WRONG)
  geometry.py first-touch 1.0/6.0 SHORT  hold R +0.122  (n_hold 264,464)
  (0.75/6.0 is best_both_halves +0.149; ship 1.0/6.0 to stay out of the FX 5-8 pip bleed)
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Proposed by swarm shard 01 (verified_w4.json). ALL36 leftover #3.
What would kill it: Isolated 1-ATR spike highs fade no more often than they
continue; then the 22-vs-24 split I drew is overfit to two last-hour US names.
...
def _signal(o, h, l, c, v, atr):
    ...
    n=len(c); body=np.abs(c-o); out=np.zeros(n,dtype=bool)
    for t in range(20,n):
        hh=np.max(h[t-19:t+1])
        prior_thrust=(body[t-1]>=0.80*atr[t-1]) and (c[t-1]>o[t-1])
        out[t]=(h[t]>=hh-1e-12) and (body[t]>=1.0*atr[t]) and (c[t]>o[t]) and (not prior_thrust)
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "isolated_spike_high"
    spots = [
        {
            "id": "named_surface",
