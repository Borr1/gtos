"""dsp_wide_bar_takes_both_extremes_then_reverse -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

One of the last two bars is wide enough to print both a fresh 20-bar high and a fresh 20-bar
low, or to take a just-made high and dump it in the same bar. A person sees a stop-run both
ways; a sweep-high column and a sweep-low column each fire and disagree. LNK 03:30 made
6.159 and 6.114 in one 2.22 ATR down bar; 03:45 reclaimed to 6.140; 04:00 went through the
high (20). JP225 02:45 high 45329 (the 20-high) and low 45229 (through the 20-low 45225) on
a 1.68 ATR red, then 03:00 +10.46 ATR (40). Event 21 swept both flags via a weekend gap,
then filled UP. Event 6's 3.89 ATR bar made the high only — not both — and is a different
object.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.163 (n=18,705)  R_hold +0.133 (n=17,229)
...
TAG = "dsp_wide_bar_takes_both_extremes_then_reverse"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(21,n):
        for j in (t, t-1):
            rng=h[j]-l[j]
            if rng < 1.4*atr[j]: continue
            hh=np.max(np.r_[h[j-19:j], h[j+1:t+1]]) if j<t else np.max(h[j-19:j])
            ll=np.min(np.r_[l[j-19:j], l[j+1:t+1]]) if j<t else np.min(l[j-19:j])
            if (h[j] >= hh - 0.10*atr[j]) and (l[j] <= ll + 0.15*atr[j]):
                out[t]=True
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "wide_bar_takes_both_extremes_then_reverse"
    spots = [
        {
            "id": "named_surface",
