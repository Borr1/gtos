"""dsp_small_bar_on_thrust_high -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

A large up bar, then a small bar that sits on top of it and makes the 20-bar high. That is
acceptance at the high, not a spike. BA (22) at 20:00: 1.54-ATR thrust, then a 0.21-ATR bar
at 154.85, then 4.14 ATR through 157. JP225 (11) is the cousin (thrust then another up bar
through the high). This is what Event 24 is not.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves SHORT 0.75/6.0
  R_disc +0.103 (n=86,753)  R_hold +0.129 (n=112,973)
...
TAG = "dsp_small_bar_on_thrust_high"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(20,n):
        ...
        out[t]=(body[t-1]>=1.2*atr[t-1]) and (c[t-1]>o[t-1]) and (body[t]<=0.55*atr[t]) and (h[t]>=hh-1e-12)
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "small_bar_on_thrust_high"
