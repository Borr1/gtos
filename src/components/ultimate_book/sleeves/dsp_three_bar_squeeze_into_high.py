"""dsp_three_bar_squeeze_into_high -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

The market had room below the 20-bar high three bars ago. The last three bars squeeze 1.2+
ATR back into that high and the standing bar does not close through it. The squeeze is a
stop-run of shorts, not a new trend. GBPCAD 3 is the clean copy (1.7387→1.7441 into 1.7451,
0.02-ATR doji, then 4.49 then 4.75 ATR down). ADSGn 27 is the delayed copy (morning flush to
176.51, repair into 179.1, 15:30 dump with EUR already −4.26). USDPLN 25 squeezed into 3.907
and went the other way.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves SHORT 0.75/6.0
  R_disc +0.096 (n=417,255)  R_hold +0.119 (n=562,152)
...
TAG = "dsp_three_bar_squeeze_into_high"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(24,n):
        ...
        squeeze=(c[t]-c[t-3])>=1.2*at
        had_room=(hh-c[t-3])>=0.8*at
        at_high=h[t]>=hh-0.15*at
        not_through=((hh-c[t])>=0.12*at) and abs(c[t]-o[t])<1.5*at
        out[t]=squeeze and had_room and at_high and not_through
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "three_bar_squeeze_into_high"
