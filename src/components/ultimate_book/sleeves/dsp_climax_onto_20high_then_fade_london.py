"""dsp_climax_onto_20high_then_fade_london -- UNUSED-GEO-6-10 lead 8 london. NOT ARMED.

Isolated 2-4 ATR up thrust that closes on the 20/96 high, masked to London-local
08:00-09:00 via NEW_YORK_PLUS_7 + EU DST. Separate fire set from cash-hole.
Do not pool. Measured best cell is 1.0/4.0 LONG (not 0.75).

MEASURED (never assumed):
  mask first: London-local 08:00-09:00 (not hour==9)
  n after mask: disc=901 / hold=727 (107 symbols fired)
  best_both_halves LONG 1.0/4.0 hold R +0.210  (n_hold 693)
  V2 up-rate is not the side authority
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Formula verbatim SETUP_LIBRARY_V1.json name climax_onto_20high_then_fade.
...
from .dsp_unused_b_clock import mask_london_first_hour

TAG = "dsp_climax_onto_20high_then_fade_london"
ON_SURFACE: tuple[str, ...] = ("EURUSD", "GBPUSD", "USDJPY", "XAUUSD")

_WARMUP = 200
_STOP_ATR = 1.0
_TARGET_ATR = 4.0
_DIRECTION = 1          # +1 long / -1 short  — geometry 1.0/4.0 measured best
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    session_mask = False
    try:
        session_mask = bool(mask_london_first_hour(symbol, bar_time, bar_times, n))
    except Exception:
        session_mask = False
    facts = {
        "sleeve": TAG,
    ...
    facts["session_mask_name"] = "mask_london_first_hour"
    pattern_side = "climax_onto_20high_then_fade_london"
    spots = [
        {
            "id": "named_surface",
