"""dsp_spring_close_on_20low_through_the_box -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

A compact 20-bar box (range under 5.5 ATR, not a cascade) prints a down bar that makes the
20-low and closes in the bottom 40%. The next expansion is UP through the box, not a second
flush. AUDCHF 4 is the only clean one on this shard: k=-1 makes 0.81063, closes 0.81091,
then 12:30 takes 0.81475 — through the 20-high 0.81411 in one bar. World EURUSD was already
-5.03. This is the lead the close-on-low=down rule would kill. I am not sure it repeats.
That is why it has to be tested, not dropped.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.146 (n=374,966)  R_hold +0.148 (n=476,106)
...
TAG = "dsp_spring_close_on_20low_through_the_box"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(22,n):
        if l[t] > np.min(l[t-19:t]) + 0.05*atr[t]: continue
        if c[t] >= o[t]: continue
        rng=np.maximum(h[t]-l[t], 1e-12)
        if (c[t]-l[t]) > 0.40*rng: continue
        if (np.max(h[t-19:t+1])-np.min(l[t-19:t+1])) >= 5.5*atr[t]: continue
        if np.max(o[t-12:t]-c[t-12:t]) >= 2.0*np.max(atr[t-12:t]): continue
        out[t]=True
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "spring_close_on_20low_through_the_box"
