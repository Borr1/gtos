"""dsp_weekend_gap_then_bleed_into_20low -- unused-window lead 1, 2026-08-21.

A session hole prints a gap down. The next 4-8 bars do not repair it — they bleed
tick-by-tick into a new 20-bar low with small bodies. Session mask is the
Sunday/Monday broker hole after F2 spread-normalize (forbid ±30 min of broker
00:00; not hour-00 entry). Formula verbatim verified_w0.json.
V2 said LONG (agreed). Side is best_both_halves, not V2.
...
MEASURED (never assumed):
  ...
  best_both_halves LONG 0.75/6.0  hold R +0.451  (n_hold 8,939)
  ship 1.0/6.0 same side  hold R +0.389  (n_hold 8,939)  clears +0.12
  day-blocked hold SE 0.067 / MDE80 0.142 / blocks 390
...
from ._unused_a_session_mask import mask_weekend_gap

TAG = "dsp_weekend_gap_then_bleed_into_20low"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(22,n):
        ...
        for j in range(t, t-8, -1):
            if o[j] <= c[j-1] - 0.40*atr[j]:
                g=j; break
        if g is None: continue
        if c[t]>=o[g]: continue
        if l[t] > np.min(l[t-19:t+1]) + 0.25*atr[t]: continue
        if np.mean(np.abs(c[t-4:t+1]-o[t-4:t+1]) / np.maximum(atr[t-4:t+1], 1e-12)) >= 0.70: continue
        out[t]=True
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    session_mask = False
    try:
        session_mask = bool(mask_weekend_gap(symbol=symbol, bar_time=bar_time, bar_times=bar_times))
    except Exception:
        session_mask = False
    facts = {
        "sleeve": TAG,
    ...
    facts["session_mask_name"] = "mask_weekend_gap"
    pattern_side = "weekend_gap_then_bleed_into_20low"
