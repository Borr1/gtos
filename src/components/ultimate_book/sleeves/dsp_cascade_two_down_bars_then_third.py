"""dsp_cascade_two_down_bars_then_third -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

Two consecutive (or one-apart) down bars of 1.5+ ATR each have already started the move. The
standing bar may be the second of those, body still under 4 ATR. The named displacement is
wave three of a cascade that a person can already see. AMD 3: k=-1 is already 2.70 ATR to
93.41, then -4.01. USDNOK 7: 1.90 then 1.79 into 8.5689, then -5.42. EURCZK 22: k=-1 is
already 2.66 ATR reversing the morning probe, then -12.03. INTC 39: 15:30 is already 5.40
ATR (takes 57.82 and closes on 56.92), 15:45 is wave two. This is the known 'last bar was
huge' with a sign and a count: two down bars, not one isolated spike that reverses (EURPLN
2, CORN 27).

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves LONG 0.75/6.0
  R_disc +0.160 (n=22,719)  R_hold +0.149 (n=29,240)
...
TAG = "dsp_cascade_two_down_bars_then_third"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(8,n):
        at=max(atr[t],1e-12)
        downs=[j for j in range(t-2,t+1) if j>=0 and (o[j]-c[j])>=1.5*atr[j] and (o[j]-c[j])<4.0*atr[j]]
        if len(downs)<2: continue
        if c[t]>=min(o[j] for j in downs): continue
        if t>=8 and (c[t]-c[t-8])>-3.0*at: continue
        out[t]=True
    out
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "cascade_two_down_bars_then_third"
