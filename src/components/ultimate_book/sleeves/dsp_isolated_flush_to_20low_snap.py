"""dsp_isolated_flush_to_20low_snap -- weekend leftover, 2026-08-22.

A single 1.8+ ATR down bar in the last four bars tags or takes the 20-bar low,
and there was not already a 2 ATR down bar in the eight bars before that flush.
Stops under the low have fired; the absence of a prior cascade says this is a
liquidation, not a trend. The next 1-3 bars snap the other way.

T3 singleton `dsp_c32`. Unmasked. Do not share live c19 / c02 / c15 / c10.

MEASURED (never assumed):
  V2 up-rate 38.8% labelled SHORT (WRONG)
  geometry.py best_both_halves LONG 0.75/6.0 hold R +0.188  (n_hold 127,961)
  ship 1.0/6.0 same side hold R +0.147  (n_hold 127,961) both_positive
  scored over 163 instruments; formula evaluated bar-by-bar, causal, index <= i.

Formula verbatim SETUP_LIBRARY_V2.json / verified_w5.json shard 41.
...
TAG = "dsp_isolated_flush_to_20low_snap"
...
def _signal(o, h, l, c, v, atr):
    ...
    for t in range(20,n):
        at=max(atr[t],1e-12)
        lo20=np.min(l[max(0,t-20):t])
        flush_k=None
        for k in range(max(1,t-3),t+1):
            if (o[k]-c[k])>=1.8*max(atr[k],1e-12) and l[k]<=lo20+0.15*at:
                flush_k=k
        if flush_k is None: continue
        prior=any((o[k]-c[k])>=2.0*max(atr[k],1e-12) for k in range(max(1,flush_k-8),flush_k))
        rng=h[t]-l[t]
        low_close=(rng>0 and (c[t]-l[t])<=0.35*rng) or (t==flush_k)
        out[t]=(not prior) and low_close and abs(c[t]-o[t])<3.5*at
    sig = out
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "isolated_flush_to_20low_snap"
    spots = [
        {
            "id": "named_surface",
    ...
    try:
        ...
        return None
    try:
        _ex = float(np.min(l[max(0, i-20):i]))
