"""dsp_high_vol_doji_after_reclaimed_flush_fx -- UNUSED-GEO-6-10 lead 10 fx. NOT ARMED.

High-volume doji after a reclaimed 20-low sweep, masked to broker 15:30/15:45
and fiat FX only (both 3-letter legs in FIAT; crypto 6-letter USD excluded).
Not pooled with energy/agri (no both-halves cell — not sleeved). Ship 1.0/6.0
SHORT (0.75/6.0 won; 1.0/6.0 hold R +0.166 clears the ship gate).

MEASURED (never assumed):
  mask first: broker 15:30-15:45; fiat FX only
  n after mask: disc=1617 / hold=1015 (41 symbols fired)
  best_both_halves SHORT 0.75/6.0 hold R +0.219  (n_hold 1015)
  ship 1.0/6.0 same side hold R +0.166  (n_hold 1015) both_positive
  V2 up-rate is not the side authority
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Formula verbatim verified_w0.json name high_vol_doji_after_reclaimed_flush.
Status forward_only: split-sample backtest, NO forward record.
...
from .dsp_unused_b_clock import mask_cash_hour_fx

TAG = "dsp_high_vol_doji_after_reclaimed_flush_fx"
ON_SURFACE: tuple[str, ...] = ("EURUSD", "GBPUSD", "USDJPY", "XAUUSD")

_WARMUP = 200
_STOP_ATR = 1.0
_TARGET_ATR = 6.0
_DIRECTION = -1         # +1 long / -1 short  — geometry 1.0/6.0 SHORT, NOT V2
...
def _signal(o, h, l, c, v, atr):
    """The discovered condition. Verbatim from verified_w0.json."""
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    session_mask = False
    try:
        session_mask = bool(mask_cash_hour_fx(symbol, bar_time, bar_times, n))
    except Exception:
        session_mask = False
    facts = {
        "sleeve": TAG,
    ...
    facts["session_mask_name"] = "mask_cash_hour_fx"
    pattern_side = "high_vol_doji_after_reclaimed_flush_fx"
    spots = [
        {
            "id": "named_surface",
    ...
    try:
        ...
        return None
    _ex = None
    for _j in range(i, i-8, -1):
        _prior = np.min(l[max(_j-19, 0):_j]) if _j > 0 else l[_j]
        if l[_j] <= _prior + 0.05 * atr[i]:
            try:
                _ex = float(_prior)
