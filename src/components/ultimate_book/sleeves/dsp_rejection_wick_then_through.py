"""dsp_rejection_wick_then_through -- J2b from ALL36-GEOMETRY 2026-08-21. Not on live tags.

Bar t tags a 20-bar high, leaves an upper wick >= 40% of its range, and closes in the
middle. It looks like a failed break. The next bar runs the high. The fade that a person
would sell is the last liquidity before the algorithms go.

MEASURED (never assumed, never up-rate):
  geometry.py best_both_halves SHORT 0.75/6.0
  R_disc +0.059 (n=224,997)  R_hold +0.118 (n=288,188)
...
TAG = "dsp_rejection_wick_then_through"
...
def _signal(o, h, l, c, v, atr):
    ...
    atr_s = np.maximum(atr, 1e-12); rng = np.maximum(h - l, 1e-12); n = len(c); sig = np.zeros(n, dtype=bool)
    for t in range(20, n):
        tagged = h[t] >= np.max(h[t-19:t])
        wick = (h[t] - max(c[t], o[t])) / rng[t] >= 0.40
        mid = 0.30 <= (c[t] - l[t]) / rng[t] <= 0.70
        sig[t] = tagged and wick and mid and (abs(c[t]-o[t]) < 4.0 * atr_s[t])
    sig
    return sig
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    pattern_side = "rejection_wick_then_through"
