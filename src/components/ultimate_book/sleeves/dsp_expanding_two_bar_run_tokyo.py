"""dsp_expanding_two_bar_run_tokyo -- UNUSED-GEO-6-10 lead 9 tokyo_cash. NOT ARMED.

Two consecutive same-direction 1.2+ ATR bodies, second does not give back the
first midpoint, masked to Tokyo cash 09:00-15:00 JST. Ship 1.0/6.0 LONG
(0.75/6.0 won; 1.0/6.0 hold R +0.131 clears the ship gate).

MEASURED (never assumed):
  mask first: Tokyo cash 09:00-15:00 JST (broker 02:00 EST / 03:00 EDT open)
  n after mask: disc=12127 / hold=10749 (98 symbols fired)
  best_both_halves LONG 0.75/6.0 hold R +0.145  (n_hold 10470)
  ship 1.0/6.0 same side hold R +0.131  (n_hold 10470) both_positive
  V2 up-rate is not the side authority
  scored over 166 instruments; formula evaluated bar-by-bar, causal, index <= i.

Formula verbatim SETUP_LIBRARY_V1.json name expanding_two_bar_run.
...
from .dsp_unused_b_clock import mask_tokyo_cash

TAG = "dsp_expanding_two_bar_run_tokyo"
...
def _signal(o, h, l, c, v, atr):
    ...
    n=len(c); body=np.abs(c-o); signed=c-o; mid=(h+l)/2.0; out=np.zeros(n,dtype=bool)
    out[1:]=(body[1:]>=1.2*atr[1:])&(body[:-1]>=1.2*atr[:-1])&(np.sign(signed[1:])==np.sign(signed[:-1]))&(np.sign(signed[1:])!=0)&(body[1:]<4.0*atr[1:])&(((signed[1:]>0)&(l[1:]>=mid[:-1]))|((signed[1:]<0)&(h[1:]<=mid[:-1])))
    return out
...
def generate(symbol: str, bars, decision_day: str, *, bar_time=None, bar_times=None,
    ...
    session_mask = False
    try:
        session_mask = bool(mask_tokyo_cash(symbol, bar_time, bar_times, n))
    except Exception:
        session_mask = False
    facts = {
        "sleeve": TAG,
    ...
    facts["session_mask_name"] = "mask_tokyo_cash"
    pattern_side = "expanding_two_bar_run_tokyo"
