...
def _band(namespace: str, scope: tuple[str, ...]) -> dict[str, float | None]:
    return _scores(
        ("lane_band", str(namespace), scope),
        {"namespace": str(namespace), "scope": list(scope)},
        {
            "declared_min_weight": (
                "The score you return is the lowest sleeve weight this state still accepts. "
                "An empty score leaves the band unset. Do not send."
            ),
            "declared_max_weight": (
                "The score you return is the highest sleeve weight this state still accepts. "
                "An empty score leaves the band unset. Do not send."
            ),
        },
    )
...
def verify_envelope(
    ...
    *,
    ...
    scope_names = tuple(sorted(str(v) for v in declared_scope)) if isinstance(declared_scope, list) else ()
...
def build_unsigned_payload(
    *,
    ...
    return {
        ...
        "declared_band": _band(str(namespace), scope),
...
def neutral_snapshot(
    ...
    *,
    ...
    return {
        ...
        "scope_sleeves": list(scope),
        "declared_band": {"min": None, "max": None},
    ...
    def snapshot_for_day(
        ...
        *,
        ...
        try:
            ...
            grace = _scores(
                ("lane_grace", str(self.namespace), day),
                {
                ...
                },
                {
                    "activation_grace_seconds": (
                        "The score you return is how many seconds after the decision-day "
