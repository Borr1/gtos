...
from .live_flow import build_live_flow_block, validate_live_flow_block
...
GENERATION_SLOT_TERMINAL_STATUSES = {
    "profile_unsupported",
    "bars_unavailable",
    "insufficient_warmup",
    "future_decision_bar",
...
def build_runtime_learning_packet(
    *,
    ...
    convergence_advisory: Mapping[str, Any] | None = None,
    ...
    convergence_advisory_clean = (
        _clean_mapping(dict(convergence_advisory or {})) if convergence_advisory else None
    )
    ...
    packet = {
        ...
        "ultimate_convergence_advisory": convergence_advisory_clean,
    ...
    # One ledger, not another logger.  The runtime-learning writer is already the durable,
    # cross-process append surface shared by production and F5.  ``live_flow`` is an additive
    # projection with a bar-grained join id, sparse evidence-bound gate observations, compact
    # broker request/result, and realized economics. Historical packets remain valid without it.
    packet["live_flow"] = build_live_flow_block(
        namespace=namespace,
        event_type=event_type,
...
def validate_runtime_learning_packet(packet: Mapping[str, Any]) -> tuple[bool, list[str]]:
    ...
    # Additive and forward-compatible: legacy packets have no live-flow block; any packet that
    # claims the new contract must satisfy it completely.
    if "live_flow" in packet:
        issues.extend(validate_live_flow_block(packet.get("live_flow")))
    if packet.get("event_type") not in EVENT_TYPES:
        issues.append(f"unknown_event_type:{packet.get('event_type')}")
    if packet.get("event_type") == "generation_cycle_complete":
        outcome = packet.get("outcome") if isinstance(packet.get("outcome"), Mapping) else {}
        terminals = outcome.get("generation_slot_terminals")
        if not isinstance(terminals, list):
            issues.append("generation_slot_terminals_not_list")
            terminals = []
        bridge = packet.get("bridge") if isinstance(packet.get("bridge"), Mapping) else {}
        generation = (
        ...
        if not isinstance(expected, int) or isinstance(expected, bool) or expected < 0:
            issues.append("generation_active_symbol_slot_count_invalid")
        elif expected != len(terminals):
            issues.append("generation_slot_count_not_conserved")
        slot_keys: list[tuple[str, str, str]] = []
        for index, terminal in enumerate(terminals):
            if not isinstance(terminal, Mapping):
                issues.append(f"generation_terminal_not_mapping:{index}")
                continue
            for key in (
                "sleeve",
                "symbol",
                ...
                if terminal.get(key) in (None, ""):
                    issues.append(f"generation_terminal_missing:{index}:{key}")
            status = str(terminal.get("terminal_status") or "")
            if status not in GENERATION_SLOT_TERMINAL_STATUSES:
                issues.append(f"generation_terminal_unknown_status:{index}:{status}")
            slot_keys.append((
                str(terminal.get("sleeve") or ""),
                str(terminal.get("symbol") or ""),
                str(terminal.get("timeframe") or ""),
            ))
            if status == "candidate_emitted":
                for key in ("direction", "decision_bar_iso"):
                    if terminal.get(key) in (None, ""):
                        issues.append(f"generation_emitted_terminal_missing:{index}:{key}")
        if len(slot_keys) != len(set(slot_keys)):
            issues.append("generation_terminal_slot_keys_not_unique")
