...
def _build_precision_examples(price_fmt: str, symbol: str) -> tuple[str, str, str]:
    ...
    invalid_block = (
        ...
        # ADR-006: 0.25/3 hardcoded -> {ob_atr_mult}/{min_ticks} substituted post.
        f"(must be strictly > 0; pick the max of "
        "{ob_atr_mult}*H1 ATR or {min_ticks} ticks)\n"
...
def build_system_prompt(config: dict | None = None) -> str:
    ...
    Framework gating
    ----------------
    The optional ``breaker_re_entry`` section is appended only when
    ``model_a.enabled_frameworks`` contains either ``breaker_re_entry`` (the
    explicit additive name, 2026-04-25) or the legacy ``breaker_retest``.
    Without that flag the placeholder collapses to an empty string, leaving
    V3 byte-for-byte unchanged for the OB-retest-only configuration.
    """
    if config is None:
        config = {}
    ...
    # ``breaker_re_entry`` framework section is additive (2026-04-25): only
    # injected when the explicit-name flag (or its legacy ``breaker_retest``
    # alias) appears in ``model_a.enabled_frameworks``. The default
    # OB-retest-only config substitutes an empty string \u2014 V3 prompt remains
    # byte-identical to its 2026-04-24 baseline for that path.
    enabled_fw = config.get("model_a", {}).get("enabled_frameworks", []) or []
    breaker_section = (
        BREAKER_RE_ENTRY_SECTION
        if ("breaker_re_entry" in enabled_fw or "breaker_retest" in enabled_fw)
        else ""
    )
    ...
    text = text.replace("{breaker_re_entry_section}", breaker_section)

    # ADR-006 (2026-04-26): config-driven SL buffer multipliers replace
    # hardcoded constants. Per-instrument tight-FX overrides flow through
    # apply_profile_overrides → apply_instrument_overrides, so EURUSD and
    # GBPUSD see ob_atr_mult=0.50 / min_ticks=8 instead of 0.25 / 5.
...
ALLOW-LIST (the ONLY legal NO_TRADE reasons you may emit):
  ...
  (R5) `self_check_failed`         — one of the seven SELF-CHECK items (see
...
ONLY to evaluate C1/C2/C3 (and apply the seven SELF-CHECK items to your
...
## FRAMEWORKS (UP TO THREE STANDALONE PATHS)
You evaluate independent trading frameworks and emit ONE decision per candle.
The frameworks active in this build are determined by the
``model_a.enabled_frameworks`` config and surface in this prompt as follows:

  (FW1) `ob_retest`        — H1 OB retest with M15 displacement confirmation.
                             Default path, ALWAYS active. Geometry, gates, and
                             POI rules below remain UNCHANGED from V3.
  (FW2) `fvg_fill`         — M15 Fair Value Gap retracement-fill with
                             H1-aligned bias. Activated 2026-04-25 (CLAUDE.md
                             §Validated Numbers: FVG-in-impulse +7-20pp WR
                             across 6 instruments, Bonferroni-surviving). The
                             dedicated ``## FVG_FILL FRAMEWORK`` section below
                             specifies its setup definition, confluence
                             requirements, SL placement, and POI/entry
                             constraints.
  (FW3) `breaker_re_entry` — H1 breaker (mitigated/flipped OB) re-entry. When
                             enabled, the ``## FRAMEWORK ADDENDUM — BREAKER
                             RE-ENTRY`` block injected after this section
                             specifies setup definition, qualifying conditions
                             (B1-B4), and SL geometry ({breaker_atr_mult}×H1 ATR buffer vs
                             {ob_atr_mult}× for ob_retest). Only emit
                             ``framework: "breaker_re_entry"`` when that
                             addendum is present below.
...
self_check_failed" when your numbers actually pass all seven self-checks.
...
## FVG_FILL FRAMEWORK (standalone, additive — activated 2026-04-25)

FVG_FILL is an INDEPENDENT entry pattern from `ob_retest`. It targets the
retracement-fill of a freshly created M15 Fair Value Gap that was generated
by an aligned displacement candle, and does NOT require an opposing-OB
retest. Use it ONLY when `ob_retest` cannot fire (Step C above).

### Setup definition (MUST hold for fvg_fill to qualify)

  (F1) Displacement candle on M15 — within the last 12 M15 candles, an M15
       BOS or CHoCH event with `displacement_present=True` exists in the
       direction matching C3 (bullish for LONG, bearish for SHORT). The
       MSO `## M15 — ...` section lists structure events with their
       `disp` and `ratio` fields; treat `displacement_present=True` AND
       `displacement_ratio >= 1.5` as the qualifying displacement.

  (F2) Unfilled FVG on M15 — the displacement candle (or one of its two
       neighbors at the indices reported in `candle_indices`) created an
       M15 FVG that is STILL UNFILLED at the candle being evaluated. The
       MSO `## M15 — ...` section lists `Unfilled FVGs (N):` with each
       FVG's `top` and `bottom`. For LONG, only `bullish` FVGs qualify;
       for SHORT, only `bearish` FVGs qualify. If `Unfilled FVGs` is
       absent or contains zero direction-matching entries, fvg_fill does
       NOT qualify — return to Step D (NO_TRADE / R4).

  (F3) H1-direction confluence — the qualifying displacement direction
       MUST match the H1 structure direction from C1. This is the
       framework's "structural alignment" gate. fvg_fill counter-trend
       to H1 is forbidden.

  (F4) Fill confirmation — the M15 candle being evaluated has retraced
       INTO the FVG range (its current close is within `[bottom, top]`
       of the qualifying unfilled FVG). The classic fill-trigger pattern
       is a candle whose body re-enters the FVG with a rejection wick;
       at this gate-layer, "current candle close inside the FVG range"
       is the operational test. The MSO is grounded in OHLCV; you must
       verify the close is inside the gap from the M15 candle list.

If ANY of F1-F4 fails, fvg_fill does NOT qualify. Do NOT relax these
conditions — fall through to Step D.

### POI citation (when fvg_fill fires)

  - `h1_setup.poi_identified`     = true
  - `h1_setup.poi_type`            = `"FVG"`
  - `h1_setup.poi_price_level`     = midpoint of the qualifying M15 FVG
                                     (i.e., (top + bottom) / 2)
  - `h1_setup.zone`                = `"discount"` for LONG, `"premium"`
                                     for SHORT (FVG-fill always retraces
                                     into the same-side discount/premium
                                     of the impulse leg)
  - `h1_setup.causing_event_type`  = the type of the qualifying M15
                                     displacement event ("BOS" or "CHoCH")

  Note: this is the ONLY case in this prompt where the cited
  `h1_setup.poi_price_level` is sourced from M15 (the FVG's midpoint).
  The STRICT RULE — H1 POI SOURCE block above scopes to `ob_retest`. For
  fvg_fill, the L2 `entry_in_fvg` check (downstream) validates the
  M15-FVG citation directly against the MSO's `M15 — ... Unfilled FVGs`
  array; the L2 `h1_poi_exists` check is bypassed for fvg_fill by the
  framework switch.

### Trade parameters (when fvg_fill fires)

  - direction         : LONG (bullish FVG) or SHORT (bearish FVG); must
                        match C3.
  - entry_price       : midpoint of the qualifying M15 FVG (same value
                        as `h1_setup.poi_price_level`). MSO-precision.
  - stop_loss         : place BEYOND the FAR EXTREME of the qualifying
                        M15 displacement candle (the candle whose body
                        created the FVG), plus a non-zero buffer. For
                        LONG, the FAR EXTREME is the displacement
                        candle's `low`; SL = displacement_low - buffer.
                        For SHORT, the FAR EXTREME is the displacement
                        candle's `high`; SL = displacement_high + buffer.
                        Minimum {sl_min_display} total distance from entry.
                        The buffer is computed identically to ob_retest:
                        `max({ob_atr_mult} x H1 ATR(14), {min_ticks} x smallest tick)`.
  ...
  - sl_buffer_applied : the EXACT buffer value you used (>0, MSO precision).

ALL geometric invariants from the PRECISION block below apply unchanged
(strict ordering, non-degeneracy, non-zero buffer, correct decimal places).
The downstream L2 `entry_in_fvg` check validates that:
  (a) the cited FVG range (top, bottom) matches an actual unfilled FVG
      in the MSO's `## M15 — ...` section,
  (b) `entry_price` is within `[bottom, top]` of that FVG,
  (c) `stop_loss` is geometrically beyond the displacement candle that
      created the FVG (below for LONG, above for SHORT).

If any of (a)-(c) fails, the trade is rejected at L2 — do not attempt
to skirt them by citing a near-miss FVG.

## TRADE PARAMETERS (if CANDIDATE)
For `framework: "ob_retest"`, compute trade parameters from the MSO price data:
...
- risk_reward_ratio: 1.5
- position_size_lots: 0.01
- sl_buffer_applied: compute a non-zero buffer using max({ob_atr_mult} x H1 ATR(14) from MSO, {min_ticks} x the smallest price tick shown). For LONG the buffer sits BELOW the protected swing low; for SHORT it sits ABOVE the protected swing high. The stop_loss is the swing ± buffer. Report the exact buffer value you applied in this field (use the same decimal precision as prices in the MSO).
- take_profit_2, take_profit_3: 0.0
For `framework: "fvg_fill"`, follow the parameter rules in the FVG_FILL
FRAMEWORK section above (entry = FVG midpoint; SL = displacement-extreme ±
buffer; same RR=1.5; same precision invariants).
...
- h1_setup: Report the POI you cite for the chosen framework. For `ob_retest`,
  this is the nearest unmitigated H1 OB (poi_type="OB", poi_price_level =
  midpoint of OB zone, causing_event_type from the BOS/CHoCH that created it).
  For `fvg_fill`, this is the qualifying unfilled M15 FVG (poi_type="FVG",
  poi_price_level = midpoint of FVG, causing_event_type from the M15
  displacement event that created the FVG). zone = premium/discount based on
  impulse structure. If no qualifying POI exists for either framework,
  poi_identified=false, poi_type="none", poi_price_level=0.
...
```json
{
  ...
  "framework": "ob_retest | fvg_fill | none",
  "kill_zone": "london | ny",
  "frameworks_evaluated": {
    "ob_retest": {"qualified": <bool>, "reason": "<1 sentence>"},
    "fvg_fill":  {"qualified": <bool>, "reason": "<1 sentence>"}
  },
  "reasoning": {
    "daily_bias": {
...
6. POI source — framework-dependent:
   - For `framework: "ob_retest"`: h1_setup.poi_price_level falls INSIDE
     the bounds of a zone listed in the MSO's `## H1 — Structure: ...`
     section. It is NOT a price taken from `## M15 — ...`, `## D1 — ...`,
     or `## H4 — ...`, and NOT a fabricated round-number level.
   - For `framework: "fvg_fill"`: h1_setup.poi_price_level equals the
     midpoint of an unfilled M15 FVG listed under the MSO's `## M15 —
     ...` section, in the direction matching C3, AND that FVG was
     created by an aligned displacement candle within the recent 12
     M15 candles (per the FVG_FILL FRAMEWORK section). Citing an FVG
     not in the MSO's M15 list is a schema violation.
7. Framework consistency — the value of `framework` you emit MUST match
   the framework named verbatim in `reasoning.overall_reasoning`. If
   your overall_reasoning describes the setup as `fvg_fill` (or
   `breaker_re_entry`), the top-level `framework` field MUST be that
   same string — do NOT default to `ob_retest` while the reasoning
   describes a different framework. Additionally, when
   `decision == "CANDIDATE"`, AT LEAST ONE entry in
   `frameworks_evaluated` MUST have `qualified == true`. Emitting
   CANDIDATE while every framework's `qualified` flag is `false` is a
   schema violation — the correct response in that case is NO_TRADE
   with `no_trade_reason: "no_qualifying_h1_poi"` (R4).

If any of 1–7 fails, emit NO_TRADE with `no_trade_reason` exactly equal
to one of:
  - `wrong_side_sl`            (use this when items 1 or 2 fail)
  - `degenerate_trade_parameters` (use this when item 4 fails)
  - `self_check_failed`        (use this when item 3, 5, 6, or 7 fails;
...
# \u2500\u2500 Breaker re-entry framework section (additive 2026-04-25) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
#
# Injected into the system prompt only when ``model_a.enabled_frameworks``
# contains ``breaker_re_entry`` (or its legacy ``breaker_retest`` alias). The
# section ADDS a second valid framework alongside ob_retest \u2014 it does not
# override or weaken any C-gate, geometric invariant, self-check, or
# enumerated NO_TRADE allow-list rule from the base V3 prompt.
#
# Concept (from research/vision_program_2026-04-25/03_EDGE_TAXONOMY.md, E5
# breaker re-entry, "cheapest top-5 deploy"): a previously mitigated OB has
# its polarity flipped \u2014 a mitigated bullish OB now acts as supply (resistance)
# and a mitigated bearish OB now acts as demand (support). The structurally
# distinct trade is to re-enter on the OPPOSITE side of the original OB
# direction, anchored on the ``breaker_blocks`` MSO field (NOT
# ``order_blocks``). Detection is already performed by
# ``identify_breaker_blocks`` in ``src/components/market_state.py``; the V3
# prompt mentioned breakers as a fallback POI source under the OB-retest
# framework, but never assigned them their own framework label or risk
# geometry.
#
# Why pure addition: Wave-1 edge taxonomy concluded the AI may already have
# been finding mitigated OBs informally and labelling them ``ob_retest``
# (this would explain part of the A6 "AI may be hurting" finding). Activating
# the explicit ``breaker_re_entry`` label with deterministic L2 verification
# (``entry_in_breaker``) lets the system measure per-framework WR and SHRINK
# fabricated CANDIDATEs that pass ob_retest's L2 but ground in a flipped
# zone, without removing or weakening the existing OB-retest path.
BREAKER_RE_ENTRY_SECTION = """

## FRAMEWORK ADDENDUM \u2014 BREAKER RE-ENTRY (additive 2026-04-25)

In addition to the OB Retest framework above, you may classify a setup as
**breaker_re_entry** when the H1 POI is a *flipped* (mitigated) order
block \u2014 a "breaker block" \u2014 instead of an unmitigated OB. The C-gate
decision rule (C1 + C2 + C3) is IDENTICAL to OB-retest; only the POI
source and SL anchor differ. Touch count, distance, displacement quality,
and zone width are STILL not gate criteria; G1-G5 forbidden patterns from
"NO FOURTH GATE" still apply unchanged.

### What is a breaker block (B-DEFINITION)

A breaker block is the residual zone of an order block that has FAILED its
purpose: price punched through the OB (the OB is now ``mitigated`` /
``retested`` in the original-direction sense), and structure has confirmed
the new direction. The zone of the *original* OB now acts as a continuation
level on the OPPOSITE side from the OB's original polarity:

  - A previously **bullish OB** (formerly support / demand) that was broken
    DOWNWARD becomes a **bearish breaker** \u2014 the same price band now acts
    as supply / resistance. A re-entry here is a SHORT setup.
  - A previously **bearish OB** (formerly resistance / supply) that was
    broken UPWARD becomes a **bullish breaker** \u2014 the same price band now
    acts as demand / support. A re-entry here is a LONG setup.

The MSO renders these in each H1/M15 timeframe under
``Unretested Breaker Blocks (N):`` with the line shape:

    {direction} breaker {zone_high}-{zone_low} (orig={original_ob_direction} OB,
    formed={formation_time}, mitigated={mitigation_time})

The ``direction`` field on a breaker is the FLIPPED polarity (post-mitigation
trade side), not the original OB direction. Use ``direction`` as the
required-side check, NOT ``original_ob_direction``.

### When breaker_re_entry is the right framework label (B-WHEN)

Apply ``"framework": "breaker_re_entry"`` when ALL of these hold:

  (B1) The H1 timeframe in the MSO contains an UNRETESTED breaker block
       in the same direction as your C3-determined trade direction
       (bullish breaker \u2192 LONG, bearish breaker \u2192 SHORT).
  (B2) You are NOT also citing an unmitigated H1 OB at the same price
       level. If both an unmitigated OB and an unretested breaker exist in
       the same direction at overlapping zones, prefer ``ob_retest`` and
       cite the OB \u2014 that is the cleaner instance of the same edge.
  (B3) The breaker's flipped polarity (its ``direction`` field) AGREES with
       H1 structure direction from C1. A bullish breaker requires H1
       structure ``bullish``; a bearish breaker requires H1 structure
       ``bearish``. Mismatched direction = C3 FAIL = NO_TRADE
       (``c3_direction_mismatch``), not breaker_re_entry.
  (B4) Standard C1 + C2 + C3 all PASS per the base prompt rules.

If (B1) does not hold (no qualifying H1 breaker for your direction) but an
unmitigated H1 OB does exist, use ``ob_retest`` as before. If neither an
OB nor a breaker exists in the C3 direction, NO_TRADE with
``no_qualifying_h1_poi`` per R4 \u2014 that allow-list rule is unchanged.

### Trade parameters for breaker_re_entry (B-PARAMS)

When ``framework == "breaker_re_entry"``:

  - ``entry_price``: the side of the breaker zone facing the entry path.
      LONG (bullish breaker, demand): ``zone_high`` of the breaker.
      SHORT (bearish breaker, supply): ``zone_low`` of the breaker.
    (Same convention as OB-retest's "ob_high for LONG, ob_low for SHORT",
    just sourced from the breaker zone instead of an OB zone.)
  - ``stop_loss``: BEYOND the FAR side of the breaker, plus a non-zero
    buffer of ``max({breaker_atr_mult} * H1 ATR(14), {min_ticks} * smallest tick)`` \u2014 note the
    breaker buffer is {breaker_atr_mult} ATR, NOT the OB-retest {ob_atr_mult} ATR. Breakers are
    frequently retested with deeper wicks than fresh OBs (the cascade has
    already exhausted some liquidity), so a wider structural buffer is
    required. The total SL distance must still meet the per-instrument
    ``{sl_min_display}`` minimum from the base prompt.
      LONG: stop_loss = breaker.zone_low \u2212 buffer  (strictly below the
            breaker's far extreme).
      SHORT: stop_loss = breaker.zone_high + buffer (strictly above the
            breaker's far extreme).
  - ``take_profit_1``: same 1.5R rule as OB-retest. LONG:
    ``entry + 1.5 * |entry - stop_loss|``. SHORT:
    ``entry - 1.5 * |stop_loss - entry|``.
  - ``risk_reward_ratio``: 1.5 (unchanged).
  - ``position_size_lots``: 0.01 (unchanged).
  - ``sl_buffer_applied``: report the exact buffer value (the
    ``max({breaker_atr_mult} * H1 ATR, {min_ticks} * tick)`` you actually applied), strictly > 0,
    using MSO decimal precision.
  - ``take_profit_2`` and ``take_profit_3``: 0.0 (unchanged).

The geometric invariants from the PRECISION block (LONG: SL < entry < TP1;
SHORT: TP1 < entry < SL; entry != SL; entry != TP1; sl_buffer_applied > 0)
all apply UNCHANGED.

### POI emission for breaker_re_entry (B-POI)

When ``framework == "breaker_re_entry"``:
  - ``framework`` field: emit ``"breaker_re_entry"``.
  - ``h1_setup.poi_identified``: ``true``.
  - ``h1_setup.poi_type``: ``"breaker_block"``.
  - ``h1_setup.poi_price_level``: midpoint of the breaker zone
    (``(zone_high + zone_low) / 2``).
  - ``h1_setup.causing_event_type``: the breaker's ``causing_event``
    field from the MSO (typically ``BOS`` or ``CHoCH``).
  - All other ``reasoning`` fields follow the OB-retest reporting rules
    unchanged.

### L2 verification (downstream \u2014 informational)

A deterministic gate ``entry_in_breaker`` (``src/components/verification.py``)
verifies that, when ``framework == "breaker_re_entry"``:
  (i) the cited POI midpoint resolves to an unretested H1 breaker in the
      MSO whose ``direction`` matches your trade side;
  (ii) the entry_price falls inside (or within tolerance of) that breaker
       zone;
  (iii) the stop_loss sits BEYOND the breaker's far extreme on the correct
        side for direction.
A breaker_re_entry CANDIDATE that fails ``entry_in_breaker`` will be L2
rejected the same way an ob_retest CANDIDATE that fails ``entry_in_ob`` is
rejected \u2014 there is no looser standard.

### What this section does NOT change

The base V3 prompt's three structural gates (C1/C2/C3), the enumerated
NO_TRADE allow-list (R1-R8), the forbidden-pattern blocks (G1-G5), the
NO FOURTH GATE rule, the H1 POI source rule (PROMPT V2), the PRECISION
block + SELF-CHECK items (1-7), and the OB-retest trade-parameters
specification all apply UNCHANGED. ``breaker_re_entry`` is a parallel POI
source path with its own SL anchor; it is NOT a fourth gate or a
...
# The ``breaker_re_entry`` placeholder is substituted with the empty string
# here \u2014 the default ``SYSTEM_PROMPT`` constant remains byte-identical to its
# pre-2026-04-25 V3 baseline so existing tests and historical fixtures
# continue to compare exactly. Live builds via ``build_system_prompt(config)``
# inject the breaker section when the flag is enabled.
...
SYSTEM_PROMPT = (
    ...
    .replace("{breaker_re_entry_section}", "")
)
...
def _format_tf(tf_name: str, tf_data: dict) -> str:
    ...
    all_obs = tf_data.get("order_blocks", []) or []
    obs = [ob for ob in all_obs if not ob.get("mitigated", False)]
    if obs:
    ...
    # Mitigated OBs — render as a flagged subsection so the AI can see
    # them and explicitly route to fvg_fill / breaker_re_entry when every
    # same-direction OB is already retested. Without this, a trending
    # market with all-mitigated OBs presents an empty "Unmitigated OBs"
    # section, and the AI is forced to *infer* OB exhaustion from
    # absence — leaving the dispatch dependent on the user-prompt's
    # last-line phrasing. See research/halluc1_gbpjpy_deep_dive_2026-04-28
    # for the GBPJPY 2026-04-28 01:30 smoking gun.
    mitigated_obs = [ob for ob in all_obs if ob.get("mitigated", False)]
    if mitigated_obs:
        parts.append(
            f"  Mitigated OBs ({len(mitigated_obs)}, "
            f"already retested — not eligible for ob_retest framework):"
        )
        for ob in mitigated_obs[-5:]:
            parts.append(f"    {ob.get('type','?')} {ob.get('high',0):{_PRICE_FMT}}-{ob.get('low',0):{_PRICE_FMT}} "
                       f"body={ob.get('open',0):{_PRICE_FMT}}-{ob.get('close',0):{_PRICE_FMT}} "
                       f"evt={ob.get('causing_event_type','?')} touches={ob.get('touch_count',0)} "
                       f"({ob.get('formation_time','?')[:16]})")
...
def format_cross_instrument_context(
    ...
    Correlation gate (deferred-master B.1, ADR-006 follow-up): when
    an explicit ``candidate_symbol`` or ``config.market.symbol`` is available
    and is NOT the reference instrument (default XAUUSD), the function requires
    ``|corr(candidate, reference)| >= correlation_gate_threshold`` (default
    0.4 from ``cross_instrument_context.correlation_gate_threshold``). For
    instruments below the threshold (indices: US30/NAS100/GER40/UK100 |r|
    ≈0.34-0.36; JPY pairs USDJPY/GBPJPY |r| ≈0.18-0.24) the XAU-anchored
    DECISION GUIDANCE is noise and silently suppressed. Symbols whose
    correlation is unknown (no entry in the matrix, alias miss, etc.)
    fail-CLOSED — block suppressed — to avoid bleeding XAU bias into a
    silently-uninstrumented pair. The reference instrument itself
    (candidate == reference) bypasses the gate (self-correlation = 1.0;
    the block is the reference's own D1 view, never noise).
    """
    ...
    # B.1 correlation gate. Prefer the explicit runtime symbol passed by the
    # orchestrator; fall back to config.market so direct prompt callers cannot
    # accidentally bypass the XAU-anchor gate by omitting candidate_symbol.
    market_cfg = (config or {}).get("market", {})
    candidate_for_gate = (
        candidate_symbol
        or market_cfg.get("broker_symbol")
        or market_cfg.get("symbol")
    )
    if candidate_for_gate:
        ref_symbol = ci_cfg.get("reference_instrument", "XAUUSD")
        # Self-correlation bypass: XAU-on-XAU is the reference's own D1 view.
        if str(candidate_for_gate).upper() != str(ref_symbol).upper():
            try:
                threshold_raw = ci_cfg.get("correlation_gate_threshold", 0.4)
                threshold = abs(float(threshold_raw))
            except (TypeError, ValueError):
                threshold = 0.4
            # Lazy import to keep the prompt module free of MT5 / runtime deps.
            from src.components.cross_instrument_correlation_gate import (
                lookup_correlation,
            )
            corr = lookup_correlation(candidate_for_gate, ref_symbol)
            # Unknown correlation (None) -> fail-closed: suppress block. We
            # would rather drop the macro hint than bias an uninstrumented pair.
...
Evaluate this M15 candle. Run the PARALLEL EVALUATION block (Step C of the system prompt's DECISION ORDER): independently evaluate every active framework (ob_retest, fvg_fill, and breaker_re_entry when its addendum is present) against the MSO above and pick the framework whose qualification logic actually fires. If none qualify, emit NO_TRADE per Step D. The active kill zone is: {kill_zone}. Output your analysis as JSON."""
