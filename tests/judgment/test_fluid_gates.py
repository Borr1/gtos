...
from src.judgment.cycle import REQUIRED_LOG_KEYS, _required_log_consts, run_fluid_gate_cycle
...
from tests.judgment.cages import assert_refuse_broker
...
def test_apply_requires_flag_and_prove_and_never_place() -> None:
    ...
    if place_authorized():
        assert place.apply is True
        assert place.reason == "place_apply_authorized"
        assert place.mode == "place"
    else:
...
def test_cycle_shadow_log_and_no_apply(tmp_path: Path) -> None:
    ...
    for key in REQUIRED_LOG_KEYS:
        assert key in doc
    for key, value in _required_log_consts().items():
...
def test_cycle_high_confidence_place_stake_still_vetoes(tmp_path: Path) -> None:
    ...
    if place_authorized():
        assert result.apply.apply is True
        assert result.apply.reason == "place_apply_authorized"
        assert result.conf_gate.broker_effect is False
    else:
        assert result.apply.apply is False
...
def test_score_steals_attached() -> None:
    ...
    assert "ENFORCE JEV_EVERYWHERE" in text
    ...
    assert "0" in chair
    p0 = (REPO / "judgment" / "CHAIR_P0_SHADOW_ENABLE_HIST_PROVE.md").read_text(encoding="utf-8")
    assert "GTOS_JEV_FLUID_GATES_SHADOW=1" in p0
    assert "GTOS_JEV_FLUID_GATES_APPLY" in p0
    assert "EMPTY" in p0
