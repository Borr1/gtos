...
if not hits:
    deals = mt5.history_deals_get(now - timedelta(hours=24), now + timedelta(hours=2)) or []
    for d in deals:
...
if exit_deal:
    ...
    near_sl = abs(exit_px - ORIG_SL) <= max(0.00005, 0.25 * r_pts) if r_pts else False
    near_tp = abs(exit_px - TP) <= max(0.00005, 0.25 * abs(TP - ENTRY)) if TP else False
    import re
    ...
    if rname == "SL" or "[sl" in c or near_sl:
        ...
        if m:
            ...
            if abs(sl_lvl - ORIG_SL) > 0.00005 and abs(exit_px - ORIG_SL) > max(0.00005, 0.25 * r_pts):
                exit_class = "moved_stop"
            else:
                exit_class = "orig_stop"
        elif abs(exit_px - ORIG_SL) > max(0.0001, 0.5 * r_pts):
            exit_class = "moved_stop"
