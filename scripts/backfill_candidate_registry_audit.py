#!/usr/bin/env python3
"""Backfill append-only candidate-registry contract audit rows."""
...
DEFAULT_OUTPUT = Path("shadow_logs/candidate_registry_audit.jsonl")
DEFAULT_REPORT_JSON = Path("research/program_control/LTO002_CANDIDATE_REGISTRY_AUDIT_2026-05-05.json")
DEFAULT_REPORT_MD = Path("research/program_control/LTO002_CANDIDATE_REGISTRY_AUDIT_2026-05-05.md")
...
def build_report(rows: list[dict[str, Any]], appended_rows: list[dict[str, Any]], output_path: Path) -> dict[str, Any]:
    status_counts = Counter(str(row.get("registry_audit_status") or "UNKNOWN") for row in rows)
    ...
    action_examples = [
        ...
        for row in rows
        if row.get("registry_audit_status") == ACTION_REQUIRED
    ][:25]
    return {
        "schema_version": "lto002_candidate_registry_audit_v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "promotion_verdict": PROMOTION_VERDICT,
        "status": "ACTION_REQUIRED" if action_examples else "OK_WITH_DOCUMENTED_REGISTRY_LIMITATIONS",
        ...
        "registry_audit_status_counts": dict(status_counts),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# LTO-002 Candidate Registry Audit - 2026-05-05",
        "",
        ...
        "## Status Counts",
        "",
        f"`{report['registry_audit_status_counts']}`",
        "",
...
def main() -> int:
    ...
    rows = build_candidate_registry_audit_rows(
