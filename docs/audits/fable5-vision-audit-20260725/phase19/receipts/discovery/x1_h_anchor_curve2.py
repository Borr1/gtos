...
def main():
    ...
    ew = [json.loads(l) for l in gzip.open(f"{DISC}/x1_H_SEED_V1.jsonl.gz", "rt")]
    ...
    with gzip.open(f"{DISC}/x1_H_ANCHOR2_ROWS_V1.jsonl.gz", "wt") as f:
        for rec in out:
    ...
    for f_, rs in fam.items():
        ...
        res["per_family_curve"][f_]["A_as_ran"] = cl(
            ...
            [d for x, d in zip(rs, days) if x["A_r"] is not None])
    with open(f"{DISC}/x1_H_ANCHOR2_CURVE_V1.json", "w") as f:
