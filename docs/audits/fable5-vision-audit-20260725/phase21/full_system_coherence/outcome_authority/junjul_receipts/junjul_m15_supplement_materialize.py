#!/usr/bin/env python3
"""M15 lead-in supplement materializer (june_2026 only) — executed 2026-08-11.

Verbatim record of the driver run against the LANE_INPUTS_TRUE_UTC_V1 hold from
a sparse origin/main (8afdfd849) worktree. See
JUNJUL_M15_SUPPLEMENT_MATERIALIZATION_V1.md; pre-write cross-check detail in
M15_SUPP_XCHECK.json.
"""
...
LEADIN = Path("/tmp/junjul-lane-mat-20260810/export/junjul_2026_m15_leadin_20260811")
SCR = Path("/tmp/junjul-lane-mat-20260810/scratch")
FAMILY = "junjul_2026_m15_leadin_20260811"
COMMIT = "e81a166e1433a0aea910752a3b000a5f15cb59a9"
EXPECT_JUNE_ROOT = "68c4ce3f2fcda3a9216b87520a6970e2e6b8dd1e5a68a25b686f2e9259d9178c"
...
(SCR/"LANE_INPUT_REGISTRY.pre-m15supp.backup.json").write_bytes(reg_before_bytes)
jm_before_bytes = (HOLD/"manifests/june_2026.json").read_bytes()
(SCR/"june_2026.pre-m15supp.backup.json").write_bytes(jm_before_bytes)
...
if len(old_bars) != 120: fatal(f"expected 120 existing bar rows, got {len(old_bars)}")
...
if len(new_bars) != 144 or manifest["bar_source_count"] != 144 or manifest["bar_symbol_count"] != 24:
    fatal("counts wrong")
...
if {key(r): r for r in kept} != {key(r): r for r in old_bars}:
    fatal("existing 120 entries not preserved")
...
xcheck = json.loads((SCR/"M15_SUPP_XCHECK.json").read_text())
receipt_core = {
    "schema": lane.MATERIALIZATION_SCHEMA,
    "status": "LANE_TRUE_UTC_M15_LEADIN_SUPPLEMENT_COMPLETE",
...
lane._write_json(HOLD/"receipts/SOURCES_june_2026_m15_supplement.json", receipt)
print("done")
