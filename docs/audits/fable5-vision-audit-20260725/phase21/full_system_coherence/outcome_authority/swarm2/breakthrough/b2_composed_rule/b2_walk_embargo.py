...
EMBARGO = 1  # days held back from training beyond the strict prior-day rule
T0 = time.time()
...
def main():
    ...
    scores = {}
    pending: list = []   # rows awaiting absorption, EMBARGO days deep
        ...
        for day in days:
            ...
            if test:
                ...
                log(stage="day", month=month, day=day, n_test=len(test),
                    ...
                    fit_s=round(fit_s, 1))
            pending.append(rows)
            while len(pending) > EMBARGO:
                absorb(pending.pop(0))
        del by_day

    with gzip.open(HERE / "scores_embargo1.pkl.gz", "wb") as fh:
