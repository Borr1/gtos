...
class GovernorStateBuilder:
    ...
    def reconstruct_day_start_balance(self, mt5, now_utc: Optional[datetime] = None) -> Optional[float]:
        """Broker-correct daily-loss baseline = current balance - realized closed P&L since the server-day
        ...
        Mirrors the trusted monitor_books.py reconstruction. Returns None when balance or account-wide deal
        history cannot be read -> the caller must FAIL CLOSED rather than trust a wrong baseline.
        ...
        for d in deals:
            try:
                if int(d.get("entry", 0)) != 1:        # DEAL_ENTRY_OUT (close) only; skip opens/balance ops
                    continue
                t = d.get("time")
                if t is not None and t < boundary:     # precise boundary guard (no off-by-one at the edge)
                    continue
                realized += (float(d.get("profit") or 0.0)
