...
flag *and* a legal prove receipt exists. Jev places only via place_choice when GTOS_JEV_PLACE_APPLY=1 (Owner unlock). Fluid cycle LABEL/shadow here.
...
def _place_unlocked() -> bool:
    return str(__import__('os').environ.get('GTOS_JEV_PLACE_APPLY', '')).strip().lower() in {'1', 'true', 'yes', 'on'}
...
REQUIRED_LOG_CONSTS = {
    ...
    "never_place": (not _place_unlocked()),
    "never_remint": (not _place_unlocked()),
    "never_flatten": (not _place_unlocked()),
    "broker_effect": False,
}
