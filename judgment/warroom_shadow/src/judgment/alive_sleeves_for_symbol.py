...
try:
    from sleeve_select import F5_AFFINITY_KEEP_PRIORS  # type: ignore
except Exception:  # noqa: BLE001
    F5_AFFINITY_KEEP_PRIORS = {
...
def load_research_armed_tags() -> Tuple[List[str], Dict[str, Any]]:
    """Base overlay + ADD_* fragments; prefer EFFECTIVE merge if present.

    Also honors CANDIDATE_BUILT tags when GTOS_INCLUDE_CANDIDATE_BUILT=1 (optional).
    """
    ...
    # Optional: merge CANDIDATE_BUILT keys that have ON_SURFACE for requested symbols later
    if os.environ.get("GTOS_INCLUDE_CANDIDATE_BUILT", "").strip().lower() in ("1", "true", "yes"):
        ...
        # soft: ensure asian_fade present when CANDIDATE_BUILT lists it
        tags.add("asian_fade")
        meta["candidate_built_forced"] = ["asian_fade"]
    if not tags and meta["source"] is None:
        meta["source"] = "unavailable"
    # Chair APPLY SAME_TAG: asian_fade must be research-armed when ADD fragment exists
    add_af = _CL / "RESEARCH_ARMED_TAGS_OVERLAY_ADD_asian_fade_20260920.json"
    if add_af.exists() and "asian_fade" not in tags:
        tags.add("asian_fade")
        meta["chair_same_tag_ensure"] = "asian_fade"
...
def alive_sleeves_for_symbol(
    ...
    *,
    ...
    # F5 affinity KEEP priors (Chair/Edge named) — inject even if not yet ON_SURFACE-composed
    prior_injected: List[str] = []
    for p in F5_AFFINITY_KEEP_PRIORS.get(symbol, ()):
        if p not in alive:
            alive.append(p)
            prior_injected.append(p)
    alive = sorted(set(alive))
    ...
    return {
        ...
        "compose": {
            ...
            "corr_invented": False,
            "news_invented": False,
            "f5_keep_priors_injected": prior_injected,
            "f5_keep_priors_for_symbol": list(F5_AFFINITY_KEEP_PRIORS.get(symbol, ())),
        },
        "apply": False,
        "place": False,
        "promote": False,
