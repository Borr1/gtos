...
**Revision:** v2 — 20260411 — Fixed VWAP vs OB comparison language per PEER_REVIEW_AUDIT.md Finding 3.
...
| Instrument | N Events | Reversion Rate | Binom p | Assessment |
|------------|----------|----------------|---------|------------|
| XAUUSD | 1749 | 24.5% | 1.0000 | Too low for trading |
| US30 | 2168 | 22.8% | 1.0000 | Too low for trading |
| USDJPY | 931 | 19.4% | 1.0000 | Too low for trading |
| GBPJPY | 2284 | 23.1% | 1.0000 | Too low for trading |
| GBPUSD | 3528 | 22.3% | 1.0000 | Too low for trading |

> **Note (v2):** The original table included a "vs OB (70%)" column. This comparison has been removed.
> VWAP reversion rate (24.5%) and OB continuation rate (~70%) measure different phenomena
> and are not directly comparable: the VWAP metric measures whether price deviating 1 ATR
> from session VWAP returns within 0.25 ATR within 4 M15 bars (a reversion test), while the
> OB metric measures whether price reaching an order-block zone continues in the impulse
> direction (a continuation test). The conclusion that VWAP zones are not viable as entry
> signals stands on VWAP's absolute weakness (24.5% is too low regardless of any comparison),
> not on the numerical gap relative to OB rates.
...
- VWAP reversion rate (1.0 ATR, lag-4): 24.5% — too low for a viable trading signal on its own merits. Do NOT replace OB zones with VWAP zones. May serve as a confirming filter (price at VWAP + at OB zone = confluence). *(Note: a previously cited comparison to OB continuation rate has been removed — those metrics measure different phenomena and are not directly comparable. See table note above.)*
