...
- n_jumps=535 is sufficient for reliability — not preliminary on sample size
...
- **GBPJPY H1 continuation** replicated with 0/2 simpler methods at Bonferroni threshold  
  - Direction is consistent: both methods show positive lag_1 (Method A: 1.4bps p=0.048, Method B: 1.9bps p=0.130)  
  - Large-jump split (>median magnitude) recovers 3.0bps p=0.048 for Method A — confirms effect is concentrated in extreme moves  
  - The 4σ/24-bar original method selected only the top ~6% of 2.5σ+ jumps, where continuation is strongest (9.32bps)  
- **XAUUSD H1 reversion** replicated with 0/2 simpler methods at Bonferroni threshold  
  - Direction is consistent: both methods show negative lag_1 (Method A: -1.7bps, Method B: -4.2bps p=0.062)  
  - Effect strengthens with stricter threshold, consistent with the original finding  
