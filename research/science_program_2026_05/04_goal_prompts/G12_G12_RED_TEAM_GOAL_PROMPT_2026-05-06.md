# G12 Red Team Leakage Source Validity Goal Prompt - 2026-05-06

**Role:** `red_team`
**Domain:** `methodology red team`
...
Review top hypotheses for leakage, post-hoc selection, duplicate counting, synthetic-vs-actual label confusion, source invalidity, and promotion drift.
...
After the first synthesis pass, read outputs from neighboring lanes: G1, G9, G11. Add only cross-domain hypotheses that survive source, leakage, and killed-route checks.
...
Do not mark `G12` complete until files exist, focused checks are recorded, blockers are explicit, and every row/report carries `NO_PROMOTION_VERDICT`.
