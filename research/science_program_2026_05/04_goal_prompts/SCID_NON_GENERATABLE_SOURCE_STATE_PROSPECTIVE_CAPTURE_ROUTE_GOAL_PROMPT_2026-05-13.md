# SCID_NON_GENERATABLE_SOURCE_STATE_PROSPECTIVE_CAPTURE_ROUTE Goal Prompt

Evidence class: source-control/unblocking only.

Objective: follow the accepted G0 blocked-17 synthesis and build the strongest auditable source-control artifact for `prospective capture for non-generatable strategy/source-state fields`. Consume the exact blocked-17 denominator (`17` cards), keep the other `15` blocked cards excluded, keep ready-8 and expansion denominators untouched, and preserve source/status evidence without opening results.
...
Target route: `SCID_NON_GENERATABLE_SOURCE_STATE_PROSPECTIVE_CAPTURE_ROUTE`.

Cards touched by this route: `EXE-003, GEO-003, GEO-004, HAZ-003, MIC-003, UNC-005`.

Terminal closure required: every NON_GENERATABLE_HISTORICAL_SOURCE_STATE dependency is either recovered from explicit source-safe logs or converted into exact prospective capture fields and verifier fixtures.
