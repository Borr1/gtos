# G6 Momentum Continuation Breakout Mean Reversion Goal Prompt - 2026-05-06

**Role:** `science_lane`
**Domain:** `momentum, continuation, breakout, mean reversion`
...
Study continuation/no-retrace, opening ranges, impulse-pullback, trend persistence, reversal exhaustion, and stat-arb analogues as primitive mechanisms.
...
After the first synthesis pass, read outputs from neighboring lanes: G3, G5, G10. Add only cross-domain hypotheses that survive source, leakage, and killed-route checks.
...
Do not mark `G6` complete until files exist, focused checks are recorded, blockers are explicit, and every row/report carries `NO_PROMOTION_VERDICT`.
