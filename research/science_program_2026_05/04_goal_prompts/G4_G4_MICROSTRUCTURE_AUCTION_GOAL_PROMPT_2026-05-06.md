# G4 Market Microstructure Order Book Auction Goal Prompt - 2026-05-06
...
Extract mechanisms from OFI, queueing, liquidity, footprint, volume profile, VWAP, opening/closing auctions, stop cascades, and futures-proxy transfer.
...
After the first synthesis pass, read outputs from neighboring lanes: G3, G6, G11. Add only cross-domain hypotheses that survive source, leakage, and killed-route checks.
...
Do not mark `G4` complete until files exist, focused checks are recorded, blockers are explicit, and every row/report carries `NO_PROMOTION_VERDICT`.
