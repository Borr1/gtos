#!/usr/bin/env python3
"""Verify expanded-market repair implementation handoff execution checkpoint."""
...
CODE_FILES = [
    REPO / "src/research_infra/moonshot_expanded_market_repair_implementation_handoff_execution.py",
    ROUTE_DIR / "build_expanded_market_repair_implementation_handoff_execution_2026_05_17.py",
    Path(__file__),
]
OUTPUT_FILES = [
    RESULT_PATH,
...
def main() -> None:
    ...
    input_handoff_ids = ids_from(
        INPUT_HANDOFF_LEDGER,
    ...
    for row in iter_jsonl(HANDOFF_EXECUTION_LEDGER):
        execution_count += 1
        execution_input_ids.add(str(row.get("input_repair_implementation_handoff_row_id") or ""))
        execution_held_ids.add(str(row.get("input_repair_final_artifact_execution_row_id") or ""))
        decision_counts[row.get("keep_kill_redesign_implement_decision")] += 1
        if not boundary_ok(row):
            issues.append("one or more handoff execution rows failed branch-local boundary checks")
            break
        if row.get("handoff_execution_status") == "EXPANDED_MARKET_REPAIR_IMPLEMENTATION_HANDOFF_EXECUTION_PASS":
            execution_pass_count += 1
        else:
            issues.append("one or more handoff executions did not pass")
            break
        if row.get("held_final_artifact_execution_found") is not True or row.get("handoff_execution_match") is not True:
            issues.append("one or more handoff executions lacks held-row match proof")
            break
        if row.get("implementation_handoff_cost_adjusted_simulated_r") is None:
            issues.append("one or more handoff executions lacks simulated R")
            break
    ...
    for row in iter_jsonl(EVIDENCE_EXECUTION_LEDGER):
        evidence_count += 1
        evidence_input_ids.add(str(row.get("input_repair_implementation_handoff_evidence_row_id") or ""))
        decision_counts[row.get("keep_kill_redesign_implement_decision")] += 1
        if not boundary_ok(row):
            issues.append("one or more evidence execution rows failed branch-local boundary checks")
            break
        if row.get("implementation_handoff_execution_evidence_status") != "PRESERVED_NONCANDIDATE_REPAIR_IMPLEMENTATION_HANDOFF_EXECUTION_EVIDENCE":
    ...
    for row in iter_jsonl(CONTROL_EXECUTION_LEDGER):
        control_count += 1
        control_input_ids.add(str(row.get("input_repair_implementation_handoff_control_row_id") or ""))
        decision_counts[row.get("keep_kill_redesign_implement_decision")] += 1
        if not boundary_ok(row):
    ...
    if execution_count != 1734 or execution_pass_count != 1734:
        issues.append("handoff execution count mismatch")
    if execution_input_ids != input_handoff_ids:
        issues.append("handoff executions do not consume every handoff row")
    if execution_held_ids != held_ids:
        issues.append("handoff executions do not consume every held final-artifact execution row")
    if evidence_count != 44386:
        issues.append("evidence execution count mismatch")
    if evidence_input_ids != input_evidence_ids:
        issues.append("evidence executions do not consume every handoff evidence row")
    ...
    if control_input_ids != input_control_ids:
        issues.append("control executions do not consume every handoff control row")
    if counts.get("handoff_execution_rows") != execution_count:
        issues.append("result handoff execution count mismatch")
    if counts.get("handoff_execution_pass_rows") != execution_pass_count:
        issues.append("result handoff execution pass count mismatch")
        ...
        json.dumps(
            {
                ...
                "issues": issues,
                "counts": {
                    "handoff_execution_rows": execution_count,
