#!/usr/bin/env python3
"""Build implementation-priority rows from alternate-source scoring."""
...
from src.research_infra.moonshot_expanded_market_implementation_priority import (
    EXPANDED_MARKET_IMPLEMENTATION_PRIORITY,
    boundary_row,
    implementation_priority_row,
    issue_rows,
    research_boundary,
)
...
HELPER_MODULE = REPO / "src/research_infra/moonshot_expanded_market_implementation_priority.py"
BUILDER_MODULE = Path(__file__)
VERIFIER_MODULE = ROUTE_DIR / "verify_expanded_market_implementation_priority_2026_05_17.py"
...
def aggregate_key(row: dict[str, Any]) -> tuple[str, ...]:
    return (
        str(row.get("implementation_priority_class") or ""),
        str(row.get("implementation_priority_tier") or ""),
...
def empty_bucket() -> dict[str, Any]:
    return {
        "row_count": 0,
        "implementation_candidate_rows": 0,
        "missing_rows": 0,
        "priority_sum": 0.0,
        "priority_count": 0,
        "alternate_sum": 0.0,
...
def update_bucket(bucket: dict[str, Any], row: dict[str, Any]) -> None:
    ...
    if row.get("implementation_priority_class") == "implementation-priority-positive-alternate-source-repair":
        bucket["implementation_candidate_rows"] += 1
            ...
            boundary_row(
                {
                    "expanded_market_implementation_priority_aggregate_row_id": (
                        f"OHLC-GTOS-EXPANDED-MARKET-IMPLEMENTATION-PRIORITY-AGG-{len(rows) + 1:06d}"
                    ),
                    ...
                    "implementation_candidate_rows": bucket["implementation_candidate_rows"],
                    "missing_rows": bucket["missing_rows"],
                    "average_implementation_priority_score": rounded(
                        bucket["priority_sum"] / bucket["priority_count"] if bucket["priority_count"] else None
                    ),
                    "average_alternate_source_proxy_cost_adjusted_simulated_r": rounded(
                        bucket["alternate_sum"] / bucket["alternate_count"] if bucket["alternate_count"] else None
                    ),
...
def update_manifest(generated_at: str) -> None:
    ...
    entries = [
        (RESULT_PATH, "expanded_market_implementation_priority_result"),
        (ROW_LEDGER, "expanded_market_implementation_priority_rows"),
        (AGGREGATE_LEDGER, "expanded_market_implementation_priority_aggregates"),
        (ISSUE_LEDGER, "expanded_market_implementation_priority_issues"),
        (SYSTEM_LEDGER, "expanded_market_implementation_priority_system"),
        (SUMMARY_PATH, "expanded_market_implementation_priority_summary"),
        (BUILDER_MODULE, "expanded_market_implementation_priority_builder"),
        (VERIFIER_MODULE, "expanded_market_implementation_priority_verifier"),
        (HELPER_MODULE, "expanded_market_implementation_priority_helper"),
        (TEST_MODULE, "expanded_market_implementation_priority_tests"),
    ]
    rows = [
    ...
    manifest["latest_expanded_market_implementation_priority"] = {
...
def replace_active_checkpoint(generated_at: str, counts: dict[str, Any]) -> None:
    marker = "## Checkpoint 255 - Expanded-Market Implementation Priority"
...
Trigger: direct continuation after Checkpoint 254. Positive alternate-source repair rows were turned into implementation-priority rows with numeric priority scores, while all other rows were preserved as carry-forward, kill, redesign, acquisition, or signal-geometry evidence.

Rows:
- input alternate-source scoring rows: {counts["input_alternate_source_scoring_rows"]}
- implementation-priority rows: {counts["implementation_priority_rows"]}
...
Immediate continuation: materialize the positive alternate-source repair priority rows into branch-local implementation candidates with self-tests; keep all preserved rows row-addressable.
"""
    write_text(ACTIVE_LEDGER, text.rstrip() + "\n\n" + addition)


def build_summary(generated_at: str, counts: dict[str, Any], result: dict[str, Any]) -> str:
    return f"""# Expanded-Market Implementation Priority
...
def main() -> None:
    ...
    priority_class_counts: Counter[str] = Counter()
    ...
    with open(long_path(ROW_LEDGER), "w", encoding="utf-8", newline="\n") as handle:
        for row_count, source_row in enumerate(iter_jsonl(INPUT_ROW_LEDGER), 1):
            row = implementation_priority_row(source_row, row_count)
            handle.write(json.dumps(row, sort_keys=True) + "\n")
            priority_class_counts[row.get("implementation_priority_class")] += 1
            priority_tier_counts[row.get("implementation_priority_tier")] += 1
    ...
    implementation_candidate_rows = priority_class_counts.get("implementation-priority-positive-alternate-source-repair", 0)
    preserved_evidence_rows = row_count - implementation_candidate_rows
    system_rows = [
        boundary_row(
            {
                ...
                "implementation_candidate_rows": implementation_candidate_rows,
                "preserved_evidence_rows": preserved_evidence_rows,
                ...
                "metadata": {
                    "input_alternate_source_result_ok": input_result.get("ok"),
    ...
    counts = {
        "input_alternate_source_scoring_rows": row_count,
        "implementation_priority_rows": row_count,
        "aggregate_rows": len(aggregate_rows),
        "issue_rows": len(issues),
        "implementation_candidate_rows": implementation_candidate_rows,
    ...
    result = {
        ...
        "ok": not issues,
        "issues": issues,
        "expanded_market_implementation_priority_surface": EXPANDED_MARKET_IMPLEMENTATION_PRIORITY,
        "research_boundary": research_boundary(),
        "inputs": {
            "alternate_source_scoring_result": str(INPUT_RESULT.relative_to(REPO)).replace("\\", "/"),
            "alternate_source_scoring_rows": str(INPUT_ROW_LEDGER.relative_to(REPO)).replace("\\", "/"),
        },
        "outputs": {
    ...
    replace_sprint_event(
        {
            "event": "checkpoint_255_expanded_market_implementation_priority",
            "generated_utc": generated_at,
            "result": str(RESULT_PATH.relative_to(REPO)).replace("\\", "/"),
            "counts": counts,
