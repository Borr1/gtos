#!/usr/bin/env python3
"""Verify expanded-market repair application table checkpoint."""
...
CODE_FILES = [
    REPO / "src/research_infra/moonshot_expanded_market_repair_application_table.py",
    ROUTE_DIR / "build_expanded_market_repair_application_table_2026_05_17.py",
    Path(__file__),
]
OUTPUT_FILES = [RESULT_PATH, APPLICATION_LEDGER, EVIDENCE_LEDGER, CONTROL_LEDGER, AGGREGATE_LEDGER, ISSUE_LEDGER, SYSTEM_LEDGER, SUMMARY_PATH]
...
def main() -> None:
    ...
    input_execution_ids = {
        str(row.get("expanded_market_impl_candidate_execution_row_id") or "")
        for row in iter_jsonl(INPUT_EXECUTION_LEDGER)
    }
    input_evidence_ids = {
        str(row.get("expanded_market_impl_candidate_evidence_execution_row_id") or "")
        for row in iter_jsonl(INPUT_EVIDENCE_LEDGER)
    }
    input_control_ids = {
        str(row.get("expanded_market_impl_candidate_control_row_id") or "")
    ...
    for row in iter_jsonl(APPLICATION_LEDGER):
        application_count += 1
        application_input_ids.add(str(row.get("input_impl_candidate_execution_row_id") or ""))
        decision_counts[row.get("keep_kill_redesign_implement_decision")] += 1
        if not boundary_ok(row):
            issues.append("one or more application rows failed branch-local boundary checks")
            break
        if row.get("repair_application_status") == "BRANCH_LOCAL_REPAIR_APPLICATION_READY":
            ready_count += 1
        else:
            issues.append("one or more repair application rows is not ready")
            break
        if row.get("repair_application_cost_adjusted_simulated_r") in (None, ""):
            issues.append("one or more repair application rows lacks simulated R")
            break
        if not row.get("repair_application_source_path") or not row.get("repair_application_source_file_sha256"):
            issues.append("one or more repair application rows lacks source path/hash")
            break
    ...
    for row in iter_jsonl(EVIDENCE_LEDGER):
        evidence_count += 1
        evidence_input_ids.add(str(row.get("input_impl_candidate_evidence_execution_row_id") or ""))
        decision_counts[row.get("keep_kill_redesign_implement_decision")] += 1
        if not boundary_ok(row):
            issues.append("one or more evidence rows failed branch-local boundary checks")
            break
        if row.get("repair_application_evidence_status") != "PRESERVED_NONCANDIDATE_REPAIR_APPLICATION_EVIDENCE":
    ...
    for row in iter_jsonl(CONTROL_LEDGER):
        control_count += 1
        control_input_ids.add(str(row.get("input_impl_candidate_control_row_id") or ""))
        decision_counts[row.get("keep_kill_redesign_implement_decision")] += 1
        if not boundary_ok(row):
        ...
        if row.get("control_status") == "EXPANDED_MARKET_REPAIR_APPLICATION_CONTROL_PASS":
            ...
            issues.append("one or more repair application controls did not pass")
            break
        if row.get("noncandidate_scope_leakage_count") != 0:
            issues.append("repair application controls must report zero leakage")
            break
    ...
    if application_count != 1734 or ready_count != 1734:
        issues.append("application count mismatch")
    if application_input_ids != input_execution_ids:
        issues.append("application rows do not consume every input execution row")
    if evidence_count != 44386:
        issues.append("evidence count mismatch")
    if evidence_input_ids != input_evidence_ids:
        issues.append("evidence rows do not consume every input evidence execution row")
    ...
    if counts.get("application_rows") != application_count:
        issues.append("result application count mismatch")
    if counts.get("ready_application_rows") != ready_count:
        issues.append("result ready count mismatch")
