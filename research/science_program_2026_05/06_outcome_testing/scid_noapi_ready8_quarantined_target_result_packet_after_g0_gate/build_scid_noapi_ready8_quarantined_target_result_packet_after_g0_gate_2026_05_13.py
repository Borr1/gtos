"""Build the quarantined READY8 no-API neutral target-result packet.
...
G0-opened READY8 packet, while preserving NO_PROMOTION_VERDICT and leaving
independent acceptance to the next G12 audit.
"""
...
DATE_TAG = "2026-05-13"
PREFIX = "SCID_NOAPI_READY8_TARGET_RESULT"
ROUTE_ID = "SCID_NOAPI_READY8_QUARANTINED_TARGET_RESULT_PACKET_AFTER_G0_GATE"
EVIDENCE_CLASS = "SCID_NOAPI_READY8_QUARANTINED_TARGET_RESULT_PACKET_ONLY"
SCHEMA_VERSION = "scid_noapi_ready8_quarantined_target_result_packet_v1"
TERMINAL_DECISION = "MATERIALIZED_QUARANTINED_READY8_NOAPI_TARGET_RESULT_PACKET_G12_AUDIT_REQUIRED"

CONTROLLING_PROMPT = (
    PROMPT_DIR / "SCID_NOAPI_READY8_QUARANTINED_RESULT_PACKET_AFTER_G0_GATE_GOAL_PROMPT_2026-05-13.md"
)
NEXT_G12_PROMPT = (
    PROMPT_DIR / "G12_SCID_NOAPI_READY8_QUARANTINED_TARGET_RESULT_PACKET_AUDIT_GOAL_PROMPT_2026-05-13.md"
)
NEXT_G12_STARTER = (
    ROUTE_DIR / "G12_SCID_NOAPI_READY8_QUARANTINED_TARGET_RESULT_PACKET_AUDIT_STARTER_2026-05-13.txt"
)

G0_GATE_DIR = OUTCOME_DIR / "g0napi_ready8_future_result_opening_gate_after_g12_audit"
G12_AUDIT_DIR = OUTCOME_DIR / "g12_scid_noapi_ready8_rowset_target_horizon_packet_audit"
...
G0_DECISION = G0_GATE_DIR / "G0NAPI_READY8_DECISION_LEDGER_2026-05-13.json"
G0_VERIFICATION = G0_GATE_DIR / "G0NAPI_READY8_VERIFICATION_RESULT_2026-05-13.json"
G12_DECISION = G12_AUDIT_DIR / "G12_SCID_NOAPI_READY8_DECISION_LEDGER_2026-05-12.json"
READY8_ROWSET_MANIFEST = READY8_PACKET_DIR / "SCID_NOAPI_READY8_ROWSET_MANIFEST_2026-05-12.json"
READY8_ROWSET_ROWS = READY8_PACKET_DIR / "SCID_NOAPI_READY8_ROWSET_ROWS_2026-05-12.jsonl"
READY8_TARGET_CONTRACT = READY8_PACKET_DIR / "SCID_NOAPI_READY8_TARGET_HORIZON_CONTRACT_2026-05-12.json"
READY8_DUPLICATE_MANIFEST = READY8_PACKET_DIR / "SCID_NOAPI_READY8_DUPLICATE_DENOMINATOR_MANIFEST_2026-05-12.json"
READY8_PARTITION_MANIFEST = READY8_PACKET_DIR / "SCID_NOAPI_READY8_PARTITION_CONTROL_MANIFEST_2026-05-12.json"
...
def collect_rowset_denominator() -> dict[str, Any]:
    ...
    for row in iter_jsonl(READY8_ROWSET_ROWS):
        ...
        partition = row.get("partition_assignment")
        per_card[card_id] += 1
        ...
        per_baseline_family[row.get("baseline_assignment_family")] += 1
        if candidate_id:
...
def common_row_fields(rowset_row: dict[str, Any], target_family: str, horizon: int) -> dict[str, Any]:
    ...
    return {
        **COMMON_SAFE_FIELDS,
        ...
        "partition_assignment": rowset_row.get("partition_assignment"),
    ...
    try:
        for card_id in READY_CARD_IDS:
            for family in TARGET_FAMILIES:
                ...
                path.parent.mkdir(parents=True, exist_ok=True)
                handles[(card_id, family)] = path.open("w", encoding="utf-8", newline="\n")
    ...
    return {
        **safe_base("target_source_join_ledger"),
        ...
        "actual_scope": {
            ...
            "accepted_card_denominator": g0_decision.get("ready8_scope", {}).get("accepted_card_denominator"),
            "blocked_dependencies_preserved_outside_denominator": g0_decision.get("ready8_scope", {}).get("blocked_dependency_rows"),
            "row_exclusions": g0_decision.get("ready8_scope", {}).get("row_level_exclusions"),
        },
        "input_hash_checks": {
        ...
        "upstream_gate_acceptance": {
            ...
            "g0_result_packet_prompt_opened": g0_decision.get("result_packet_prompt_opened"),
            "g0_may_open_future_quarantined_result_packet_prompt": g0_decision.get("may_open_future_quarantined_result_packet_prompt"),
            "g0_may_score_results_now": g0_decision.get("may_score_results_now"),
        },
...
def duplicate_denominator_ledger(denominator: dict[str, Any], target_stats: dict[str, Any]) -> dict[str, Any]:
    return {
        **safe_base("duplicate_and_denominator_ledger"),
        ...
        "formula": "24112 ready8 rowset rows x 4 horizons x 2 neutral target families = 192896 target-result rows",
...
def partition_control_ledger(denominator: dict[str, Any], target_stats: dict[str, Any]) -> dict[str, Any]:
    return {
        **safe_base("partition_control_ledger"),
        "partition_policy": "READY8 preserves upstream sealed/stress partition assignments; no discovery/development/forward/contaminated pools are opened.",
        "source_candidate_partition_counts": {
    ...
    return {
        **safe_base("adjacent_noapi_route_family_sidecar_ledger"),
        ...
        "anti_boxing_questions_pursued": [
            "Could source coverage, duplicate stability, calendar/fix context, lifecycle status, or proxy validity explain later packet quality without entering the ready8 denominator?",
...
def saturation_md(target_stats: dict[str, Any]) -> str:
    return f"""# READY8 Quarantined Target Result Saturation And Self-Red-Team
...
- Denominator leakage: checked by the duplicate/denominator ledger. The packet contains `{target_stats['target_result_row_count']}` target terminal rows from `24,112` ready-8 rowset rows only; blocked dependencies and expansion candidates remain outside the denominator.
...
def completion_audit(denominator: dict[str, Any], target_stats: dict[str, Any]) -> dict[str, Any]:
    checklist = [
        ...
        ("g0_gate_artifacts_read", True, repo_path(G0_GATE_DIR)),
        ("g12_audit_route_read", True, repo_path(G12_AUDIT_DIR)),
        ("ready8_source_control_packet_read", True, repo_path(READY8_PACKET_DIR)),
        ...
        ("standalone_verifier_exists", True, f"verify_scid_noapi_ready8_quarantined_target_result_packet_after_g0_gate_{DATE_TAG.replace('-', '_')}.py"),
        ("focused_tests_exist", True, f"test_scid_noapi_ready8_quarantined_target_result_packet_after_g0_gate_{DATE_TAG.replace('-', '_')}.py"),
        ("next_g12_prompt_and_starter_exist", NEXT_G12_PROMPT.exists() and NEXT_G12_STARTER.exists(), f"{repo_path(NEXT_G12_PROMPT)} / {repo_path(NEXT_G12_STARTER)}"),
        ("no_forbidden_surfaces_opened", True, "safe flags and verifier forbidden-field scan"),
        ("terminal_decision_allowed", TERMINAL_DECISION in {"MATERIALIZED_QUARANTINED_READY8_NOAPI_TARGET_RESULT_PACKET_G12_AUDIT_REQUIRED", "KEEP_READY8_RESULT_PACKET_CLOSED_WITH_EXACT_REPAIR_OR_SOURCE_BLOCKERS"}, TERMINAL_DECISION),
    ]
    return {
        **safe_base("completion_audit"),
        "objective_restatement": "Materialize the separate quarantined no-API neutral target-result packet for exactly the accepted READY8 rowset after G0 gate acceptance, with fail-closed statuses and no validation/promotion/live/API/broker/paid/remote/registry/trading-risk surfaces.",
        ...
        "anti_boxing_questions_pursued": [
            ...
            "adjacent no-API route families inventoried as quarantined sidecars",
        ],
...
def next_g12_prompt_text() -> str:
    return f"""# G12 SCID No-API READY8 Quarantined Target Result Packet Audit
...
4. Read the G0 gate, G12 source-control audit, READY8 materialization packet, and this result-packet route at `{repo_path(ROUTE_DIR)}`.
...
def next_g12_starter_text() -> str:
    return (
        ...
        "run mandatory preflight/context refresh first; do not rely on chat or compaction memory; stay "
        "G12_SCID_NOAPI_READY8_QUARANTINED_TARGET_RESULT_PACKET_AUDIT_ONLY with no validation/promotion/live/API/"
        "paid-vendor/broker-account-order-history-deal-position/raw-market-blob/registry/remote/trading-risk-safety-"
        "prompt-decision changes; independently audit the READY8 target-result packet counts, hashes, source/as-of joins, "
        "fail-closed statuses, sidecar quarantine, forbidden-field closure, verifier/focused tests, and completion audit; "
        "emit exact accept/reject decision, next G0 or repair prompt/starter, scoped commits, NO_PROMOTION_VERDICT, "
        "validation_safe=false, outcome_review_opened=false, live_effect=false; pursue blockers until cleared, proven "
        "impossible from approved routes, or reduced to exact source/access/capture requirements."
    )
...
def output_manifest(extra_paths: Iterable[Path] = ()) -> dict[str, Any]:
    artifact_paths = [
        ROOT / ".gitattributes",
        Path(__file__),
        ROUTE_DIR / f"verify_scid_noapi_ready8_quarantined_target_result_packet_after_g0_gate_{DATE_TAG.replace('-', '_')}.py",
        ROUTE_DIR / f"test_scid_noapi_ready8_quarantined_target_result_packet_after_g0_gate_{DATE_TAG.replace('-', '_')}.py",
        NEXT_G12_PROMPT,
        NEXT_G12_STARTER,
        *result_paths(),
