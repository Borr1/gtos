"""Verify the READY8 quarantined no-API target-result packet."""
...
def required_files() -> list[Path]:
    return [
        ...
        Path(__file__),
        ROUTE_DIR / "test_scid_noapi_ready8_quarantined_target_result_packet_after_g0_gate_2026_05_13.py",
...
def verify() -> dict[str, Any]:
    ...
    checks["required_files_exist"] = all(path.exists() for path in files)
    for path in files:
        if not path.exists():
            issues.append({"check": "required_files_exist", "path": builder.repo_path(path)})

    json_files = [path for path in files if path.suffix == ".json" and path.exists()]
    ...
    checks["next_g12_prompt_exists"] = builder.NEXT_G12_PROMPT.exists() and builder.NEXT_G12_STARTER.exists()
    ...
    return {
        ...
        "schema_version": "scid_noapi_ready8_quarantined_target_result_packet_verifier_v1",
