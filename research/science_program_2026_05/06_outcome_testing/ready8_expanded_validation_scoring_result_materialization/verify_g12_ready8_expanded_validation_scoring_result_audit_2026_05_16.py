"""Verify G12 READY8 expanded scoring result audit artifacts."""
...
import build_g12_ready8_expanded_validation_scoring_result_audit_2026_05_16 as builder
...
def verify(mark_focused_tests_ok: bool = False, focused_tests_summary: str | None = None) -> dict[str, Any]:
    ...
    required_files = [
        ...
        f"G12_R8EXP_SCORING_AUDIT_FAILURE_INTEL_{DATE}.jsonl",
        f"G12_R8EXP_SCORING_AUDIT_INSTRUCTION_COVERAGE_{DATE}.json",
        f"G12_R8EXP_SCORING_AUDIT_MATERIAL_ROWS_{DATE}.jsonl",
        f"G12_R8EXP_SCORING_AUDIT_OUTPUT_MANIFEST_{DATE}.json",
        f"G12_R8EXP_SCORING_AUDIT_PACKET_COVERAGE_{DATE}.jsonl",
        f"G12_R8EXP_SCORING_AUDIT_R8_MANIFEST_{DATE}.jsonl",
        f"G12_R8EXP_SCORING_AUDIT_RECOMPUTATION_{DATE}.json",
        f"G12_R8EXP_SCORING_AUDIT_REPAIRED_TARGET_{DATE}.jsonl",
        f"G12_R8EXP_SCORING_AUDIT_SOURCE_DRIFT_{DATE}.jsonl",
        f"G12_R8EXP_SCORING_AUDIT_SOURCE_HASH_{DATE}.json",
        f"G12_R8EXP_SCORING_AUDIT_SYNTHESIS_{DATE}.md",
    ...
    if missing:
        errors.append(f"missing required G12 audit files: {missing}")
    checks["required_files_exist"] = not missing

    counts = {
        "material_rows": count_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_MATERIAL_ROWS_{DATE}.jsonl")),
        "packet_coverage": count_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_PACKET_COVERAGE_{DATE}.jsonl")),
        "failure_intel": count_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_FAILURE_INTEL_{DATE}.jsonl")),
        "repaired_target": count_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_REPAIRED_TARGET_{DATE}.jsonl")),
        "source_drift": count_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_SOURCE_DRIFT_{DATE}.jsonl")),
        "r8_manifest": count_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_R8_MANIFEST_{DATE}.jsonl")),
    }
    expected_counts = {
        "material_rows": 11112,
        "packet_coverage": 182,
    ...
    for key, expected in expected_counts.items():
        if counts[key] != expected:
            errors.append(f"{key} count {counts[key]} != {expected}")
    checks["audit_row_counts"] = counts
    checks["audit_row_counts_match_expected"] = all(counts[k] == v for k, v in expected_counts.items())

    decision = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_DECISION_{DATE}.json"))
    completion = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_COMPLETION_{DATE}.json"))
    recomputation = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_RECOMPUTATION_{DATE}.json"))
    source_hash = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_SOURCE_HASH_{DATE}.json"))
    instruction = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_INSTRUCTION_COVERAGE_{DATE}.json"))
    for label, doc in [
        ("decision", decision),
    ...
    if not checks["completion_standard_met"]:
        errors.append("completion standard not met")
    ...
    packet_bad = [
        row["packet_row_id"]
        for row in iter_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_PACKET_COVERAGE_{DATE}.jsonl"))
        if row.get("audit_status") != "PACKET_ROW_FULLY_COVERED"
    ]
    checks["packet_coverage_all_ok"] = not packet_bad
    if packet_bad:
        errors.append(f"packet coverage failures: {packet_bad[:10]}")

    bad_failure = [
        row.get("source_line_number")
        for row in iter_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_FAILURE_INTEL_{DATE}.jsonl"))
    ...
    bad_repaired = [
        row.get("source_line_number")
        for row in iter_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_REPAIRED_TARGET_{DATE}.jsonl"))
    ...
    real_drift = [
        row["source_path"]
        for row in iter_jsonl(route_file(f"G12_R8EXP_SCORING_AUDIT_SOURCE_DRIFT_{DATE}.jsonl"))
        if row.get("drift_classification") == "REAL_IMMUTABLE_SOURCE_DRIFT" or row.get("blocking")
    ]
    checks["real_immutable_source_drift_count"] = len(real_drift)
    if real_drift:
        errors.append(f"real immutable source drift: {real_drift[:10]}")

    manifest = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_OUTPUT_MANIFEST_{DATE}.json"))
    ...
    for item in manifest.get("files", []):
        ...
        if current != item["sha256"]:
            if not item["path"].endswith(
                (
                    f"G12_R8EXP_SCORING_AUDIT_VERIFICATION_RESULT_{DATE}.json",
                    f"G12_R8EXP_SCORING_AUDIT_FOCUSED_TEST_RESULT_{DATE}.json",
                )
            ):
    ...
    for source in source_hash.get("sources", []):
        ...
        current_canonical = builder.sha256_file_canonical_lf(path)
        expected_hashes = {source.get("sha256"), source.get("sha256_canonical_lf")}
        if current_raw not in expected_hashes and current_canonical not in expected_hashes:
            if source["path"] in builder.MUTABLE_COORDINATION_PATHS:
                source_hash_warnings.append(f"bounded mutable coordination source drift {source['path']}")
            else:
    ...
    checks["g12_mutable_coordination_source_hash_drift_count"] = len(source_hash_warnings)
    ...
    for path in ROUTE_DIR.glob("G12_R8EXP_SCORING_AUDIT_*.jsonl"):
    ...
    if mark_focused_tests_ok:
        focused = {
            **builder.safe_base(),
            ...
            "status": "PASSED",
            "command": f"py -3 -m pytest {rel(ROUTE_DIR / 'test_g12_ready8_expanded_validation_scoring_result_audit_2026_05_16.py')} -q",
            "summary": focused_tests_summary or "focused pytest passed",
        }
        builder.write_json(route_file(f"G12_R8EXP_SCORING_AUDIT_FOCUSED_TEST_RESULT_{DATE}.json"), focused)
    elif route_file(f"G12_R8EXP_SCORING_AUDIT_FOCUSED_TEST_RESULT_{DATE}.json").exists():
        focused = read_json(route_file(f"G12_R8EXP_SCORING_AUDIT_FOCUSED_TEST_RESULT_{DATE}.json"))
    ...
    builder.write_json(route_file(f"G12_R8EXP_SCORING_AUDIT_VERIFICATION_RESULT_{DATE}.json"), result)
