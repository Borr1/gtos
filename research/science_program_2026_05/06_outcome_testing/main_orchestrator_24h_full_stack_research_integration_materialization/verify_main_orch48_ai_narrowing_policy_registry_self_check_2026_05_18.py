...
EXPECTED_MATCHED_POLICY_ROWS = 231
EXPECTED_MANIFEST_OUTPUT_COUNT = 2
EXPECTED_EVAL_STATUS_COUNTS = {
    "AI_NARROWING_REGISTRY_MATCH_KEEP_AI_UNCHANGED": 10,
    "AI_NARROWING_REGISTRY_MATCH_PRE_AI_SELECTOR_REVIEW_READY": 199,
    "AI_NARROWING_REGISTRY_MATCH_PRE_AI_SELECTOR_REVIEW_WITH_CAPACITY_BLOCKLIST": 22,
}
...
def verify() -> dict[str, Any]:
    ...
    if summary.get("matched_policy_rows") != EXPECTED_MATCHED_POLICY_ROWS:
        issues.append(f"matched_policy_rows_unexpected:{summary.get('matched_policy_rows')}")
    if summary.get("policy_self_check_pass_rows") != EXPECTED_ROWS:
        issues.append(f"policy_self_check_pass_rows_unexpected:{summary.get('policy_self_check_pass_rows')}")
    if summary.get("policy_self_check_fail_rows") != 0:
        issues.append(f"policy_self_check_fail_rows_nonzero:{summary.get('policy_self_check_fail_rows')}")
    ...
    if summary.get("ai_narrowing_registry_eval_status_counts") != EXPECTED_EVAL_STATUS_COUNTS:
        issues.append(f"eval_status_counts_unexpected:{summary.get('ai_narrowing_registry_eval_status_counts')}")
    ...
    if effect.get("default_off_ai_narrowing_registry_available") is not True:
        issues.append("effect_registry_not_available")
    ...
    for row in rows:
        row_id = row.get("ai_narrowing_registry_eval_row_id")
        if row.get("matched_policy_rows") != 1:
            issues.append(f"matched_policy_rows_not_one:{row_id}")
            break
        if not row.get("policy_self_check_pass"):
            issues.append(f"policy_self_check_failed:{row_id}")
            break
        for key in (
            "ai_call_skip_allowed_now",
            "production_change_opened_now",
    ...
    result = {
        ...
        "issues": issues,
        "rows": len(rows),
        "matched_policy_rows": summary.get("matched_policy_rows"),
        ...
        "ai_call_skip_allowed_now_rows": summary.get("ai_call_skip_allowed_now_rows"),
        ...
        "policy_self_check_fail_rows": summary.get("policy_self_check_fail_rows"),
