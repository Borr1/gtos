...
LEDGER = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_CONTRACT_LEDGER_{DATE}.jsonl"
SUMMARY = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_CONTRACT_SUMMARY_{DATE}.json"
MANIFEST = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_CONTRACT_MANIFEST_{DATE}.json"
VERIFY_RESULT = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_CONTRACT_VERIFY_RESULT_{DATE}.json"
...
def verify() -> dict[str, Any]:
    ...
    if summary.get("contract_rows") != len(rows):
        issues.append("contract_rows_mismatch")
    if len(rows) != 1072:
        issues.append(f"contract_rows_unexpected:{len(rows)}")
    if summary.get("ready_contract_rows") != 1072:
        issues.append(f"ready_contract_rows_unexpected:{summary.get('ready_contract_rows')}")
    if summary.get("input_action_rows") != 10969:
        issues.append(f"input_action_rows_unexpected:{summary.get('input_action_rows')}")
    if int(summary.get("min_required_event_field_count") or 0) <= 0:
        issues.append("required_event_fields_empty")
    ...
    required_event_field_counts = summary.get("required_event_field_counts") or {}
    for field in ("source_component", "proxy_r_class"):
        if required_event_field_counts.get(field, 0) <= 0:
            issues.append(f"required_event_field_missing:{field}")
            break
    ...
    for row in rows:
        ...
        if row.get("source_capture_contract_status") != "READY_DEFAULT_OFF_NUMERIC_ROUTER_CATALOG_EVENT_CONTRACT":
            issues.append(f"contract_not_ready:{row_id}")
            break
        if int(row.get("required_event_field_count") or 0) <= 0:
            issues.append(f"required_event_fields_empty:{row_id}")
            break
    ...
    result = {
        ...
        "ready_contract_rows": summary.get("ready_contract_rows"),
        "input_action_rows": summary.get("input_action_rows"),
        "catalog_type_counts": catalog_counts,
        "required_event_field_counts": required_event_field_counts,
