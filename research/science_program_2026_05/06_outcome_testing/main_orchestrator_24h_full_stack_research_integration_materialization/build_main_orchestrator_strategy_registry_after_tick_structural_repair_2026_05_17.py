"""Materialize strategy registry metadata after tick structural source repair."""

from __future__ import annotations

import hashlib
import json
import sys
from collections import Counter, defaultdict
from datetime import datetime, timezone
from pathlib import Path
from typing import Any


def find_repo_root(start: Path) -> Path:
    for candidate in [start, *start.parents]:
        if (candidate / ".git").exists():
            return candidate
    raise RuntimeError(f"Could not locate repo root from {start}")


REPO_ROOT = find_repo_root(Path(__file__).resolve())
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from src.research_infra.forward_capture import FOLLOW_STRATEGY_REGISTRY


DATE = "2026-05-17"
ROUTE_ID = "MAIN_ORCHESTRATOR_24H_FULL_STACK_RESEARCH_INTEGRATION_AND_RESULT_MATERIALIZATION"
ROUTE_DIR = Path(__file__).resolve().parent
SAFE_FLAGS = {
    "NO_PROMOTION_VERDICT": True,
    "validation_safe": False,
    "outcome_review_opened": False,
    "live_effect": False,
}

INPUT_TICK_LEDGER = ROUTE_DIR / "MAIN_ORCH24_TICK_STRUCTURAL_SOURCE_DERIVATION_REPAIR_LEDGER_2026-05-17.jsonl"
INPUT_TICK_SUMMARY = ROUTE_DIR / "MAIN_ORCH24_TICK_STRUCTURAL_SOURCE_DERIVATION_REPAIR_SUMMARY_2026-05-17.json"
INPUT_FORWARD_CAPTURE = Path("src/research_infra/forward_capture.py")
INPUT_FORWARD_CAPTURE_TESTS = Path("tests/test_forward_capture_shadow_loggers.py")

OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_STRATEGY_REGISTRY_AFTER_TICK_STRUCTURAL_REPAIR_LEDGER_{DATE}.jsonl"
OUTPUT_SUMMARY = ROUTE_DIR / f"MAIN_ORCH24_STRATEGY_REGISTRY_AFTER_TICK_STRUCTURAL_REPAIR_SUMMARY_{DATE}.json"
OUTPUT_MANIFEST = ROUTE_DIR / f"MAIN_ORCH24_STRATEGY_REGISTRY_AFTER_TICK_STRUCTURAL_REPAIR_OUTPUT_MANIFEST_{DATE}.json"

WATCHED_STRATEGY_IDS = (
    "V2_STRUCT_SWING_PROTECTED",
    "V2_STRUCT_FVG_MID_EDGE",
    "V3_FVG_ONLY_RESCUE_RISK_BANK",
)
STALE_TOKENS = (
    "190 source-repair rows",
    "3 true FVG source-repair rows remain",
    "source-repair rows remain",
    "remaining rows",
)


def utc_now() -> str:
    return datetime.now(timezone.utc).isoformat()


def sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def read_json(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def read_jsonl(path: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    with path.open("r", encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, start=1):
            if not line.strip():
                continue
            row = json.loads(line)
            row["_source_line_no"] = line_no
            rows.append(row)
    return rows


def write_jsonl(path: Path, rows: list[dict[str, Any]]) -> None:
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        for row in rows:
            handle.write(json.dumps(row, sort_keys=True, separators=(",", ":")) + "\n")


def write_json(path: Path, payload: dict[str, Any]) -> None:
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def safe_float(value: Any) -> float | None:
    try:
        return float(value)
    except (TypeError, ValueError):
        return None


def target_tick_rows(rows: list[dict[str, Any]], strategy_id: str) -> list[dict[str, Any]]:
    return [
        row
        for row in rows
        if row.get("strategy_id") == strategy_id
        and row.get("tick_structural_derivation_repair_status") != "NOT_TARGET_ROW"
    ]


def stats_for_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    values = [safe_float(row.get("after_proxy_r")) for row in rows]
    numeric = [value for value in values if value is not None]
    proxy_sum = round(sum(numeric), 8)
    return {
        "rows": len(rows),
        "numeric_proxy_rows": len(numeric),
        "proxy_r_sum": proxy_sum,
        "proxy_r_mean": round(proxy_sum / len(numeric), 8) if numeric else None,
        "action_counts": dict(Counter(row.get("action_class") for row in rows)),
        "implementation_decision_counts": dict(
            Counter(row.get("implementation_decision") for row in rows)
        ),
        "repair_status_counts": dict(
            Counter(row.get("tick_structural_derivation_repair_status") for row in rows)
        ),
    }


def stale_tokens(snapshot: dict[str, Any]) -> list[str]:
    text = " ".join(
        str(snapshot.get(field) or "")
        for field in ("branch_decision", "decision_evidence", "implementation_candidate")
    )
    return [token for token in STALE_TOKENS if token in text]


def build_rows() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    generated = utc_now()
    tick_rows = read_jsonl(INPUT_TICK_LEDGER)
    tick_summary = read_json(INPUT_TICK_SUMMARY)
    registry = {str(item.get("strategy_id") or ""): dict(item) for item in FOLLOW_STRATEGY_REGISTRY}
    output_rows: list[dict[str, Any]] = []

    for strategy_id in WATCHED_STRATEGY_IDS:
        snapshot = registry[strategy_id]
        rows = target_tick_rows(tick_rows, strategy_id)
        stats = stats_for_rows(rows)
        output_rows.append(
            {
                "row_id": f"MAIN-ORCH24-STRATEGY-REGISTRY-TICK-STRUCT-{len(output_rows) + 1:03d}",
                "route_id": ROUTE_ID,
                "generated_utc": generated,
                "strategy_id": strategy_id,
                "family": snapshot.get("family"),
                "evidence_role": snapshot.get("evidence_role"),
                "promotion_verdict": snapshot.get("promotion_verdict"),
                "branch_decision": snapshot.get("branch_decision"),
                "decision_evidence": snapshot.get("decision_evidence"),
                "implementation_candidate": snapshot.get("implementation_candidate"),
                "stale_metadata_tokens": stale_tokens(snapshot),
                "current_target_rows": stats["rows"],
                "current_numeric_proxy_rows": stats["numeric_proxy_rows"],
                "current_proxy_r_sum": stats["proxy_r_sum"],
                "current_proxy_r_mean": stats["proxy_r_mean"],
                "current_action_counts": stats["action_counts"],
                "current_implementation_decision_counts": stats[
                    "implementation_decision_counts"
                ],
                "current_repair_status_counts": stats["repair_status_counts"],
                "metadata_refresh_proxy_row_delta": 0,
                "metadata_refresh_proxy_r_sum_delta": 0.0,
                "exact_r_rows": 0,
                "safe_flags": SAFE_FLAGS,
                "no_live_behavior": True,
                "no_shadow_log_append": True,
            }
        )

    stale_count = sum(1 for row in output_rows if row["stale_metadata_tokens"])
    summary = {
        "route_id": ROUTE_ID,
        "evidence_class": "MAIN_ORCH24_STRATEGY_REGISTRY_AFTER_TICK_STRUCTURAL_REPAIR",
        "generated_utc": generated,
        "safe_flags": SAFE_FLAGS,
        "claim_boundary": (
            "Forward-capture strategy registry metadata is refreshed after the tick-derived "
            "source-repair closure. This changes future metadata labels only; it does not "
            "append shadow logs, rescore historical rows, change live behavior, or open "
            "promotion claims."
        ),
        "rows": len(output_rows),
        "watched_strategy_ids": list(WATCHED_STRATEGY_IDS),
        "input_action_rows": tick_summary["rows"],
        "tick_source_repair_rows_before": tick_summary["target_rows_before"],
        "tick_source_repair_rows_after": tick_summary["target_rows_remaining_source_repair"],
        "tick_numeric_proxy_row_delta": tick_summary["proxy_delta"]["numeric_proxy_rows_delta"],
        "tick_proxy_r_sum_delta": tick_summary["proxy_delta"]["proxy_r_sum_delta"],
        "metadata_rows_with_stale_tokens": stale_count,
        "metadata_refresh_proxy_row_delta": 0,
        "metadata_refresh_proxy_r_sum_delta": 0.0,
        "exact_r_rows": 0,
        "strategy_proxy_snapshot": {
            row["strategy_id"]: {
                "current_target_rows": row["current_target_rows"],
                "current_numeric_proxy_rows": row["current_numeric_proxy_rows"],
                "current_proxy_r_sum": row["current_proxy_r_sum"],
                "current_action_counts": row["current_action_counts"],
                "current_implementation_decision_counts": row[
                    "current_implementation_decision_counts"
                ],
                "stale_metadata_tokens": row["stale_metadata_tokens"],
            }
            for row in output_rows
        },
        "plate_decision": "STRATEGY_REGISTRY_METADATA_REFRESHED_AFTER_TICK_STRUCTURAL_SOURCE_REPAIR",
    }
    return output_rows, summary


def build_manifest(summary: dict[str, Any]) -> dict[str, Any]:
    inputs = {
        "input_tick_ledger": INPUT_TICK_LEDGER,
        "input_tick_summary": INPUT_TICK_SUMMARY,
        "forward_capture": INPUT_FORWARD_CAPTURE,
        "forward_capture_tests": INPUT_FORWARD_CAPTURE_TESTS,
    }
    outputs = {
        "ledger": OUTPUT_LEDGER,
        "summary": OUTPUT_SUMMARY,
    }
    return {
        "route_id": ROUTE_ID,
        "evidence_class": summary["evidence_class"],
        "generated_utc": summary["generated_utc"],
        "safe_flags": SAFE_FLAGS,
        "inputs": {
            name: {
                "path": str(path),
                "exists": path.exists(),
                "sha256": sha256_file(path) if path.exists() else None,
                "bytes": path.stat().st_size if path.exists() else None,
            }
            for name, path in inputs.items()
        },
        "outputs": {
            name: {"path": str(path), "sha256": sha256_file(path), "bytes": path.stat().st_size}
            for name, path in outputs.items()
        },
        "summary_counts": {
            "rows": summary["rows"],
            "metadata_rows_with_stale_tokens": summary["metadata_rows_with_stale_tokens"],
            "metadata_refresh_proxy_row_delta": summary["metadata_refresh_proxy_row_delta"],
            "metadata_refresh_proxy_r_sum_delta": summary["metadata_refresh_proxy_r_sum_delta"],
            "input_action_rows": summary["input_action_rows"],
        },
    }


def main() -> None:
    rows, summary = build_rows()
    write_jsonl(OUTPUT_LEDGER, rows)
    write_json(OUTPUT_SUMMARY, summary)
    write_json(OUTPUT_MANIFEST, build_manifest(summary))
    print(
        json.dumps(
            {
                "ok": True,
                "rows": summary["rows"],
                "metadata_rows_with_stale_tokens": summary[
                    "metadata_rows_with_stale_tokens"
                ],
                "metadata_refresh_proxy_row_delta": summary[
                    "metadata_refresh_proxy_row_delta"
                ],
                "metadata_refresh_proxy_r_sum_delta": summary[
                    "metadata_refresh_proxy_r_sum_delta"
                ],
            },
            sort_keys=True,
        )
    )


if __name__ == "__main__":
    main()
