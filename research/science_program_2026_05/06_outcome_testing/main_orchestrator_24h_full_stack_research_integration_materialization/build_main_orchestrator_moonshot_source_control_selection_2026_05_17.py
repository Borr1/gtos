"""Select concrete actions from moonshot source/control repair rows."""
...
OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_SOURCE_CONTROL_SELECTION_LEDGER_{DATE}.jsonl"
SUMMARY_PATH = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_SOURCE_CONTROL_SELECTION_SUMMARY_{DATE}.json"
MANIFEST_PATH = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_SOURCE_CONTROL_SELECTION_OUTPUT_MANIFEST_{DATE}.json"
...
def read_source_control_rows(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, 1):
            if not line.strip():
                continue
            row = json.loads(line)
            if row.get("main_action_class") != "SOURCE_REPAIR":
                continue
...
def add_source_control_payload(
    row: dict[str, Any],
    *,
    sequence: int,
    generated_utc: str,
    ...
    selected["source_control_selection_row_id"] = f"MAIN-ORCH24-MOONSHOT-SOURCE-CONTROL-SELECTION-{sequence:06d}"
    ...
    selected["source_control_selection_not_standalone_promotion"] = True
    selected["missed_opportunity_audit"] = {
        "what_was_tried": "branch-local source/control repair row was imported and selected on main",
        "why_not_live_or_independently_countable": (
            "row is source/control or proxy-control evidence; it is not broker actual R or validation-safe R"
        ),
        "mechanism_remains_useful": selected.get("computed_decision"),
        "downstream_path": selected.get("downstream_path"),
        "next_action": selected.get("source_control_selection_decision"),
    }
    return selected
...
def build() -> dict[str, Any]:
    ...
    input_rows = read_source_control_rows(INPUT_LEDGER)
    selected_rows = [
        add_source_control_payload(row, sequence=index, generated_utc=generated_utc, input_sha256=input_sha256)
        for index, row in enumerate(input_rows, 1)
    ]

    summary = summarize_source_control_selection(selected_rows)
    summary.update(
        {
            ...
            "input_ledger": {
                ...
                "source_control_rows": len(input_rows),
            },
            "safe_flags": SAFE_FLAGS,
            "implement_default_off_rows": summary["source_control_selection_class_counts"].get(
                "IMPLEMENT_DEFAULT_OFF", 0
            ),
            "redesign_rows": summary["source_control_selection_class_counts"].get("REDESIGN", 0),
            "source_repair_rows": summary["source_control_selection_class_counts"].get("SOURCE_REPAIR", 0),
            "unclassified_rows": summary["source_control_selection_class_counts"].get("UNCLASSIFIED", 0),
            "implement_default_off_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("source_control_selection_class") == "IMPLEMENT_DEFAULT_OFF"
            ),
            "redesign_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("source_control_selection_class") == "REDESIGN"
            ),
            "source_repair_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("source_control_selection_class") == "SOURCE_REPAIR"
            ),
            "source_control_class_by_source_component": {
                component: dict(
                    Counter(
                        row["source_control_selection_class"]
    ...
    manifest = {
        ...
        "date": DATE,
        "generated_utc": generated_utc,
        "description": "Concrete source/control repair, default-off, and redesign selection from moonshot repair rows.",
        "input_files": {
            "moonshot_action_intake_ledger": summary["input_ledger"],
        },
        "output_files": {
            "source_control_selection_ledger": {
