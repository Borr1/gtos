...
STRATEGIES = {"V2_STRUCT_OB_BOUNDARY", "V2B_OB_BOUNDARY_PROSPECTIVE"}
SAFE_FLAGS = {
...
def main() -> None:
    ...
    for row in sorted(rows, key=lambda item: (str(item.get("strategy_id")), str(item.get("candidate_id")))):
        ledger_rows.append(
            {
                "row_id": "MAIN-ORCH24-V2-V2B-DUPAWARE-" + stable_id(row.get("candidate_id"), row.get("strategy_id")),
    ...
    summary = {
        ...
        "decision": "REPLAY_DUPLICATE_AWARE_OB_BOUNDARY_ONLY_NO_PROMOTION",
        "interpretation": "Current disk duplicate-aware numeric proxy rows are slightly positive in aggregate but remain replay/shadow evidence only, not validation or promotion.",
        "safe_flags": SAFE_FLAGS,
    }
    ...
    manifest = {
        ...
        "artifact_count": 3,
        "artifacts": [
            artifact_record("duplicate_aware_replay_ledger", ledger_path),
