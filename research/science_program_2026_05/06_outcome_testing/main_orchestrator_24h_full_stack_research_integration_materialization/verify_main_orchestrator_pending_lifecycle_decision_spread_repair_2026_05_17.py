...
VERIFY_RESULT = ROUTE_DIR / (
    f"MAIN_ORCH24_PENDING_LIFECYCLE_DECISION_SPREAD_REPAIR_VERIFICATION_RESULT_{DATE}.json"
)
...
EXPECTED_STATUS_COUNTS = {
    ...
    "PENDING_LIFECYCLE_DECISION_SPREAD_REPAIRED_FROM_LEGACY_SPREAD_PROXY": 5,
    "PENDING_LIFECYCLE_RECOMPUTE_APPLIED_NO_DECISION_SPREAD_REPAIR": 269,
}
EXPECTED_BEFORE_PENDING_BRANCH_COUNTS = {
...
EXPECTED_AFTER_PENDING_BRANCH_COUNTS = {
    "KEEP_PENDING_LIFECYCLE_HYPOTHETICAL_PATH_SCORER": 235,
...
EXPECTED_BEFORE_DECISION_SPREAD_STATUS_COUNTS = {
    "NO_PENDING_LIFECYCLE_STATUS_MAP": 235,
    "SOURCE_NOT_CAPTURED_IN_CURRENT_ROW": 39,
}
EXPECTED_AFTER_DECISION_SPREAD_STATUS_COUNTS = {
    "DERIVED_FROM_PENDING_LIFECYCLE_LEGACY_SPREAD_AS_DECISION_SPREAD_PROXY": 5,
    "NO_PENDING_LIFECYCLE_STATUS_MAP": 235,
    "SOURCE_NOT_CAPTURED_IN_CURRENT_ROW": 34,
}
EXPECTED_BEFORE_SOURCE_COMPLETE_COUNTS = {"": 235, "False": 39}
EXPECTED_AFTER_SOURCE_COMPLETE_COUNTS = {"": 235, "False": 34, "True": 5}
EXPECTED_RECONSTRUCTED_CANDIDATES = [
...
def main() -> None:
    ...
    checks.append(check("status_counts_expected", summary.get("status_counts") == EXPECTED_STATUS_COUNTS and counter(output_rows, "pending_lifecycle_decision_spread_repair_status") == EXPECTED_STATUS_COUNTS))
    checks.append(check("pending_branch_counts_expected", summary.get("before_pending_branch_counts") == EXPECTED_BEFORE_PENDING_BRANCH_COUNTS and summary.get("after_pending_branch_counts") == EXPECTED_AFTER_PENDING_BRANCH_COUNTS and counter(output_pending, "branch_decision") == EXPECTED_AFTER_PENDING_BRANCH_COUNTS))
    checks.append(check("decision_spread_status_counts_expected", summary.get("before_pending_decision_spread_status_counts") == EXPECTED_BEFORE_DECISION_SPREAD_STATUS_COUNTS and summary.get("after_pending_decision_spread_status_counts") == EXPECTED_AFTER_DECISION_SPREAD_STATUS_COUNTS and nested_status_counter(output_pending, "decision_spread_value_source_safe") == EXPECTED_AFTER_DECISION_SPREAD_STATUS_COUNTS))
    checks.append(check("source_complete_counts_expected", summary.get("before_pending_source_complete_counts") == EXPECTED_BEFORE_SOURCE_COMPLETE_COUNTS and summary.get("after_pending_source_complete_counts") == EXPECTED_AFTER_SOURCE_COMPLETE_COUNTS and counter(output_pending, "pending_lifecycle_source_capture_complete") == EXPECTED_AFTER_SOURCE_COMPLETE_COUNTS))
    checks.append(check("reconstructed_rows_expected", summary.get("reconstructed_decision_spread_rows") == len(reconstructed) == 5 and summary.get("reconstructed_decision_spread_candidates") == EXPECTED_RECONSTRUCTED_CANDIDATES))
    checks.append(check("repair_stats_expected", summary.get("decision_repair_stats", {}).get("branch_changed") == 5 and summary.get("decision_repair_stats", {}).get("source_capture_complete_changed") == 5 and summary.get("decision_repair_stats", {}).get("proxy_changed", 0) == 0))
    ...
    reconstructed_errors: list[dict[str, Any]] = []
    for row in reconstructed:
        statuses = row.get("pending_lifecycle_source_capture_statuses") or {}
        derivations = row.get("pending_lifecycle_source_capture_derivations") or {}
        if row.get("pending_lifecycle_source_capture_complete") is not True:
            reconstructed_errors.append({"row_id": row.get("row_id"), "error": "source_not_complete"})
        if statuses.get("decision_spread_value_source_safe") != "DERIVED_FROM_PENDING_LIFECYCLE_LEGACY_SPREAD_AS_DECISION_SPREAD_PROXY":
            reconstructed_errors.append({"row_id": row.get("row_id"), "error": "decision_spread_not_derived"})
        if statuses.get("decision_spread_unit") != "DERIVED_FROM_PENDING_LIFECYCLE_LEGACY_SPREAD_AS_DECISION_SPREAD_PROXY":
            reconstructed_errors.append({"row_id": row.get("row_id"), "error": "decision_spread_unit_not_derived"})
        if derivations.get("decision_spread_value_source_safe") != "DERIVED_FROM_PENDING_LIFECYCLE_LEGACY_SPREAD_AS_DECISION_SPREAD_PROXY":
            reconstructed_errors.append({"row_id": row.get("row_id"), "error": "derivation_missing"})
        if row.get("before_pending_decision_spread_repair_pending_lifecycle_source_capture_complete") is not False:
            reconstructed_errors.append({"row_id": row.get("row_id"), "error": "before_was_not_partial"})
        if row.get("before_pending_decision_spread_repair_after_proxy_r") != row.get("after_proxy_r"):
            reconstructed_errors.append({"row_id": row.get("row_id"), "error": "proxy_changed"})
    checks.append(check("reconstructed_rows_have_source_safe_derivation", not reconstructed_errors, reconstructed_errors[:20]))
    ...
    result = {
        ...
        "checks": checks,
        "plate_decision": (
            "PENDING_LIFECYCLE_DECISION_SPREAD_REPAIR_VERIFIED"
            if ok
            else "PENDING_LIFECYCLE_DECISION_SPREAD_REPAIR_REJECTED"
        ),
