"""Build row-level implementation/recompute branch actions from latest plates.

This consumes already-materialized main-orchestrator ledgers and turns their
scorer/source/tick decisions into an executable queue of keep, kill, redesign,
source-repair, and default-off implementation actions. It does not append live
shadow logs, change trading logic, or claim broker/account R.
"""
...
SOURCE_IMPL_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_SOURCE_UPGRADED_DEGRADED_BRANCH_DECISION_LEDGER_{DATE}.jsonl"

OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_IMPLEMENTATION_RECOMPUTE_BRANCH_ACTION_LEDGER_{DATE}.jsonl"
OUTPUT_SUMMARY = ROUTE_DIR / f"MAIN_ORCH24_IMPLEMENTATION_RECOMPUTE_BRANCH_ACTION_SUMMARY_{DATE}.json"
OUTPUT_MANIFEST = ROUTE_DIR / f"MAIN_ORCH24_IMPLEMENTATION_RECOMPUTE_BRANCH_ACTION_OUTPUT_MANIFEST_{DATE}.json"
...
def classify_implementation_decision(decision: str) -> str:
    ...
    if "M15_ORDERING_REPAIR" in decision:
        return "M15_ORDERING_REPAIR"
    return "REVIEW_REQUIRED"


def source_action_class(bucket: str) -> str:
    return {
        "keep_confirmed_source": "KEEP",
        "degrade_avoid_or_repair": "DOWNGRADE_OR_REPAIR",
        "m15_ordering_repair": "M15_ORDERING_REPAIR",
...
def coverage_status(action_class: str) -> str:
    return {
        ...
        "M15_ORDERING_REPAIR": "QUEUED_M15_ORDERING_REPAIR",
...
def primitive_family(source_plate: str, source_capture_surface: str | None, decision: str) -> str:
    ...
    if source_plate == "source_upgraded_degraded_branch_decisions":
...
def make_action_row(
    *,
    ...
    primitive_family_name: str | None = None,
) -> dict[str, Any]:
    coverage = coverage_status(action_class)
    return {
        "row_id": row_id,
        "route_id": ROUTE_ID,
        ...
        "primitive_family": primitive_family_name
        or primitive_family(source_plate, source_capture_surface, implementation_decision),
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for row in read_jsonl(FVG_STRUCT_LEDGER):
        ...
        generated_rows.append(
            make_action_row(
                row_id=f"MAIN-ORCH24-ACTION-{seq:05d}",
                source_plate="fvg_structural_scorer_candidate_recompute",
                source_artifact=FVG_STRUCT_LEDGER,
                source_row=row,
                action_class=action_class,
                branch_decision=str(row.get("after_branch_decision") or decision or action_class),
    ...
    for row in read_jsonl(SOURCE_IMPL_LEDGER):
        seq += 1
        bucket = str(row.get("implementation_bucket") or "")
        action_class = source_action_class(bucket)
        generated_rows.append(
            make_action_row(
                row_id=f"MAIN-ORCH24-ACTION-{seq:05d}",
                source_plate="source_upgraded_degraded_branch_decisions",
                source_artifact=SOURCE_IMPL_LEDGER,
                source_row=row,
                action_class=action_class,
                branch_decision=str(row.get("branch_decision") or ""),
                implementation_decision=str(row.get("proposed_system_use") or ""),
                current_action=str(row.get("current_main_action") or ""),
                exact_r=None,
                before_proxy_r=None,
                after_proxy_r=None,
                proxy_r_delta=None,
                data_requirement_state=row.get("data_requirement_state"),
                source_capture_surface="source_upgraded_degraded_implication",
            )
        )
...
def build_manifest(paths: list[Path]) -> dict[str, Any]:
    inputs = [FVG_STRUCT_LEDGER, ENTRY_TICK_LEDGER, PENDING_DERIVATION_LEDGER, SOURCE_IMPL_LEDGER]
    return {
        "route_id": ROUTE_ID,
        "generated_utc": utc_now(),
        "input_artifacts": {
...
def main() -> None:
    ...
    summary = {
        "route_id": ROUTE_ID,
        "generated_utc": utc_now(),
        "evidence_class": "MAIN_ORCH24_IMPLEMENTATION_RECOMPUTE_BRANCH_ACTION_LEDGER",
        "claim_boundary": (
            "Row-level branch-action queue from already-materialized scorer, tick-offset, "
            "pending-derivation, and source-implication plates. No live behavior, broker R, "
            "promotion, validation-safe, or execution change."
        ),
        "action_rows": len(rows),
        "source_plate_counts": counter(rows, "source_plate"),
        "action_class_counts": counter(rows, "action_class"),
        "coverage_status_counts": counter(rows, "coverage_status"),
        "primitive_family_counts": counter(rows, "primitive_family"),
        "branch_decision_counts": counter(rows, "branch_decision"),
        "implementation_decision_counts": counter(rows, "implementation_decision"),
        "proxy_delta_summary_by_plate": proxy_delta_summary(rows),
        "safe_flags": SAFE_FLAGS,
    }
