"""Select concrete actions from moonshot no-fill redesign proxy scores."""
...
OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_NOFILL_REDESIGN_SELECTION_LEDGER_{DATE}.jsonl"
SUMMARY_PATH = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_NOFILL_REDESIGN_SELECTION_SUMMARY_{DATE}.json"
MANIFEST_PATH = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_NOFILL_REDESIGN_SELECTION_OUTPUT_MANIFEST_{DATE}.json"
...
def read_nofill_rows(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, 1):
            if not line.strip():
                continue
            row = json.loads(line)
            if row.get("main_action_class") != "REDESIGN_SCORE":
                continue
...
def add_nofill_payload(
    ...
    *,
    ...
    selected["nofill_selection_row_id"] = f"MAIN-ORCH24-MOONSHOT-NOFILL-SELECTION-{sequence:06d}"
    ...
    selected["nofill_selection_not_standalone_promotion"] = True
    selected["missed_opportunity_audit"] = {
        "what_was_tried": "branch-local no-fill redesign proxy row was imported and selected on main",
        "why_not_live_or_independently_countable": (
            "proxy delta is branch-local redesign/control evidence; not broker actual R or validation-safe R"
        ),
        "mechanism_remains_useful": selected.get("computed_decision"),
        "downstream_path": selected.get("downstream_path"),
        "next_action": selected.get("nofill_selection_decision"),
    }
    return selected
...
def build() -> dict[str, Any]:
    ...
    input_rows = read_nofill_rows(INPUT_LEDGER)
    selected_rows = [
        add_nofill_payload(row, sequence=index, generated_utc=generated_utc, input_sha256=input_sha256)
        for index, row in enumerate(input_rows, 1)
    ]

    summary = summarize_nofill_redesign_selection(selected_rows)
    summary.update(
        {
            ...
            "input_ledger": {
                ...
                "nofill_rows": len(input_rows),
            },
            "safe_flags": SAFE_FLAGS,
            "implement_default_off_rows": summary["nofill_selection_class_counts"].get("IMPLEMENT_DEFAULT_OFF", 0),
            "redesign_rows": summary["nofill_selection_class_counts"].get("REDESIGN", 0),
            "source_repair_rows": summary["nofill_selection_class_counts"].get("SOURCE_REPAIR", 0),
            "unclassified_rows": summary["nofill_selection_class_counts"].get("UNCLASSIFIED", 0),
            "implement_default_off_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("nofill_selection_class") == "IMPLEMENT_DEFAULT_OFF"
            ),
            "redesign_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("nofill_selection_class") == "REDESIGN"
            ),
            "source_repair_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("nofill_selection_class") == "SOURCE_REPAIR"
            ),
            "nofill_class_by_source_component": {
                component: dict(
                    Counter(row["nofill_selection_class"] for row in selected_rows if row.get("source_component") == component)
                )
    ...
    manifest = {
        ...
        "date": DATE,
        "generated_utc": generated_utc,
        "description": "Concrete no-fill redesign/source-repair/default-off selection from moonshot proxy scores.",
        "input_files": {
            "moonshot_action_intake_ledger": summary["input_ledger"],
        },
        "output_files": {
            "nofill_selection_ledger": {
