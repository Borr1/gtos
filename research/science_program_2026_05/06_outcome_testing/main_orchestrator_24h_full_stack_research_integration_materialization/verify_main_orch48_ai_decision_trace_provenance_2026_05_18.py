...
EXPECTED_STATUS_ROWS = 7
EXPECTED_MALFORMED_RESPONSE_ROWS = 39
...
def verify() -> dict[str, Any]:
    ...
    if summary.get("config_enabled_rows") != 1:
        issues.append(f"config_enabled_rows_unexpected:{summary.get('config_enabled_rows')}")
    if summary.get("logger_module_present_rows") != 1:
        issues.append(f"logger_module_present_rows_unexpected:{summary.get('logger_module_present_rows')}")
    if summary.get("primary_analyzer_records_trace_rows") != 1:
        issues.append(
            f"primary_analyzer_records_trace_rows_unexpected:{summary.get('primary_analyzer_records_trace_rows')}"
        )
    if summary.get("expected_response_status_present_rows") != EXPECTED_STATUS_ROWS:
        issues.append(
            f"expected_response_status_present_rows_unexpected:{summary.get('expected_response_status_present_rows')}"
        )
    if summary.get("hash_only_self_check_passed_rows") != 1:
        issues.append(f"hash_only_self_check_passed_rows_unexpected:{summary.get('hash_only_self_check_passed_rows')}")
    ...
    if summary.get("inherited_malformed_response_rows") != EXPECTED_MALFORMED_RESPONSE_ROWS:
        issues.append(
            f"inherited_malformed_response_rows_unexpected:{summary.get('inherited_malformed_response_rows')}"
        )
    if summary.get("runtime_observability_effect_if_runtime_reenabled_rows") != 2:
        issues.append(
            "runtime_observability_effect_if_runtime_reenabled_rows_unexpected:"
            f"{summary.get('runtime_observability_effect_if_runtime_reenabled_rows')}"
        )
    ...
    expected_surfaces = {
        "ai_decision_trace_ai_architecture_inheritance",
        "ai_decision_trace_config_activation",
        "ai_decision_trace_hash_only_self_check",
        "ai_decision_trace_runtime_patch",
        "ai_decision_trace_test_coverage",
    }
    ...
    for key in (
        "ai_decision_trace_logger_enabled",
        "hash_only_prompt_response_provenance",
    ...
    for key in (
        "trading_decision_behavior_changed",
    ...
    runtime_rows = [row for row in rows if row.get("audit_surface") == "ai_decision_trace_runtime_patch"]
    if len(runtime_rows) != 1:
        issues.append(f"runtime_patch_row_count_unexpected:{len(runtime_rows)}")
    else:
        row = runtime_rows[0]
        for key in ("logger_module_present", "hash_only_schema_present", "primary_analyzer_imports_logger", "primary_analyzer_records_trace"):
            if row.get(key) is not True:
                issues.append(f"runtime_patch_{key}_not_true:{row.get(key)}")
        missing = [status for status, present in (row.get("expected_response_status_presence") or {}).items() if not present]
        if missing:
            issues.append(f"missing_response_statuses:{missing}")

    hash_rows = [row for row in rows if row.get("audit_surface") == "ai_decision_trace_hash_only_self_check"]
    if len(hash_rows) != 1:
        issues.append(f"hash_only_row_count_unexpected:{len(hash_rows)}")
    else:
        row = hash_rows[0]
        if row.get("hash_only_self_check_passed") is not True:
            issues.append("hash_only_self_check_failed")
        if row.get("stores_full_prompt_or_response_text") is not False:
            issues.append(f"hash_only_text_storage_unexpected:{row.get('stores_full_prompt_or_response_text')}")

    for row in rows:
        boundary = row.get("research_boundary") or {}
        row_id = row.get("ai_decision_trace_provenance_row_id")
        if boundary.get("trading_decision_behavior_changed"):
            issues.append(f"trading_decision_behavior_changed_claimed:{row_id}")
        for key in ("paid_api_or_vendor_call", "broker_operation", "runtime_trading_or_live_broker_effect", "runtime_candidate_use_permitted"):
            if boundary.get(key):
                ...
                break
        if boundary.get("stores_full_prompt_or_response_text"):
            issues.append(f"full_text_storage_claimed:{row_id}")
    ...
    result = {
        ...
        "issues": issues,
        "rows": len(rows),
        "config_enabled_rows": summary.get("config_enabled_rows"),
        "hash_only_self_check_passed_rows": summary.get("hash_only_self_check_passed_rows"),
        "trading_decision_behavior_changed_rows": summary.get("trading_decision_behavior_changed_rows"),
