...
def main() -> None:
    build_script = ROUTE_DIR / "build_main_orchestrator_fvg_structural_scorer_candidate_recompute_2026_05_16.py"
    verify_script = Path(__file__).resolve()
    ledger_path = ROUTE_DIR / f"MAIN_ORCH24_FVG_STRUCTURAL_SCORER_CANDIDATE_RECOMPUTE_LEDGER_{DATE}.jsonl"
    summary_path = ROUTE_DIR / f"MAIN_ORCH24_FVG_STRUCTURAL_SCORER_CANDIDATE_RECOMPUTE_SUMMARY_{DATE}.json"
    manifest_path = ROUTE_DIR / f"MAIN_ORCH24_FVG_STRUCTURAL_SCORER_CANDIDATE_RECOMPUTE_OUTPUT_MANIFEST_{DATE}.json"
    verification_path = ROUTE_DIR / f"MAIN_ORCH24_FVG_STRUCTURAL_SCORER_CANDIDATE_RECOMPUTE_VERIFICATION_RESULT_{DATE}.json"
    ...
    checks.append(
        check(
            "fvg_rows_are_scored_killed_redesigned_or_missing_source",
            not [
                row
                for row in rows
                if row.get("source_capture_surface") == "standalone_fvg_poi_scorer"
    ...
    checks.append(
        check(
            "structural_rows_are_scored_killed_or_missing_source",
            not [
                row
                for row in rows
                if row.get("source_capture_surface")
                in {"structural_lock_metadata_scorer", "swing_protected_stop_scorer"}
                and row.get("after_score_status") == "SCORER_NOT_IMPLEMENTED"
            ],
        )
    )
    ...
    fvg_surface = surface_summary.get("standalone_fvg_poi_scorer", {})
    swing_surface = surface_summary.get("swing_protected_stop_scorer", {})
    checks.append(
        check(
            "standalone_fvg_poi_surface_has_numeric_and_kill_decisions",
            fvg_surface.get("after_numeric_proxy_rows") == 6
            and round(float(fvg_surface.get("after_proxy_r_sum", 0.0)), 10) == -6.0
            and fvg_surface.get("implementation_decision_counts", {}).get(
                "KILL_STANDALONE_FVG_SCORER_FOR_NON_FVG_POI_ROWS"
            )
            == 144,
            fvg_surface,
        )
    ...
    checks.append(
        check(
            "swing_protected_surface_has_confirmed_numeric_and_kill_decisions",
            swing_surface.get("after_numeric_proxy_rows") == 49
            and round(float(swing_surface.get("after_proxy_r_sum", 0.0)), 10) == 19.0
            and swing_surface.get("implementation_decision_counts", {}).get(
                "KILL_SWING_PROTECTED_SCORER_FOR_UNPROTECTED_STOP_ROWS"
            )
            == 14,
            swing_surface,
        )
    )
    previous_delta = summary.get("delta_vs_previous_shared_metadata_checkpoint", {})
    checks.append(
        check(
            "stricter_candidate_gates_reduce_broad_metadata_credit",
            previous_delta.get("after_numeric_proxy_row_delta_vs_previous") == -116
            and round(float(previous_delta.get("after_proxy_r_sum_delta_vs_previous", 0.0)), 10) == -29.0,
            previous_delta,
        )
    )
    ...
    result = {
        ...
        "plate_decision": "FVG_STRUCTURAL_SCORER_IMPLEMENTATION_CANDIDATES_RECOMPUTED_WITH_CURRENT_ROW_PROXY_R_DELTAS",
        "can_mark_fvg_structural_scorer_candidate_recompute_plate_complete": ok,
