...
DOSSIER_SUMMARY = ROUTE_DIR / f"MAIN_ORCH48_FINAL_REVIEW_SELECTOR_PROD_CHANGE_DOSSIER_SUMMARY_{DATE}.json"
...
CODE_SURFACES = [
    ...
    Path(
        ...
        "build_main_orch48_ai_narrowing_policy_2026_05_18.py"
    ),
    Path(
        "research/science_program_2026_05/06_outcome_testing/"
        "main_orchestrator_24h_full_stack_research_integration_materialization/"
        "verify_main_orch48_ai_narrowing_policy_2026_05_18.py"
    ),
]
...
def build() -> dict[str, Any]:
    ...
    dossier_summary_sha = sha256_path(DOSSIER_SUMMARY)
    dossier_summary = read_json(DOSSIER_SUMMARY)
    dossier_rows = read_jsonl_with_line_no(DOSSIER_LEDGER)

    policy_rows: list[dict[str, Any]] = []
    for source_line_no, dossier_row in dossier_rows:
        policy_rows.append(
            final_review_selector_ai_narrowing_policy_row(
                dossier_row,
                policy_row_id=f"MAIN-ORCH48-AI-NARROWING-POLICY-{len(policy_rows) + 1:08d}",
                source_artifact=display_path(DOSSIER_LEDGER),
                source_line_no=source_line_no,
                source_sha256=dossier_sha,
            )
        )
    write_jsonl(OUTPUT_LEDGER, policy_rows)

    policy_summary = summarize_final_review_selector_ai_narrowing_policy(policy_rows)
    summary = {
        "route_id": ROUTE_ID,
        "schema_version": SCHEMA_VERSION,
        "generated_utc": utc_now(),
        "main_repo_head_at_build": run_git_in_repo("rev-parse", "HEAD"),
        "input_selector_production_change_dossier_ledger": {
            "path": display_path(DOSSIER_LEDGER),
            "rows": count_lines(DOSSIER_LEDGER),
            "sha256": dossier_sha,
        },
        "input_selector_production_change_dossier_summary": {
            "path": display_path(DOSSIER_SUMMARY),
            "sha256": dossier_summary_sha,
            "dossier_rows": dossier_summary.get("rows"),
            "production_change_dossier_status_counts": dossier_summary.get("production_change_dossier_status_counts"),
        },
        "code_surfaces": code_surface_summary(),
        **policy_summary,
        "implementation_effect": {
            "ai_narrowing_policy_default_off_available": True,
            "default_off_mechanical_selector_scope_rows_for_review": policy_summary["ai_narrowing_review_ready_rows"],
            "capacity_blocklist_required_before_ai_narrowing_rows": policy_summary[
    ...
    return {
        "ok": True,
        "route_id": ROUTE_ID,
        "policy_rows": summary["rows"],
        "ai_narrowing_review_ready_rows": summary["ai_narrowing_review_ready_rows"],
        "capacity_blocklist_required_before_ai_narrowing_rows": summary[
            "capacity_blocklist_required_before_ai_narrowing_rows"
        ],
        "live_ai_runtime_change_now_rows": summary["live_ai_runtime_change_now_rows"],
    }
