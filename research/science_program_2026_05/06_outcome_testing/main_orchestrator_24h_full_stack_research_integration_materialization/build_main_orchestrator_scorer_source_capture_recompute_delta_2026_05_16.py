"""Recompute repaired scorer/source-capture surfaces against current rows.

This builder is read-only for shadow logs. It materializes the current-row
before/after delta caused by scorer/source-capture repairs without appending to
live shadow logs or changing live behavior.
"""
...
OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_SCORER_SOURCE_CAPTURE_RECOMPUTE_DELTA_LEDGER_{DATE}.jsonl"
OUTPUT_SUMMARY = ROUTE_DIR / f"MAIN_ORCH24_SCORER_SOURCE_CAPTURE_RECOMPUTE_DELTA_SUMMARY_{DATE}.json"
OUTPUT_MANIFEST = ROUTE_DIR / f"MAIN_ORCH24_SCORER_SOURCE_CAPTURE_RECOMPUTE_DELTA_OUTPUT_MANIFEST_{DATE}.json"
...
def source_capture_surface(strategy_id: str) -> str:
    if strategy_id in FVG_REQUIRED_IDS:
        return "standalone_fvg_metadata_scorer"
...
def implementation_decision(row: dict[str, Any]) -> str:
    ...
    if strategy_id in FVG_REQUIRED_IDS:
        if status == "SCORED_FVG_METADATA_PROXY_SHARED_CANDIDATE_PATH":
            if is_numeric(row.get("strategy_proxy_r")):
                return "IMPLEMENT_DEFAULT_OFF_FVG_METADATA_SHARED_PATH_PROXY_SCORER_NOW"
            return "IMPLEMENT_DEFAULT_OFF_FVG_METADATA_SCORER_WITH_AMBIGUITY_EXCLUSION"
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for cid in sorted(candidates):
        ...
        for strategy_id in PRIORITY_STRATEGIES:
            ...
            out.append(
                {
                    "row_id": f"MAIN-ORCH24-SCORER-DELTA-{len(out) + 1:05d}",
                    "route_id": ROUTE_ID,
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_class": "MAIN_ORCH24_REPAIRED_SCORER_SOURCE_CAPTURE_CURRENT_ROW_RECOMPUTE",
        "claim_boundary": (
            "Current latest-row recompute of repaired default-off scorer/source-capture surfaces only. "
            "Exact broker R is not opened; proxy R is from existing candidate path, LTF, and pending lifecycle evidence."
        ),
