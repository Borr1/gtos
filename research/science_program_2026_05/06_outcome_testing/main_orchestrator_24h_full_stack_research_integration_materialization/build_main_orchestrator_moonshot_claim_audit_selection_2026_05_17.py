"""Select concrete actions from moonshot current-claim audit and recheck rows."""
...
OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_CLAIM_AUDIT_SELECTION_LEDGER_{DATE}.jsonl"
SUMMARY_PATH = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_CLAIM_AUDIT_SELECTION_SUMMARY_{DATE}.json"
MANIFEST_PATH = ROUTE_DIR / f"MAIN_ORCH24_MOONSHOT_CLAIM_AUDIT_SELECTION_OUTPUT_MANIFEST_{DATE}.json"
...
def read_claim_audit_rows(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, 1):
            if not line.strip():
                continue
            row = json.loads(line)
            if row.get("main_action_class") not in {"CURRENT_CLAIM_REJECTION", "RECHECK"}:
                continue
...
def add_claim_audit_payload(
    ...
    *,
    ...
    selected["claim_audit_selection_row_id"] = f"MAIN-ORCH24-MOONSHOT-CLAIM-AUDIT-SELECTION-{sequence:06d}"
    ...
    selected["claim_audit_selection_not_standalone_promotion"] = True
    selected["missed_opportunity_audit"] = {
        "what_was_tried": "branch-local current-claim audit or recheck row was imported and selected on main",
        "why_not_live_or_independently_countable": (
            "audit/recheck row has no computable R and rejects or repairs only the current claim"
        ),
        "mechanism_remains_useful": selected.get("computed_decision"),
        "downstream_path": selected.get("downstream_path"),
        "next_action": selected.get("claim_audit_selection_decision"),
    }
    return selected
...
def build() -> dict[str, Any]:
    ...
    input_rows = read_claim_audit_rows(INPUT_LEDGER)
    selected_rows = [
        add_claim_audit_payload(row, sequence=index, generated_utc=generated_utc, input_sha256=input_sha256)
        for index, row in enumerate(input_rows, 1)
    ]

    summary = summarize_claim_audit_selection(selected_rows)
    summary.update(
        {
            ...
            "input_ledger": {
                ...
                "claim_audit_rows": len(input_rows),
            },
            "safe_flags": SAFE_FLAGS,
            "redesign_rows": summary["claim_audit_selection_class_counts"].get("REDESIGN", 0),
            "source_repair_rows": summary["claim_audit_selection_class_counts"].get("SOURCE_REPAIR", 0),
            "unclassified_rows": summary["claim_audit_selection_class_counts"].get("UNCLASSIFIED", 0),
            "redesign_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("claim_audit_selection_class") == "REDESIGN"
            ),
            "source_repair_proxy_delta_reference_sum_not_r": proxy_sum(
                selected_rows, lambda row: row.get("claim_audit_selection_class") == "SOURCE_REPAIR"
            ),
            "claim_audit_class_by_main_action": {
                action_class: dict(
                    Counter(
                        row["claim_audit_selection_class"]
                        for row in selected_rows
                        if row.get("main_action_class") == action_class
                    )
                )
                for action_class in sorted({str(row.get("main_action_class")) for row in selected_rows})
            },
            "research_safety": {
                "opens_exact_r": False,
    ...
    manifest = {
        ...
        "date": DATE,
        "generated_utc": generated_utc,
        "description": "Concrete current-claim audit and recheck selection from moonshot audit rows.",
        "input_files": {
            "moonshot_action_intake_ledger": summary["input_ledger"],
        },
        "output_files": {
            "claim_audit_selection_ledger": {
