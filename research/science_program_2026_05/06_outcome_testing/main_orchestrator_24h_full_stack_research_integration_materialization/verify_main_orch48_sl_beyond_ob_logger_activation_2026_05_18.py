...
FIELD_LEDGER = ROUTE_DIR / f"{ROUTE_ID}_LEDGER_{DATE}.jsonl"
...
def verify() -> dict[str, Any]:
    issues: list[str] = []
    for path in (FIELD_LEDGER, SUMMARY, MANIFEST):
        if not path.exists():
            issues.append(f"missing_output:{path.name}")

    rows = read_jsonl(FIELD_LEDGER) if FIELD_LEDGER.exists() else []
    ...
    if config_enabled() is not True:
        issues.append("config_logger_not_enabled")
    if summary.get("config_sl_beyond_ob_decisions_logger_enabled") is not True:
        issues.append("summary_config_logger_not_enabled")
    for key in ("logger_wiring_present", "logger_has_config_gate", "logger_failure_isolated", "logger_skips_skip_rows"):
    ...
    for path in (FIELD_LEDGER, SUMMARY):
    ...
    result = {
        ...
        "config_sl_beyond_ob_decisions_logger_enabled": summary.get(
            "config_sl_beyond_ob_decisions_logger_enabled"
        ),
