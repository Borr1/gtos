...
def verify() -> dict[str, Any]:
    ...
    if summary.get("contract_rows") != len(rows):
        issues.append(f"summary_rows_mismatch:{summary.get('contract_rows')}:{len(rows)}")

    for key in (
        "required_fields_in_common_metadata",
        "required_fields_in_synthetic_candidate_row",
        "orchestrator_passes_source_symbol",
        "forward_capture_test_asserts_fields",
        "runtime_halt_active",
    ):
    ...
    if summary.get("synthetic_event_missing_required_fields") != []:
        issues.append(f"synthetic_event_missing_fields:{summary.get('synthetic_event_missing_required_fields')}")

    effect = summary.get("implementation_effect") or {}
    if effect.get("future_strategy_follow_candidate_rows_contract_complete") is not True:
        issues.append("effect_future_strategy_follow_candidate_rows_contract_complete_not_true")
    for key in (
