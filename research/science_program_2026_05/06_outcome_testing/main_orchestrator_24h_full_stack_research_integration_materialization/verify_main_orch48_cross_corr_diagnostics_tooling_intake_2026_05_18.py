...
def verify() -> dict[str, Any]:
    ...
    if len(rows) != EXPECTED_ROWS:
        issues.append(f"intake_rows_unexpected:{len(rows)}")
    if summary.get("intake_rows") != len(rows):
        issues.append(f"summary_rows_mismatch:{summary.get('intake_rows')}:{len(rows)}")
    ...
    for key in (
        ...
        "missing_is_optional",
        "verify_reads_source",
        "backfill_reads_source",
        "followup_matrix_includes_source",
        "join_tests_cover_source",
        "followup_tests_cover_source",
        "runtime_halt_active",
    ):
    ...
    if effect.get("diagnostics_tooling_only") is not True:
        issues.append("effect_diagnostics_tooling_only_not_true")
    for key in (
