"""Verify SOURCE accepted branch-label repair."""
...
from build_main_orchestrator_source_accepted_branch_label_repair_2026_05_17 import (  # noqa: E402
    BRANCH_REPAIRS,
    sha256_file,
    write_json,
)
...
VERIFY_RESULT = ROUTE_DIR / f"MAIN_ORCH24_ACTION_AFTER_SOURCE_ACCEPTED_BRANCH_LABEL_REPAIR_VERIFICATION_RESULT_{DATE}.json"
...
def main() -> None:
    ...
    append_check(checks, "repaired_count", len(repaired) == 39, len(repaired))
    append_check(checks, "stale_keep_branch_left_zero", len(stale_rows) == 0, len(stale_rows))
    append_check(
        checks,
        "repaired_branch_labels_expected",
        Counter(row.get("branch_decision") for row in repaired)
        == {
            "IMPLEMENT_DEFAULT_OFF_SOURCE_ACCEPTED_AFTER_M15_ORDERING_POSITIVE_PROXY": 20,
            "IMPLEMENT_DEFAULT_OFF_SOURCE_ACCEPTED_CONFIRMED_REPLAY_REQUIRES_EXACT_SOURCE": 19,
        },
        dict(Counter(row.get("branch_decision") for row in repaired)),
    )
    append_check(checks, "repaired_numeric_rows", len(repaired_values) == 20, len(repaired_values))
    append_check(checks, "repaired_numeric_sum", round(sum(repaired_values), 8) == 4.367443, round(sum(repaired_values), 8))
    ...
    result = {
        "schema_version": "main_orch24_source_accepted_branch_label_repair_verifier_v1",
        "verified": verified,
        "decision": "SOURCE_ACCEPTED_BRANCH_LABEL_REPAIR_VERIFIED"
        if verified
        else "SOURCE_ACCEPTED_BRANCH_LABEL_REPAIR_REJECTED",
        "checks": checks,
        "summary": {
            "rows": len(rows),
