...
LEDGER = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_FIELD_AVAILABILITY_LEDGER_{DATE}.jsonl"
SUMMARY = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_FIELD_AVAILABILITY_SUMMARY_{DATE}.json"
MANIFEST = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_FIELD_AVAILABILITY_MANIFEST_{DATE}.json"
VERIFY_RESULT = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_EVENT_FIELD_AVAILABILITY_VERIFY_RESULT_{DATE}.json"
...
def verify() -> dict[str, Any]:
    issues: list[str] = []
    for path in (CONTRACT_LEDGER, LEDGER, SUMMARY, MANIFEST):
        if not path.exists():
    ...
    required_universe = set(summary.get("required_field_universe") or [])
    expected_fields = {
        "horizon_id",
        "primitive_flag",
        "proxy_r_class",
    ...
    if required_universe != expected_fields:
        issues.append("required_field_universe_unexpected")
    ...
    if input_contract_ledger.get("rows") != 1072:
        issues.append("input_contract_ledger_rows_unexpected")
    ...
    for row in rows:
        row_id = row.get("availability_row_id")
    ...
    result = {
        ...
        "aggregate_field_presence_counts": summary.get("aggregate_field_presence_counts") or {},
