...
EXPECTED_ROWS = 127
EXPECTED_JOINED_ROWS = 32
EXPECTED_MISSING_ROWS = 95
EXPECTED_JOINED_REJECT_ROWS = 5
EXPECTED_JOINED_REJECT_NO_FILLABLE_MISSED_R_ROWS = 5
...
def verify() -> dict[str, Any]:
    ...
    if summary.get("status") != "OK_TOUCH_COUNT_GATE_OUTCOME_JOIN_MATERIALIZED":
    ...
    if (summary.get("touch_count_log") or {}).get("rows") != EXPECTED_ROWS:
        issues.append(f"touch_log_rows_unexpected:{(summary.get('touch_count_log') or {}).get('rows')}")
    ...
    if join_summary.get("joined_reject_no_fillable_missed_r_rows") != EXPECTED_JOINED_REJECT_NO_FILLABLE_MISSED_R_ROWS:
        issues.append(
            "joined_reject_no_fillable_missed_r_rows_unexpected:"
            f"{join_summary.get('joined_reject_no_fillable_missed_r_rows')}"
        )
    ...
    if summary.get("gate_action") != "KEEP_TOUCH_COUNT_THRESHOLD_2_AND_REVIEW_SOURCE_CAPTURE_GAP":
        issues.append(f"gate_action_unexpected:{summary.get('gate_action')}")
    if summary.get("current_config_threshold") != 2:
        issues.append(f"current_config_threshold_unexpected:{summary.get('current_config_threshold')}")
    ...
    for row in rows:
        row_id = row.get("touch_count_gate_outcome_join_row_id")
        for key in (
            "runtime_decision_effect",
            "production_change_opened_now",
            "gate_config_change_now",
    ...
    result = {
        ...
        "joined_reject_positive_fillable_proxy_rows": join_summary.get("joined_reject_positive_fillable_proxy_rows"),
