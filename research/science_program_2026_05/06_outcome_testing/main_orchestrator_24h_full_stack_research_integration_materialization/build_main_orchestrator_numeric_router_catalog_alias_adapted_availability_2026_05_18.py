...
OUTPUT_LEDGER = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_ALIAS_ADAPTED_AVAILABILITY_LEDGER_{DATE}.jsonl"
OUTPUT_SUMMARY = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_ALIAS_ADAPTED_AVAILABILITY_SUMMARY_{DATE}.json"
OUTPUT_MANIFEST = ROUTE_DIR / f"MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_ALIAS_ADAPTED_AVAILABILITY_MANIFEST_{DATE}.json"
...
def build() -> dict[str, Any]:
    ...
    for index, relative_path in enumerate(SOURCE_PATHS, start=1):
        ...
        event_field_source_counts: Counter[str] = Counter()
            ...
            with path.open("r", encoding="utf-8") as handle:
                for line in handle:
                    ...
                    event = numeric_router_catalog_event_from_source_row(
                        source_row,
                        source_artifact=str(relative_path).replace("\\", "/"),
                        source_line_no=rows_scanned,
                        required_event_fields=required_field_universe,
                    )
                    available_fields = {
                        field for field in required_field_universe if event.get(field) not in (None, "")
                    }
                    field_presence_counts.update(available_fields)
                    event_field_source_counts.update(
                        f"{field}:{source_alias}" for field, source_alias in event.get("event_field_sources", {}).items()
                    )
                    if available_fields:
        ...
        audit_rows.append(
            {
                "alias_availability_row_id": f"MAIN-ORCH48-NUMERIC-ROUTER-CATALOG-ALIAS-AVAILABILITY-{index:04d}",
                ...
                "alias_adapted_field_presence_counts": dict(sorted(field_presence_counts.items())),
                "event_field_source_counts": dict(sorted(event_field_source_counts.items())),
    ...
    aggregate_source_counts: Counter[str] = Counter()
    for row in audit_rows:
        aggregate_field_counts.update(row["alias_adapted_field_presence_counts"])
        aggregate_source_counts.update(row["event_field_source_counts"])
    summary = {
        "route_id": "MAIN_ORCH48_NUMERIC_ROUTER_CATALOG_ALIAS_ADAPTED_AVAILABILITY",
        "schema_version": "main_orch48_numeric_router_catalog_alias_adapted_availability_v1",
        ...
        "input_adapter_spec_ledger": {
            ...
            "rows": count_lines(ADAPTER_SPEC_LEDGER),
        ...
        "contract_rows": contract_rows,
        "required_field_universe": required_field_universe,
        "aggregate_alias_adapted_field_presence_counts": dict(sorted(aggregate_field_counts.items())),
        "aggregate_event_field_source_counts": dict(sorted(aggregate_source_counts.items())),
