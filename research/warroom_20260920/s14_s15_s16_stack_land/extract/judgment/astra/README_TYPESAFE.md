...
9. **Challenge fluid-gate A1 (this tree):** shadow-first `ALIVE_MENU` +
   ...
   S14 `REGIME_GATE_SHADOW` composes on the same sidecar (buckets + one
   System One pack + label compose). S15 `COST_OF_ERROR` logs tape costs
   + CONF_GATE SHADOW bands beside those rows (vendor 0.85 is not truth).
   Historical prove:
   [`../S14_REGIME_GATE_SHADOW.md`](../S14_REGIME_GATE_SHADOW.md),
   [`../S15_COST_OF_ERROR.md`](../S15_COST_OF_ERROR.md).
