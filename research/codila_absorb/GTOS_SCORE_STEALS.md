...
| S11 | SuperQode frozen question packs + compose thresholds + verdict hash (tool_gate) | LABEL `FROZEN_PACK_COMPOSE` |
| S12 | jev-cli / OpenRouter / MCP for Chair off-place judgments | LABEL `CHAIR_JEV_CLI` |
| S13 | SimpleJev / SGLang `/v1/score` as **lab** self-host twin after TypeSafe Challenge prove | LABEL `SELFHOST_AFTER_PROVE` |

---


### S14 — Regime / sleeve gate from bucketed features  `[a]`
**Steal:** Encode book/tape into **bucketed** features (no raw tick dump) → one System One call with Choice `regime_type` + Noul `regime_change_likely` + Noul `strategy_viable` → **code** composes `stand_down` | `trade` | `half_size` + `size_factor`. Cache by state hash for replay.

**GTOS map:**
- Shadow fluid sleeve admit / corr HOLD / size_tilt factor from regime pack over Challenge `0` tape.
- Escape hatches already in S1: HOLD / ABSTAIN / ESCALATE_CHAIR / BLOCKED.
- **Never** map `trade` branch to broker/`order_send`.

**Chair LABEL:** `REGIME_GATE_SHADOW` — regime/admit/size labels from typed pack; place stays integers.

**Evidence:** matchstick-trading/jev-regime-gate (`encoder.ts`, `jev-client.ts`, `gate.ts`); WAR_ROOM_WAVE2.

---

### S15 — Cost-of-error thresholds (not magic confidence)  `[b]`
**Steal:** Author `cost_false_yes` / `cost_false_no` / `cost_human`; derive YES/NO/UNSURE from expected cost instead of copying vendor 0.85.

**GTOS map:**
- Calibrate S2 CONF_GATE bands on Challenge-labeled false-admit vs false-abstain vs Chair-review costs.
- Place-adjacent still **VETO** regardless of cost math.

**Chair LABEL:** `COST_OF_ERROR` — thresholds from Challenge costs, not blog numbers.

**Evidence:** Kungie/gut cost rule; WAR_ROOM_WAVE2.

---

### S16 — Multi-stage guardrail screens  `[c]`
**Steal:** Independent Jev screens on input / tool-pre / observation / response / claim-verify; **code policy** owns allow/review/block; heuristics never override a Jev block.

**GTOS map:**
- Chair/Dig tool path only (off broker). Complements S11 frozen packs + S4 DONE_OUTSIDE.
- Do not install as Challenge place wall.

**Chair LABEL:** `MULTI_STAGE_GUARD` — staged screens; policy owns disposition.

**Evidence:** codebam/jev-guardrails; WAR_ROOM_WAVE2.
...
9. **No mid-prove swap** to SimpleJev / classifier.dev / SGLang-as-Jev while P0-2 / TypeSafe prove is in flight.
10. **Hazem ms / $0.042** = gate budget ceilings only — not Challenge edge.
...
## Wave3 deepen (2026-09-20T08:33 ICT) — consolidation, no new S##

Chair rank for Challenge `0` shadow wires: **S14 → S15 → S16**  
Details: `war_room/WAR_ROOM_WAVE3_20260920.md` · `war_room/CHAIR_NEXT_SHADOW_WIRES.md`

| Steal | Deepen delivered |
| --- | --- |
| S14 | Bucket field map → `gold_state.v0` / harvest; concrete shadow wire JSON |
| S15 | Challenge-label cost matrix on FLUID-ADM / SIZ / PLC (no vendor 0.85) |
| S16 | Five screens → Dig/Chair MCP/tool path only (off place) |
