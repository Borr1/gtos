...
|---|---:|---:|---:|---:|---:|
| displacement_quality_score | 0.1098 | +0.0016 | 38.0% | [0.0371, 0.2413] | 2 |
| framework_id | 0.0972 | -0.0029 | 33.6% | [0.0353, 0.2155] | 1 |
| session_id | 0.0821 | +0.0013 | 28.4% | [0.0250, 0.1879] | 3 |
...
| hour_of_day_utc | 0.0000 | +0.0000 | 0.0% | [0.0000, 0.0000] | 4 |
| regime_tag | 0.0000 | +0.0000 | 0.0% | [0.0000, 0.0000] | 8 |

## H1 vs H2 importance-delta test (Bonferroni-corrected)

H1 / H2 sample sizes: **107 / 108** filled trades. One-sided Welch's t-test on bootstrap distributions of mean |SHAP| (H0: H1 ≤ H2). Bonferroni family size = 8, α = 0.050. Bootstrap resamples per half: 100.

| Feature | H1 |SHAP| | H2 |SHAP| | delta | raw p | bonf p | status |
|---|---:|---:|---:|---:|---:|---|
| ob_retest_distance_atr | 0.0000 | 0.0000 | +0.0000 | — | — | stable |
| displacement_quality_score | 0.1740 | 0.1227 | +0.0513 | 2.11e-06 | 1.69e-05 | DECAYED |
| fvg_present | 0.0000 | 0.0000 | +0.0000 | — | — | stable |
| touch_count | 0.0000 | 0.0000 | +0.0000 | — | — | stable |
| framework_id | 0.1199 | 0.1063 | +0.0136 | 0.0471 | 0.3766 | suggestive |
| session_id | 0.0853 | 0.1463 | -0.0611 | 1.0000 | 1.0000 | stable |
...
- **Top R contributor (proper SHAP)**: `displacement_quality_score` (38.0% of total |SHAP| mass)
- **Top negative contributor**: `framework_id` (mean SHAP -0.0029)
- **Most decayed H1→H2 (Bonferroni α=0.050)**: `displacement_quality_score` (delta +0.0513, bonf p = 1.69e-05)

## Caveats

- SHAP rank vs Bayesian-magnitude rank Spearman r = `0.833` (n=8).
