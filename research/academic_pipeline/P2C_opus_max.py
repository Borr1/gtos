#!/usr/bin/env python3
"""Phase 2C — Opus 4.6 Model Test: P2C_opus_max
Model: claude-opus-4-6, Effort: max, max_tokens: 2000, temperature: 0
Tests Opus 4.6 on the v1 re-engineered prompt (same 121 MSOs, no session memory).
Baseline: P2A-3 Sonnet 4.6 max (CR=38.0%, WR=69.6%, CR*WR=0.265)
...
# ── Cost constants (Opus 4.6) ──────────────────────────
INPUT_COST_PER_TOK  = 15.0 / 1_000_000   # $15/MTok
OUTPUT_COST_PER_TOK = 75.0 / 1_000_000   # $75/MTok
...
MODEL_FALLBACKS = ['claude-opus-4-6-latest']
...
def evaluate_mso(system_prompt: str, user_message: str, trade_id: str = '') -> dict:
    """Call Opus 4.6 with effort=max and the new prompt, return parsed JSON."""
    global cumulative_cost
...
effort_note = ('' if USE_EFFORT
                    ...
                    'by the SDK. Results are Opus 4.6 at default effort.\n')
...
| **{TEST_NAME} (Opus 4.6 max)** | **{pct(cr)}** | **{pct(cand_wr)}** | **{cr_wr:.4f}** | **~${avg_cost:.3f}** |
