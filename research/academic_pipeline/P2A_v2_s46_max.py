#!/usr/bin/env python3
"""Phase 2A-v2 — Evidence-Based Prompt Fixes: P2A_v2_s46_max
Model: claude-sonnet-4-6, Effort: max, max_tokens: 2000, temperature: 0
Tests 4 evidence-backed fixes: Q4 scoring, confidence mapping, grade determinism, London calibration.
Baseline: P2A-3 (CR=38.0%, WR=69.6%, CR*WR=0.265)
...
- London session setups have a historically higher base win rate. Apply the scoring criteria as-is — do not add unscored caution beyond what the framework captures.
...
Q4. PREMIUM/DISCOUNT POSITION: Is the OB within the impulse range?
  - OB on the correct side (discount for longs, premium for shorts) OR at equilibrium (45-55% of impulse): 5 points
  - OB on the wrong side (beyond the impulse extreme against trade direction): 0 points
...
## CONFIDENCE SCORE
Confidence equals your total evaluation score. Report it directly.
Show your score breakdown in the confidence_computation field, e.g.: "Q1=15 Q2=20 Q3=10 Q4=5 Q5=15 Q6=10 Q7=5 +zone_first=5 = 85 → conf 85"

## SETUP GRADE
Grade is determined solely by score. Do not override the grade based on qualitative reasoning — the score already captures setup quality.
  - Score 85+: A+
  - Score 75-84: A
  - Score 65-74: B+
  - Score 50-64: B
...
| **{TEST_NAME} (new prompt v2)** | **{pct(cr)}** | **{pct(cand_wr)}** | **{cr_wr:.4f}** |
