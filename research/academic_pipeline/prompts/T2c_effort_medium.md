...
**YOU are testing effort level: `medium`**

This is already set. Do NOT change it. Each agent runs independently on the same 121 trades.
...
**Single variable:** Sonnet 4.6 at effort=`medium` (temperature=0, no extended thinking).
...
EFFORT_LEVEL = 'medium'  # One of: 'medium', 'high', 'max'
...
- Projected monthly cost at 17 trades/month: ${monthly_cost}
...
- **Effort: `medium`** — do NOT run other effort levels
...
### Environment Setup
```bash
pip install --upgrade anthropic pandas scipy numpy
```
...
**If the model ID `claude-sonnet-4-6-20250514` fails:** Try these in order: `claude-sonnet-4-6`, `claude-sonnet-4-6-latest`. Use whichever works first and report the working ID in the results metadata. This is critical — we need to know the exact model ID for production deployment.
...
- `research/academic_pipeline/T2c_effort_medium.py` — The analysis script (save your code)
