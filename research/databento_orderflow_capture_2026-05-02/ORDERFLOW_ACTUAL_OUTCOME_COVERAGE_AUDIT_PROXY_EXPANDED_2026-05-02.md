...
- Orderflow feature candidate rows: 54
- Orderflow synthetic target available: 22
- Orderflow actual realized-R available: 1
- Coverage class counts: {'actual_realized_r_available': 1, 'candidate_join_missing': 19, 'limit_placed_no_broker_close_in_join': 3, 'no_actual_by_design_pre_execution_reject': 31}
- Final outcome counts: {'LIMIT_PLACED': 4, 'REJECTED_GATE1_SAFETY': 1, 'REJECTED_L2': 30, 'none': 19}
- Record path status counts: {'exists': 35, 'no_path': 19}

## Readout

- Orderflow feature candidate rows have actual realized-R coverage 1/54; synthetic target coverage is 22/54.
- The actual-R blocker is mostly structural, not a parser miss: rejected candidates do not create broker exits.
- 3 orderflow rows were LIMIT_PLACED without actual R in the join; those are the only near-term broker-history enrichment targets.
- 31 orderflow rows were rejected before execution, so they can only be evaluated with synthetic/path labels.
...
|---|---:|---:|---:|---|---|
| GBPUSD | 21 | 9 | 0 | {'limit_placed_no_broker_close_in_join': 2, 'no_actual_by_design_pre_execution_reject': 19} | {'LIMIT_PLACED': 2, 'REJECTED_L2': 19} |
...
| XAGUSD | 10 | 0 | 0 | {'candidate_join_missing': 10} | {'none': 10} |
