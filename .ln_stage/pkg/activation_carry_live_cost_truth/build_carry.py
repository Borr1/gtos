...
def build_payloads() -> dict[str, bytes]:
    payloads = {path: (REPO / path).read_bytes() for path in NEW_FILES}
...
def build_manifest(payloads: dict[str, bytes]) -> dict:
    ...
    return {
        ...
        "not_carried": [
            {
                ...
                "sha256_unchanged": sha((REPO / "config/agent_config.yaml").read_bytes()),
            ...
            },
            {
                ...
                "sha256_unchanged": sha(
                    (REPO / "config/profiles/operator_profile.yaml").read_bytes()
                ),
            ...
            },
            {
                ...
                "sha256_unchanged": sha(
                    (REPO / "config/profiles/redacted_account.yaml").read_bytes()
                ),
        ...
        "seal_exposure": {
            ...
            "mainline_sha256_after_CN": sha((REPO / ENGINE_PATH).read_bytes()),
